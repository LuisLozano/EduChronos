...
- L754 — ### Sesión 163 — O-curso (H4), C-windows-curso: **LAS CONDICIONES 1 A 5 SE CUMPLEN EN EL BUNDLE, EN WINDOWS 11 CON CUENTA ESTÁNDAR, CONSTRUIDO EN UNA MÁQUINA VIRTUAL NUEVA: `O-curso` TERMINADO, 9 DE 9.**
...
### Sesión 163 — O-curso (H4), C-windows-curso: **LAS CONDICIONES 1 A 5 SE CUMPLEN EN EL BUNDLE, EN WINDOWS 11 CON CUENTA ESTÁNDAR, CONSTRUIDO EN UNA MÁQUINA VIRTUAL NUEVA: `O-curso` TERMINADO, 9 DE 9.**
...
Última sesión registrada (previa): Sesión 162 — O-curso (H4), C-e2e-curso: **EL E2E DUPLICA, CAMBIA DE CURSO Y VE RECHAZADA UNA ESCRITURA EN EL ARCHIVADO, Y LA SUITE PASA A CORRER EN SERIE CON CARPETA DE DATOS PROPIA: `O-curso` EN 8 DE 9.**
  TIPO Y RITUAL: DESARROLLO — M0 + M2 de solo lectura + fase única (configuración y spec) con corridas, M3 por mutación del producto y suites + M1. El M4 es la propia corrida del e2e.
  M0. Cambio `C-e2e-curso`, Objetivo `O-curso`, Hito H4; sin R-invalidación: la Fase 12 ejecutará este spec, no lo rehará, con la condición de que el aislamiento sirva también en CI. Va antes que Windows (condición 9) por riesgo —tenía la única decisión de diseño abierta del objetivo— y porque Windows tiene que verificar el código final.
  MEDICIÓN (M2, HEAD `02d7f13`, Claude Code, solo lectura). (1) Con URL explícita, el duplicado crea `curso-AAAA-AAAA.db` en la carpeta de la base abierta, y el `rm -f app/educhronos-e2e.db*` no lo alcanzaba: la segunda corrida habría dado 409 `CURSO_YA_EXISTE`. (2) El listado de cursos del e2e incluía `app/educhronos.db`, de desarrollo, con su botón «Abrir». (3) El puntero `curso-abierto` ni se escribe ni se lee con URL explícita. (4) Cambiar de curso cambia la base de todo el proceso: con `fullyParallel: true`, dos specs se pisan siempre, y no sólo por los `unique` que preveía la ficha de la deuda. (5) Duplicar archiva la base de arranque y no hay vía para desarchivarla. (6) Basta una base vacía: sin nombre, el diálogo pide el del curso actual. (7) La escritura más simple que rechaza la guarda es el alta de nivel, y su mensaje llega al formulario.
  DECISIONES. Suite en serie (`fullyParallel: false`, `workers: 1`). Base del e2e en `app/target/e2e/`, con ruta absoluta y borrada entera en cada corrida. Invariante de orden: el spec que archiva la base de arranque corre después de todo spec que escriba, por el orden alfabético de ficheros que documenta Playwright sin paralelismo. Descartado un backend por spec (dos `spring-boot:run` a la vez sobre el mismo `target/` y un segundo `ng serve` con proxy propio): se reconsidera si aparece un segundo spec que cambie de curso. Descartadas las `dependencies` de proyectos: con `centro-minimo` rojo, Playwright se saltaría el spec dependiente.
  FASE ÚNICA (`fae5470`). `playwright.config.ts`: la serie, el `command` nuevo, los comentarios del invariante y de la carpeta, y los caducados de `D-playwright-cita-seed-difunto` (fuera `SeedCatalogoRunner`; la cifra de tablas pasa de 21 a 22, contada sobre `schema.sql`). `centro-minimo.spec.ts:22` cita la ruta nueva. `e2e/curso.spec.ts`: duplica una base sin nombre (2030/2031 → 2031/2032), da de alta un nivel en el curso nuevo como control positivo, abre el archivado, comprueba «Solo lectura» y ve el mensaje de la guarda en el formulario, que sigue abierto; `retries: 0`. El alta de nivel calca los localizadores de `centro-minimo`.
  CORRIDAS. `--list`: centro-minimo → curso → humo. Suite completa en un worker: curso y humo en verde, y centro-minimo en rojo sólo en la línea 267 (preexistente). `curso.spec.ts` dos veces seguidas en solitario: verde las dos. Después, `app/target/e2e/` contiene `educhronos-e2e.db` y `curso-2031-2032.db`, en `app/` no aparece ningún `curso-*.db` y `app/educhronos.db` conserva su md5.
  M3 POR MUTACIÓN DEL PRODUCTO, los tres cazados: m1, la guarda deja pasar al archivado (`GuardaSoloLectura.java:162`) → rojo en el rechazo; m2, duplicar sin archivar el origen (`DuplicadorCurso.java:172`) → rojo en la marca; m3, la marca no se pinta nunca (`curso-barra.ts:76`) → rojo en la marca.
  CRITERIO. `O-curso` **8 de 9** en Linux: se suma la 8. Queda la 9 (Windows). `C-e2e-curso` ✔ HECHO. Quinta sesión del objetivo, de las ocho de la métrica de §7.
  DEUDA. Se SALDAN `D-e2e-aislamiento` (bloqueaba la condición 8) y `D-playwright-cita-seed-difunto`. Nota en `D-e2e-retry-bd`, que sigue viva (sede Fase 12). No nace ninguna. R-deuda: con `O-curso` activo no queda ninguna deuda bloqueante. Se borra a mano `app/educhronos-e2e.db`, del esquema anterior, que ya no usaba nadie.
  DECISIÓN DE MAPA (del usuario tras el cierre de S161; se registra aquí). Los datos de disponibilidad del profesorado están en papel y no se digitalizarán fuera de la herramienta: el centro los introducirá con la propia `O-disponibilidad`, y hasta entonces se genera con los datos que hay. Se reescribe la «Dependencia externa» de su ficha y el orden no cambia. Pendiente externo nuevo: pedir al centro el formulario en papel, en blanco o uno relleno de ejemplo.
  DEFECTOS DE INSTRUMENTO. (1) Del M2 (Claude Code): las citas de línea de `GuardaSoloLectura.java` salieron de un `cat -n` que encadenaba varios ficheros y no son fiables; la rama de rechazo está en la 162, no en la 235. (2) Del asistente: copió esa línea al guion sin comprobarla. El primer m1 no mutó nada —el `sed` no casó y no dio error— y salió verde sin medir; lo paró el `git diff --quiet` que exigía el propio guion. Un mutante sólo cuenta si su diff no está vacío. (3) Del asistente: el borrador del spec traía localizadores propios para el alta de nivel en vez de los de `centro-minimo`, que ya resolvían la carrera de `writeValue`; lo corrigió la instrucción de reutilizar. (4) Del asistente: el ancla de 3.7 del guion de cierre llevaba «Con» en mayúscula cuando el texto va a mitad de frase; la guarda de aparición única la paró sin escribir nada, y Claude Code corrigió el ancla.
  SUITES: **solver 92, app 534, vitest 513 en 55 ficheros**, sin cambio. **e2e: 3 tests en 3 ficheros**, 2 en verde y `centro-minimo` en rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos en el cierre: INTACTOS. Esta sesión no los usó.
  LIMPIEZA (M1-bis): archivada S160 a `bitacora-sesiones.md`; degradada S161 a «Última sesión registrada (previa):»; S162 queda como única cabecera H3 viva. Condensación: no hay frente cerrado acumulado.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 22 y 10 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo esta misma sesión al insertar en gestión la nota de la fila de H4, el bullet de estado, la nota de `C-e2e-curso` hecho, la de la deuda saldada, los dos cierres de §4, la nota de `D-e2e-retry-bd` y la «Dependencia externa» reescrita de `O-disponibilidad`, que pasa de dos a siete líneas, y en el plan la cabecera de S162, el archivado de S160, la degradación de S161 y las dos fichas que pasan a CERRADA; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 39** sobre 293 tokens distintos en el corpus vivo, y la lista es IDÉNTICA a la de HEAD (39 sobre 292), medida corriendo el verificador sobre los documentos de HEAD extraídos a `/tmp`: ningún token entra ni sale. (a) El token nuevo, `C-e2e-curso`, **no lo marca el censo**: nace con 7 apariciones vivas —la fila de H4, los Cambios de la ficha de `O-curso`, los dos cierres de §4, las dos notas de cierre de las fichas del plan y la entrada del índice—, así que `D-deuda-sin-sede-en-el-plan` no engorda. (b) Una de las 7 NO es una cita: es la entrada del ÍNDICE GENERADO que reproduce la cabecera de S162 (`L753`), **cuarta reproducción medida de `D-censo-r4-rescate-por-indice`** tras las de S159, S160 y S161; sin ella el token tendría 6 y seguiría fuera de la lista, así que el rescate no cambia ninguna decisión, sólo el número. (c) La línea de R4 no rescata nada, porque desde S157 la entrada de sesión sale del corpus: las cifras que constan son las medidas antes de escribirla y se vuelven a comprobar después. (d) Extinción respecto de HEAD (`D-censo-r4-ciego-a-la-extincion`, ya instrumentada): **NINGÚN token cae a 0**, ni entre los citados en el bloque de S160 que esta sesión archiva. Las dos deudas que se cierran siguen vivas como token —`D-e2e-aislamiento` con 8 apariciones y `D-playwright-cita-seed-difunto` con 3—, porque su ficha pasa a «Deuda consciente CERRADA» y su fila de §4 se queda tachada. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S160** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador tras actualizar los dos de la bitácora, calcando el cierre de S161. (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en S161 porque hoy no nace ni se cierra ninguna de esa sede —las dos que se saldan tienen sede `O-curso`—; el filtro mecánico por la columna de sede de §4 no reproduce el criterio de S142 (da 20 filas, 5 tachadas) pero da lo mismo en HEAD y ahora, que es lo que aquí se mide; **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S161 degradada y S162 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda aguanta con `O-curso` activo: no queda ninguna deuda bloqueante.»
