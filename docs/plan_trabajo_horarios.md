...
- L730 — ### Sesión 147 — O-exportación (H3), C-alcance-exportacion: **H3 ABRE, Y SU PRIMER OBJETIVO NACE CON EL CRITERIO ESCRITO SOBRE MEDICIÓN Y NO ANTES.** Segundo M0 del proyecto que elige entre frentes, tras S142. Tres de los cuatro criterios de Fase 9 no eran medibles tal como estaban escritos; el umbral de legibilidad no se inventa, se toma de los PDF del centro (7,99 pt, cuerpo único); y la medición destapa que las horas de la jornada se guardan desplazadas una hora. Ni un fichero de código tocado.
...
- L3059 — ### Deuda consciente CERRADA (histórico)
- L3158 — ### Notas técnicas validadas en Fase 0
- L3168 — ### Notas técnicas validadas en Fase 6
- L3206 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3215 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3242 — ### Registro detallado de sesiones S10–S31
- L3251 — ## Señales globales de alerta
...
### Sesión 147 — O-exportación (H3), C-alcance-exportacion: **H3 ABRE, Y SU PRIMER OBJETIVO NACE CON EL CRITERIO ESCRITO SOBRE MEDICIÓN Y NO ANTES.** Segundo M0 del proyecto que elige entre frentes, tras S142. Tres de los cuatro criterios de Fase 9 no eran medibles tal como estaban escritos; el umbral de legibilidad no se inventa, se toma de los PDF del centro (7,99 pt, cuerpo único); y la medición destapa que las horas de la jornada se guardan desplazadas una hora. Ni un fichero de código tocado.
  TIPO Y RITUAL: MEDICIÓN — M0 + M2 de solo lectura ejecutado por Claude Code + M1. Sin M3 ni M4: no se escribió código. Quinto caso del tipo tras S136, S138, S139 y S142; sigue sin fila en la tabla de Tipos de `metodo.md`.
  M0. **Segundo M0 que elige entre FRENTES y no entre Cambios**, tras S142, cuyo razonamiento se leyó antes de repetirlo. H2 cerró en S141 y H1 en S146: no había objetivo abierto. **H3 elegido** por el orden de §2 y §5, sin hecho nuevo que lo invierta; adelantar H4 obligaría a validar el bundle dos veces, porque el paso 5 del guion también ha de pasar en Windows. **Higiene/Método descartada por R-apertura**, como en S142: recuento por la columna de sede de §4 con el criterio de S142, **16** —13 más 3—, **QUINTA reproducción consecutiva**. H3 no tenía objetivo en §3, así que este M0 lo ESCRIBE: Objetivo **`O-exportación`**, Hito **H3**, Cambio **`C-alcance-exportacion`**, creado aquí con el precedente de `C-alcance-ajuste` (S142). R-invalidación: **ninguna** —el horario se elige por id, como en `/proyeccion` y `/diagnostico`, así que no depende del `GET /api/horarios` que no existe (`D-horario-id-a-fuego`); lo único que H4 comparte es la plataforma, y el criterio lo recoge—. **Las 13 fichas con sede «O-ajuste-cierre (cerrado S146)» se revisaron contra el criterio nuevo: ninguna lo BLOQUEA**, y no se tocan. Dos condicionan el diseño y apuntan a lo mismo: `D-post-horario-sin-sesiones` (el arreglo de reconsultar el repositorio ya se ha copiado a mano cuatro veces) y `D-proyeccion-instancia-espejo` (ya hay dos mapeos de `Sesion`); la exportación compone `proyectar()` y no escribe un tercero.
  **LA LECTURA, ANTES DE MEDIR: la descomposición de H3 no era «de bajo riesgo» en el criterio**, como afirmaba §3. De los cuatro criterios de Fase 9, el 1 («buena legibilidad») y el 3 («toda la información necesaria») no tienen umbral, el 4 («funciona en Windows») sólo se verifica con el bundle de H4, y la fase **omitía el PDF por aula** que §1 exige. El 4 pasa a H4 por decisión del usuario, con el precedente del criterio de Fase 6 que difirió «la base funciona en Windows» a Fase 11. Y la solución de pantalla no viaja al papel: la rejilla condensa las celdas densas con marcas `+N`, que en papel no se pueden desplegar.
  M2, SOLO LECTURA, sobre copias en `/tmp` y sin arrancar la aplicación. Guardas: 1 horario, 819 sesiones, 28 grupos y 44 aulas. (a) **Jornada**: `tramo_semanal` guarda hora de inicio y fin en los 35 tramos, y el recreo como `es_lectivo = 0` —5 días × (6 lectivos + recreo)—; ninguna actividad dura más de un tramo (219 de 219 a 1). (b) **Nombres**: profesor 59/59 y asignatura 100/100 con `nombre_completo`; aula y grupo sólo tienen código, y el de aula lleva la descripción dentro (hasta 22 caracteres, «Taller 1 Aula Plástica»). (c) **Densidad**, reutilizando la definición de S122 (`diseno-navegacion.md` §A5) y repitiéndola sobre el horario de S137: grupo máximo 6 —22 celdas, entre ellas las de 1ºA martes T2 y jueves T5—, 840 de 840 celdas ocupadas y **1285 entradas cuadradas por dos caminos**; profesor 835 celdas, todas de 1 (835 > 819 por las 16 plazas con dos profesores); aula 819 celdas de 1 y **35 de 44 aulas con clase**. (d) **Textos**: la entrada más larga es de vista profesor, «DTec 1B-A/1B-B/1B-C/1B-D Taller 1 Aula Plástica», 47 caracteres (DIB2). (e) **PDF del centro**: A4 vertical, 28 y 43 páginas, leyenda de profesores y asignaturas en cada página, una sola fuente (Courier New, NO incrustada) y **un único cuerpo de 7,99 pt en título, rejilla y leyenda de las 71 páginas**; la celda más poblada de 1º ESO A ocupa 12 líneas (6 entradas × 2). (f) **Reutilización**: cero librerías de PDF o CSV en los tres `pom.xml` y en `package.json`; `SesionVistaDTO` lleva código y nombre de asignatura, códigos de profesor, código de aula, grupos y el par (día, orden), pero **ni el nombre del profesor ni las horas**, que viven en el catálogo y en `/api/jornada`. Bancos intactos al abrir y al cerrar; `git status` vacío.
  **LA SORPRESA QUE SE REGISTRA COMO DEUDA: las horas se guardan desplazadas una hora.** La base guarda 07:00–13:30 con recreo 10:00–10:30; la plantilla de `JornadaService` y el PDF del centro dicen 8:00–14:30 con recreo 11:00–11:30. Desfase exacto de 1 h en los 35 tramos. La hipótesis de zona horaria es muy probable y **no está verificada**. Encaja con la nota técnica de Fase 6: «`LocalTime` vuelve intacto» es cierto, pero S46 lo midió leyendo con la misma JVM que escribió, y esa prueba es ciega a un valor almacenado que depende de la zona de quien escribe. Consecuencias: el oráculo SQL NO vale para las horas, que se comprueban contra el PDF del centro; si `GET /api/jornada` devolviera 07:00, la condición 1 quedaría bloqueada; y una JVM en otra zona —los runners de CI de Fase 12 corren en UTC por defecto— leería otras horas. Nace `D-hora-tramo-dependiente-de-zona`.
  **UNA INFERENCIA QUE NO SE ASEVERA.** La distribución de densidad del horario generado reproduce exactamente la del PDF del centro, 648/83/37/22/28/22 para n = 1…6. Claude Code la anotó como coincidencia sin interpretarla; el arquitecto la lee como esperable —el catálogo se derivó de ese PDF, los 28 grupos están a holgura cero y cada desdoble es una sola actividad multiplaza, así que previsiblemente cada celda de grupo aloja una sola instancia y la densidad es propiedad del catálogo y no de la colocación—. **No medido**: lo confirmaría contar cero celdas de grupo con más de una instancia. Si se confirma, el peor caso del PDF no cambia al regenerar y `D-generacion-no-reproducible` no afecta a la condición 3.
  CRITERIO. **`O-exportación` ABRE con su criterio escrito sobre medición** (ficha en §3): (1) un PDF por vista con una página A4 por recurso —28 grupos, 59 profesores y las 44 aulas del catálogo, vacías incluidas, como imprime el centro—, horas y recreo comprobados contra el PDF del centro y leyenda de profesores y asignaturas; (2) completo y exclusivo contra un oráculo SQL independiente —1285 entradas en 840 celdas de grupo, 835 de profesor y 819 de aula—; (3) nada por debajo de 7,99 pt y revisión humana de 1ºA y DIB2; (4) el CSV basta para reconstruir las tres vistas; (5) fuentes incrustadas y codificación declarada, con la prueba en Windows real en H4. Confirmado por el usuario. **0 de 5 condiciones cumplidas**; `C-alcance-exportacion` HECHO. Advertencia para el Cambio del PDF: el precedente del centro no se transfiere solo —imprime códigos de aula cortos en Courier, unos 15 caracteres por columna, y los nuestros llegan a 22—, así que se maqueta 1ºA ANTES que nada. Criterios de Fase 9 SIN marcar, con nota de sustitución. **Objetivo activo del proyecto: `O-exportación`.**
  DEUDA. Nace **`D-hora-tramo-dependiente-de-zona`** en «Deuda técnica real», sede O-exportación, «¿Bloquea?» sin medir: se mide en el M2 del primer Cambio que imprima horas. Es técnica real y no mejora futura porque guardar una hora de reloj como un instante desplazado es una representación incorrecta aunque hoy nadie la vea. La nota técnica de Fase 6 sobre `LocalTime` gana su matiz (R5). Nace con DOS sedes —§4 y esta entrada—, precedente de S142 para no engordar `D-deuda-sin-sede-en-el-plan`. **Deuda bloqueante del proyecto: 0.**
  OBSERVACIONES QUE NO SON DEUDA, sin token por R-deuda y por falta de evidencia de fallo. `siguiente_inmediato_id` está a NULL en los 35 tramos, y las firmas públicas del solver (`referencia-codigo-solver.md`) no lo mencionan, así que el javadoc que lo atribuye al solver puede estar desfasado. Hay tres copias idénticas de cada PDF del centro (`docs/horario-referencia/pdf/`, `docs_extra/Ejemplos_SJ/` y `docs_extra/Ejemplos_SJ/HorariosProfesores/`, que no contiene horarios de profesores), familia de `D-plan-duplicado`. Siete asignaturas tienen el nombre igual al código (ALCT, FOPP, IPE, Latín, PEPA, PTEV, TICO): la leyenda imprimirá lo que diga el catálogo, y no se ha comprobado si la del centro las nombra. Y el extractor de los PDF del centro no está en el árbol —`RESUMEN-EXTRACCION.md` no afirma haberlo guardado—, así que el oráculo de la condición 2 será código nuevo.
  SUITES: 92 (solver) + 339 (app) de Maven y 478 de vitest en 52 ficheros, **HEREDADAS de S146 y NO recorridas**: la sesión no tocó una línea de código. `solver/src/main` no tocado: `referencia-codigo-solver.md` no se regenera.
  BANCOS. Referencia `educhronos-s137-centro-completo.db` en `64d671fe…`, verificada al abrir y al cerrar: **INTACTA**. Horario de referencia `educhronos-s137.db`: su **md5 completo, `dfa4c0774a842d8eb6b7a941e23df2a9`**, se mide por primera vez —la documentación sólo citaba el prefijo— y queda escrito en las decisiones permanentes. Ninguna copia nueva en `app/`: las de trabajo vivieron en `/tmp/s147/`. Ninguna generación lanzada.
  LIMPIEZA (M1-bis): archivada S145 a `bitacora-sesiones.md` (promovida a `### Sesión 145`, al final, cuerpo íntegro); degradada S146 a «Última sesión registrada (previa):»; S147 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S144 a S145. M1.5: no se condensa nada.
  R4 / COSTURA. «`verificar-cierre.py`: 0 comprobaciones duras con fallo tras el cierre; índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, con 0 y 0 entradas descuadradas. **Censo de tokens sospechosos: 33**, frente a **34** en S146 —la línea base, medida antes de tocar nada, fue 34, la cifra que el guion daba por buena—. (a) El recuento intermedio empató en 34 escondiendo un relevo: SALE `D-proyeccion-instancia-espejo`, que pasa de 1 a 4 apariciones vivas porque la ficha de `O-exportación` y esta entrada lo citan por primera vez, y ENTRA `D-censo-r4-ciego-a-la-extincion`, que cae de 3 a 1 al archivar S145 —y al que esta misma línea rescata al nombrarlo, de ahí 33 y no 34: la frase de R4 se escribe DENTRO del corpus que el censo mide, cosa que el guion no preveía—. (b) Los tres tokens nuevos NO son sospechosos: `C-alcance-exportacion` y `D-hora-tramo-dependiente-de-zona` nacen con 5 apariciones vivas cada uno; `O-exportación` tiene 17, pero su ausencia de la lista no prueba nada, porque el regex del censo sólo captura `D-` y `C-` y es CIEGO a los objetivos. (c) Extinción comprobada a mano sobre los 15 tokens citados en el bloque de S145 archivado: NINGUNO cae a 0 en el corpus vivo; los que más pierden son `D-guion-busca-token-esperado` (6 → 4), `C-gesto-rejilla` (5 → 3) y el propio `D-censo-r4-ciego-a-la-extincion` (3 → 1).»

Última sesión registrada (previa): Sesión 146 — O-ajuste-cierre (H1), C-prevalidacion-s8: **LA PREVALIDACIÓN COMPRUEBA S8 Y DEJA DE DECLARAR «CATÁLOGO SANO». O-ajuste-cierre CUMPLE SU CRITERIO Y CIERRA, y con él CIERRA H1: segundo hito terminado del proyecto.** S8 entra como cuarta regla de `PrevalidacionService` REUTILIZANDO la comprobación del solver y con severidad AVISO, primer productor de AVISO de la aplicación. Demostrada sobre el centro real por mutación de tutorías sobre copia y por la vía legítima, con la predicción escrita antes de medir y un oráculo SQL independiente: 12 de 12 predicciones. solver 91 -> 92, app 334 -> 339, vitest 477 -> 478.
