...
- L3206 — ### Deuda consciente CERRADA (histórico)
- L3305 — ### Notas técnicas validadas en Fase 0
- L3318 — ### Notas técnicas validadas en Fase 6
- L3363 — ### Notas técnicas validadas en Fase 9
- L3372 — ### Notas técnicas validadas en Fase 11
- L3473 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3482 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3509 — ### Registro detallado de sesiones S10–S31
- L3518 — ## Señales globales de alerta
...
### Sesión 154 — O-instalación (H4), C-arranque-cierre: **EDUCHRONOS SE COMPORTA COMO UN PROGRAMA DE ESCRITORIO: ABRE EL NAVEGADOR, NO SE DUPLICA, SE CIERRA DESDE LA BANDEJA Y SÓLO ESCUCHA EN LOCAL.** `O-instalación` pasa de 3 a **7 de 9**: las condiciones 4, 5, 6 y 8 quedan cumplidas y verificadas en Linux y en Windows, y las 1 y 2 RE-VERIFICADAS sobre el bundle nuevo. Decidido el mecanismo de arranque y cierre, que era la última decisión de diseño abierta del objetivo salvo las de la condición 3.
...
Última sesión registrada (previa): Sesión 153 — O-instalación (H4), C-datos-usuario: **LA BASE DEJA DE NACER JUNTO AL PROGRAMA Y SE MUDA A LA CARPETA DE DATOS DEL USUARIO.** `O-instalación` pasa de 2 a **3 de 9**: la condición 7 queda cumplida y verificada en las dos plataformas, con el dato sobreviviendo a lanzar desde dos directorios y a sustituir la carpeta del programa. Las condiciones 1 y 2 se RE-VERIFICAN sobre el bundle nuevo, que crece 4.083 B. Lo más valioso de la sesión no es el producto sino el M3: el mutante PORTANTE deja la suite VERDE.
  TIPO Y RITUAL: DESARROLLO — M0 + M2 en tres etapas (inventario, arnés y aplicación) + M3 (campaña de mutación) + M4 en las dos plataformas + M1. El M2-B necesitó TRES pasadas: las dos primeras no midieron nada por defectos del propio arnés, los dos cazados por Claude Code y no por el asistente.
  M0. Segundo Cambio de `O-instalación` tras el de construcción. Va antes que arranque, cierre o escucha porque la condición 7 es «la decisión de la que depende la Fase 10» según la propia ficha, y porque las otras tres condiciones de esa tanda no dependen de dónde vivan los datos. Alcance CERRADO por el usuario antes de escribir: no se migra una base preexistente junto al ejecutable, no se toca el `application.properties` de test, y arranque, cierre, escucha y rutas de la SPA quedan fuera.
  M2-A, INVENTARIO. La ruta se resolvía en UN solo sitio: `spring.datasource.url=jdbc:sqlite:educhronos.db` en el `application.properties` de main, relativa al working dir por decisión escrita en 2018 palabras de comentario. Cero código Java que la calculara: `SqliteForeignKeysConfig` solo envuelve el `DataSource`. El módulo `solver` no tiene configuración. Consumidores fuera de `src`: `playwright.config.ts` (3 menciones), `e2e/centro-minimo.spec.ts`, `horario/titulo.spec.ts` y dos guiones de `scripts/` que abren bancos por ruta explícita.
  M2-B, ARNÉS DE OBSERVACIÓN. Tres pasadas. La primera y la segunda dieron CERO líneas de traza y el cero parecía un hallazgo sobre el producto: era el instrumento (ver CORRECCIONES). La tercera midió lo que hacía falta: con `getOrder()` en `LOWEST_PRECEDENCE` el post-procesador ve el `application.properties` ya cargado, y la URL del argumento de línea de órdenes llega por `commandLineArgs`, POR ENCIMA del fichero. Y midió la premisa que la ficha daba por sabida: con el directorio padre ausente, la aplicación MUERE a los 1,85 s con `SQLITE_CANTOPEN` dentro de `DataSourceScriptDatabaseInitializer.runScripts`, sin llegar a levantar Tomcat. Por eso la carpeta hay que crearla ANTES de que exista el `DataSource`, y un EPP es el único punto que corre suficientemente pronto.
  M3, CAMPAÑA DE MUTACIÓN. 14 mutantes, una mutación por corrida, fuente restaurada por `diff` y `touch` antes de recompilar (sin eso Maven sirve el `.class` mutado y el mutante mide el fuente restaurado). 10 cazados de entrada. Los cuatro supervivientes se reportaron SIN taparlos, y el usuario decidió: dos se cubrieron después —`isBlank()` → `isEmpty()` en la regla de no-op, y la guarda `null` de `esWindows`—, y dos quedan fuera de alcance, uno de ellos con deuda propia. Aparte, el MUTANTE 11, portante: ver DECISIONES.
  M4, LAS DOS PLATAFORMAS. Linux: app-image construido con el `jpackage` del sistema —que falló primero con «Module java.rmi not found» porque el JDK de openSUSE no trae `jmods`, y se resolvió con `--runtime-image`, así que el runtime es el completo y NO el recortado de 14 módulos de la condición 2—. Las dos mitades literales de la condición 7 verificadas: escribir un profesor desde un directorio de trabajo y leerlo desde otro, y volver a leerlo tras duplicar la carpeta del programa y arrancar la copia. Cero `.db` dentro de las carpetas del programa y cero en los tres directorios de trabajo. Windows (cuenta de dominio `llozano`): la base en `C:\Users\llozano\AppData\Local\Educhronos\educhronos.db`, las dos mitades igual de verdes. **No cuenta para la condición 3: la cuenta es de dominio.**
  PRODUCTO. `RutaBaseDatosEnvironmentPostProcessor` en `es.yaroki.educhronos.app.config`, registrado en `META-INF/spring.factories`, con regla ÚNICA de no-op: si `spring.datasource.url` ya viene resuelta, no hace nada. De esa regla —y solo de ella— viven sin tocarlos los 33 slices `@DataJpaTest`, que traen su propia URL, y el e2e, que la pasa por argumento. Retirada la clave del `application.properties` de main, con el comentario reescrito sobre lo medido. Windows: `%LOCALAPPDATA%\Educhronos`, con respaldo en `%USERPROFILE%\AppData\Local`; resto: `$XDG_DATA_HOME/educhronos`, con respaldo en `~/.local/share/educhronos` —que es el camino corriente: la variable estaba sin definir en este mismo escritorio—. Si la carpeta no se puede crear, IllegalStateException NOMBRANDO la ruta: nada de volver al directorio de trabajo, que es el defecto que la condición elimina. 14 casos de prueba con secuencia propia (D-S101-num).
  CRITERIO. **Condición 7 CUMPLIDA (S153)**, con las dos mitades verificadas en Linux y en Windows. **Condiciones 1 y 2 RE-VERIFICADAS** sobre el bundle nuevo: la carpeta de Windows mide 231.695.946 B, 4.083 B más que los 231.691.863 de S152, y ese delta es exactamente lo que crece el jar con el post-procesador (156.367.213 frente a 156.363.130 B), así que el margen sobre los 250 MB no se mueve. `O-instalación`: **3 de 9**.
  DECISIONES DE MÉTODO. (1) **El MUTANTE 11 es el hallazgo de la sesión.** Con `getOrder()` en `HIGHEST_PRECEDENCE` el post-procesador corre ANTES que `ConfigDataEnvironmentPostProcessor`, lee un entorno sin el `application.properties` de test y les pisa la URL a los 33 slices. La suite sigue **VERDE** (92/430, cero fallos) y la carpeta `~/.local/share/educhronos` APARECE: los slices corren contra una base que no es la suya y `app/educhronos-test.db` conserva su fecha del 31 de julio sin que nada se ponga rojo. La predicción se escribió ANTES de correrlo y acertó entera. El portante estaba protegido por una comprobación de efecto lateral y por NINGÚN test. Se CUBRIÓ después con un caso que usa API pública: `ConfigDataEnvironmentPostProcessor.ORDER` comparado como VALOR y no copiado como literal, y `AnnotationAwareOrderComparator`, verificado en el bytecode que es el comparador que Boot usa de verdad. El mutante pasa de incazable por la suite a cazado por un test. (2) **Una suite verde no demostraba que la ruta fuera escribible:** los slices abren un fichero preexistente y REVIERTEN, así que ningún test ejercitaba la CREACIÓN de la base; se comprobó midiendo que la BD de test no cambiaba de fecha en una corrida entera. (3) Los supervivientes de una campaña se reportan, no se tapan: la decisión de cubrir, declarar fuera de alcance o abrir deuda es del usuario.
  DEUDA. Nace **`D-guion-escribe-donde-no-se-dijo`** (de método, en §4 y en «Deuda consciente VIVA») con TRES instancias en dos sesiones: el `-Base` del `.ps1` en S152, el `--salida` de `empaquetar-linux.sh` en S153 —entrega incompleta que viajó 190 MB— y el efecto colateral medido al demostrar el arreglo, que dejó 332 MB dentro del repo con `exit 0`. NO se cuelga de `D-guion-exit-enmascarado`, que va sobre la señal falsa: aquí el guion termina bien y escribe en otra ruta, y dos de las tres instancias no enmascaran ningún fallo. **`D-guarda-escritura-sin-caso`** nace en el M3 y esta entrada es su tercera sede: la guarda `Files.isWritable` de `crearCarpeta` no la ejercita ningún caso —el mutante que la suprime SOBREVIVE—, no se cubre porque un `chmod 0555` no discrimina si la suite corre como root y porque en Windows `isWritable` mira el atributo de solo lectura y no los permisos efectivos. **`D-jar-no-reproducible` se ejercita otra vez**: el sha256 del jar de hoy identifica esta construcción, no el commit. **Dos instancias más de `D-guion-exit-enmascarado`**, las dos del arnés (ver CORRECCIONES a). **Una instancia de `D-guion-pkill-casa-su-propio-envoltorio`**: al parar un arranque, una JVM hija sobrevivió a la muerte del proceso Maven y dejó el 8080 ocupado, de modo que la verificación siguiente abortó; es, de paso, el síntoma que la condición 5 existe para eliminar. **`D-declarado-sin-artefacto`**: dos lecciones de esta sesión se guardaron primero «en memoria» del asistente —el EPP va en `spring.factories`, `spring-boot:run` ignora el cwd del shell—, y la memoria NO es sede recuperable por el método; por eso viven en «Notas técnicas validadas en Fase 11».
  CORRECCIONES. (a) Del asistente, DOS en el arnés del M2-B y las dos cazadas por Claude Code: implementó `org.springframework.boot.env.EnvironmentPostProcessor`, `@Deprecated` desde 4.0.0, que tiene la MISMA firma que la viva y por eso compila sin un solo aviso; y lo registró en un fichero `.imports`, que en Boot 4.1 no descubre EPPs —van en `META-INF/spring.factories`—. Dos ceros que parecían hallazgos sobre el producto y eran defectos del instrumento. Causa raíz ya nombrada en S129: escrito de memoria teniendo el jar a mano. (b) Del asistente: el prompt del arnés omitió `Ordered.LOWEST_PRECEDENCE`, que es PORTANTE; lo añadió Claude Code por su cuenta y lo documentó. (c) Del asistente: la hipótesis de por qué `SHA256SUMS` salió con una sola entrada —«la huella del JDK cuelga de la rama de descarga y la caché se la salta»— era FALSA, y la refutó Claude Code reproduciéndola en un árbol de juguete; la causa real es que `--salida` admite ruta relativa y el guion hace `cd` antes de resolver el destino del volcado. En S152 no se pasó `--salida` y valió el defecto, que es absoluto. (d) Del asistente: el paso 2 de la demostración del arreglo NO podía abortar, porque el redirect fuera del subshell ya atacaba la causa; Claude Code midió los dos pasos que sí discriminaban. (e) Del compilador: `List.getLast()` es de Java 21 y el proyecto declara `maven.compiler.release=17`. (f) Del asistente: la ruta de paquete del prompt (`es.yaroki.educhronos.config`) se escribió de memoria y dejaba el único tipo del proyecto fuera de `...educhronos.app`; movido a `es.yaroki.educhronos.app.config`, donde ya vive `SqliteForeignKeysConfig`.
  SUITES: **solver 92, app 430, vitest 482 en 52 ficheros**, todo en verde. La apertura fue 92/416/482: los 14 nuevos son del post-procesador. Comprobado aparte, y no por la suite, que `~/.local/share/educhronos` NO existe al terminar `mvn test` y que `app/educhronos-test.db` conserva su fecha del 31 de julio: si la regla de no-op se rompiera, la suite seguiría verde.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos al empezar y al terminar cada tanda. Ninguno se abrió: el M4 corrió sobre una COPIA.
  LIMPIEZA (M1-bis): archivada S151 a `bitacora-sesiones.md`; degradada S152 a «Última sesión registrada (previa):»; S153 queda como única cabecera H3 viva.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (7 defectos inyectados, los 7 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 13 y 19 entradas descuadradas a 0 y 0** sobre 31 y 68 encabezados —en HEAD los dos estaban a 0 con 31 y 68: el descuadre lo produjo esta misma sesión al escribir la condición 7, la fila de H4, las dos altas de deuda, las notas técnicas y la cabecera de S153, y se corrigió regenerando—. Hubo además UN fallo duro antes del cierre, y lo cazó el verificador: la crónica de archivado del plan seguía diciendo S150 mientras los dos censos de la bitácora ya decían S151; corregido añadiendo «la de S151 en la Sesión 153» y actualizando la frase de qué cabeceras conserva el plan. **Censo de tokens sospechosos: 33** sobre 261 tokens distintos, el mismo número que declararon S151 y S152, y la comparación es honesta porque los tres se miden DESPUÉS de escribir su línea de R4. (a) La corrida previa a escribir esta línea dio **36**, y el censo de HEAD, medido con el mismo instrumento sobre las versiones commiteadas que `git show HEAD:` saca a una carpeta aparte, da **33**: tres entradas netas, y son `C-alcance-ajuste`, `C-alcance-exportacion` y `C-alcance-instalacion`, los tres Cambios de alcance cuya única cita viva estaba DENTRO del bloque de S151 que esta sesión archiva; esta misma frase los rescata a los tres al nombrarlos, de ahí 33 y no 36, **octava reproducción consecutiva de `D-censo-r4-cuenta-menciones`** —la línea de R4 se escribe DENTRO del corpus que el censo mide—. El número que vale es el medido después, y es el que consta. (b) Los tokens nuevos de esta sesión son `C-datos-usuario`, `D-guion-escribe-donde-no-se-dijo` y `D-guarda-escritura-sin-caso`: **no los marca el censo**, porque nacen con 8, 3 y 3 apariciones vivas medidas ANTES de esta línea, en las sedes de siempre —§3 y §2 para el Cambio; §4, «Deuda consciente VIVA» y esta entrada para las dos deudas—, así que `D-deuda-sin-sede-en-el-plan` no engorda. `O-instalación` sigue fuera del corpus por construcción (`D-censo-r4-ciego-a-los-objetivos`), y por eso cumplir una tercera condición no mueve el censo. (c) Extinción comprobada a mano sobre los 17 tokens citados en el bloque de S151 archivado (`D-censo-r4-ciego-a-la-extincion`): **NINGUNO cae a 0** en el corpus vivo; los tres que bajan a 1 son los ya dichos, y el resto aguanta entre 2 y 30. (d) HIGIENE/MÉTODO: **19 fichas** por la columna de sede de §4 con el criterio de S142, duodécima reproducción; el reparto es 13 en «Transversal, con la sesión de Higiene/Método», 2 en «Transversal, sesión de Higiene/Método» y 4 en «Transversal, con el script de R4 pendiente desde S101». Sube una respecto a las 18 de S152, y la que sube es `D-guion-escribe-donde-no-se-dijo`, que nace con sede transversal. **El disparador de la excepción a R-apertura NO está activo: 19 de 20, falta 1.** **R-deuda aguanta**: con las condiciones 1, 2 y 7 cumplidas, la única deuda viva que toca una condición VIVA de `O-instalación` sigue siendo `D-spa-sin-fallback-de-rutas`, que ES la condición 9 y se paga dentro del objetivo; `D-guarda-escritura-sin-caso` nace apuntando a este objetivo pero no bloquea ninguna condición, porque la 7 —ya cumplida— no pide que esa guarda esté cubierta.»
...
- **D-spa-sin-fallback-de-rutas** (S145, VIVA; desde S151 es la condición 9 de `O-instalación` y la paga ese objetivo) — el jar no reenvía las rutas profundas de la SPA
