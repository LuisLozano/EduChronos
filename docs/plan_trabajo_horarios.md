...
- L754 — ### Sesión 168 — O-aceptación (H4), C-alcance-aceptacion: **H4 ABRE SU TERCER OBJETIVO, `O-aceptación`, CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 4.** La cadena entera de §1 tenía nombre pero no tenía hito ni ficha. La medición encuentra que los pasos 2, 3 y 4 no se han ejecutado nunca en el bundle de Windows, y que el oráculo de exportación da OK sobre un PDF que pinta una clase de dos horas como si fuera de una.
- L829 — ### Bloques de Fase 2
- L837 — ### Bloques de Fase 5
- L860 — ### Bloques de Fase 6
- L905 — ### Bloques de Fase 8
- L1117 — ### Fases completadas
- L1221 — ### Cierre del modelo — Sesión 8
- L1269 — ### Decisiones permanentes (no reabrir sin razón de peso)
- L1315 — ### Método de trabajo (procedimiento vigente)
- L1348 — ### Deuda consciente VIVA
- L3222 — ### Deuda consciente CERRADA (histórico)
- L3481 — ### Notas técnicas validadas en Fase 0
- L3494 — ### Notas técnicas validadas en Fase 6
- L3539 — ### Notas técnicas validadas en Fase 9
- L3548 — ### Notas técnicas validadas en Fase 11
- L3690 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3699 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3726 — ### Registro detallado de sesiones S10–S31
- L3735 — ## Señales globales de alerta
...
Última sesión registrada (previa): Sesión 167 — O-disponibilidad (H2), C-rejilla-disponibilidad: **C-REJILLA-DISPONIBILIDAD HECHO: EL CENTRO MARCA LA DISPONIBILIDAD DE CADA PROFESOR EN UNA REJILLA Y EL HORARIO LA CUMPLE. O-DISPONIBILIDAD TERMINADO, 7 DE 7; H2 CERRADO DE NUEVO. LOS MENSAJES DE ERROR DEL BACKEND LLEGAN A LA INTERFAZ.**
  TIPO Y RITUAL: CONFIGURACIÓN/UI con M3 en la lógica real — M0 + M-mockup + M2 + fases 3 a 6 + M3 + M4 + M1, dirigida desde un solo chat.
  FASE 1 — maqueta (M-mockup) en /tmp, con tokens, jornada y mensaje del 403 reales, y contraste medido.
  FASE 2 — M2 de solo lectura: contratos de jornada y restricciones, cuerpo HTTP real de cada rechazo, reutilización del frontend.
  FASE 3 — backend: se salda `D-F8.6-ii-a`.
  FASE 4 — modelo, servicio y lógica pura de la rejilla.
  FASE 5 — diálogo y botón en la lista de profesores.
  FASE 6 — M4 en navegador y condición 7 sobre un caso sintético.
  M0. Cambio `C-rejilla-disponibilidad`, Objetivo `O-disponibilidad`, Hito H2. Sin R-invalidación: la aceptación y la Fase 12 no rehacen la rejilla; si la aceptación arreglase `D-jornada-congelada-por-disponibilidad` cambiando la identidad de los tramos, tocaría la conversión, que vive en un solo módulo. R-deuda: ninguna fila de §4 bloqueaba al abrir; el M2 midió que `D-F8.6-ii-a` bloqueaba la condición 4.
  MEDICIÓN (fase 2, HTTP real sobre copia de s137). `GET /api/jornada` da horas «HH:mm» y el recreo con `esLectivo:false` y `ordenEnDia:null`. Las restricciones usan `dia` entero 1..5 (`ordinal()+1`) frente a «LUNES» en la jornada; `peso` no viaja; el GET devuelve `motivo` y el PUT, de reemplazo total, lo borra si no viene. Los 400 (tramo inexistente, tipo inválido, duplicado) y el 404 salían sin `message`: el usuario habría leído «Bad Request» y «Not Found». Causa ya medida en S118; confirmada arrancando con la clave nueva. El 403 de solo lectura no estaba afectado (cuerpo propio de S159).
  DECISIONES: (K) a (O), en la ficha de `O-disponibilidad`: símbolos, M-mockup con fases, saldar `D-F8.6-ii-a`, `motivo` y fila huérfana, horas y jornada sin guardar.
  CÓDIGO `6a29f65` — `spring.web.error.include-message=always`, con un comentario veraz; `MensajeDeErrorHttpTest` (servidor real, `java.net.http`, lee el cuerpo y carga a mano las claves de main); comentario corregido en `horario-view.ts`.
  CÓDIGO `fae2168` — `restriccion-horaria.model.ts`, `RestriccionHorariaService` (patrón `TutoriaService`) y `profesores/disponibilidad/rejilla-disponibilidad.ts`: `construirRejilla`, `desdeRestricciones`, `aplicarPincel` (celda, día y tramo, inmutable) y `aRestricciones` (ordenado; `motivo` sólo con el mismo tipo; fila huérfana reenviada). «LUNES» → 1 sólo en `diaDeJornada`.
  CÓDIGO `1d8a6d1` — `DisponibilidadDialogo` (patrón `TutoriaDialogo`; estados cargando, cargado, error y sin jornada) y botón «Disponibilidad» en `profesor-lista`.
  M3. 28 mutantes, 28 cazados: 3 en la fase 3 (clave vieja, sin clave y, en el test, sin cargar las claves de main), 14 en la fase 4 —(l) sobrevivió primero y obligó a reforzar un caso— y 11 en la fase 5.
  M4. Maqueta en navegador: variante SVG legible, estados distinguibles en escala de grises, pincel correcto. Fase 6 en navegador sobre base vacía (carpeta de datos en /tmp, centro mínimo de S166 por REST, jornada guardada antes): condiciones 1 a 3 y guardado con reapertura; «No existe profesor con id 2» en el diálogo, con el profesor borrado desde otra pestaña; «Ya existe un profesor con codigo P1» en el alta; el 403 con el nombre del curso archivado, dentro del diálogo, que sigue abierto. Condición 7 con predicción escrita antes de generar: bloque de P1 en lunes 1–2, sin violaciones, 1 BLANDA y OPTIMAL con objetivo 1.0.
  CRITERIO. **7 de 7: `O-disponibilidad` TERMINADO.** H2 CERRADO de nuevo.
  DEUDA. Se salda `D-F8.6-ii-a`. Nacen `D-props-main-invisibles-en-tests` (técnica real), `D-curso-sin-renombrar` y `D-bundle-sobre-techo-550` (mejoras futuras), todas sin objetivo asignado y sin bloquear. `D-proyeccion-sin-duracion` suma su primera instancia en la vista. `D-F8.5-E-b` queda con su sede cerrada.
  OBSERVACIONES, sin deuda. Con la clave viva, todos los formularios muestran el mensaje del backend, que va sin tildes y a veces es técnico; no cambia ningún criterio (R-terminado). Los comentarios del frontend que afirmaban que el mensaje viajaba pasan a ser ciertos. El M4 dejó los nombres de curso cruzados en /tmp, sin efecto.
  SUITES: **solver 116, app 550, vitest 558 en 58 ficheros** (app +2; vitest +42 y 3 ficheros más). e2e: 3 tests en 3 ficheros: curso y humo en verde; centro-minimo rojo preexistente (`.instancia`, esperado 3 y recibido 0, ahora en :267). Antes hubo que instalar Chromium para Playwright en la caché de usuario, fuera del repo. Bundle inicial 571,67 → 588,34 kB (`D-bundle-sobre-techo-550`).
  LIMPIEZA (M1-bis): archivada S165 a `bitacora-sesiones.md`; degradada S166 a «Última sesión registrada (previa):»; S167 queda como única cabecera H3 viva. Condensación: no hay frente cerrado acumulado que condensar.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 23 y 10 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD (`1d8a6d1`) los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo este cierre, en gestión al añadir las Decisiones (S167), el ESTADO en S167 y las tres filas nuevas de §4, y en el plan al insertar la entrada de S167, archivar S165 y añadir las fichas; la medición del «antes» se hizo sobre una copia en /tmp con el bloque de índice de HEAD, importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 40** sobre 307 tokens distintos en el corpus vivo, frente a **40** sobre 304 en HEAD, medido corriendo el verificador sobre el árbol limpio en `1d8a6d1` antes de editar: la lista es idéntica. (a) Los tres tokens nuevos de la sesión entran sin estar en la lista: `D-props-main-invisibles-en-tests` con 3 apariciones —su ficha del plan, su fila de §4 y el cierre de la ficha de `D-F8.6-ii-a`—, y `D-curso-sin-renombrar` y `D-bundle-sobre-techo-550` con 2 cada uno —su ficha del plan y su fila de §4—. (b) Cambian de cuenta sin tocar la lista: `C-rejilla-disponibilidad` de 4 a 7 —la fila de §4 y la ficha del plan de `D-F8.6-ii-a`, que la nombran al cerrarla, y la entrada del ÍNDICE GENERADO que reproduce la cabecera de S167—; `C-dura-completa` de 10 a 9 —esa misma entrada deja de reproducir la cabecera de S166, el mecanismo de `D-censo-r4-rescate-por-indice` en sentido inverso—; `O-disponibilidad` de 33 a 36 —la línea de estado de los hitos, la fila de §4 y la ficha del plan de `D-F8.6-ii-a`—; `D-F8.6-ii-a` de 32 a 33 —las Decisiones (S167)—; `D-bundle-presupuesto` de 6 a 8 y `O-diseño` de 82 a 84 —la fila y la ficha de `D-bundle-sobre-techo-550`—; `D-hora-tramo-dependiente-de-zona` de 4 a 5 y `D8` de 12 a 13 —la decisión (O)—. (c) `D-proyeccion-sin-duracion` sigue en la lista con 1 aparición, igual que en HEAD: la instancia de S167 se escribe en su fila de §4 sin volver a nombrar el token, y no se le crea ficha en el plan —decisión del usuario: el hueco es preexistente (S148), de la familia de `D-deuda-sin-sede-en-el-plan`, y R-deuda impide saldarlo aquí—. (d) La línea de R4 no rescata nada: la entrada de sesión sale del corpus (48 líneas retiradas, con la S166 degradada, las mismas que en HEAD). (e) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (f) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (g) Los tres censos coinciden en **S165** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S165 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 21 líneas; título idéntico). (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S166 degradada y S167 como única cabecera H3 viva, y el plan tiene exactamente una. (i) El diff de costura sólo toca las regiones de G1 a G10, P1 a P6, P8, B1, B2 y `metodo.md`, más las 66 líneas del índice regenerado; P7 y P9 se omitieron por decisión del usuario, y §7 no tiene cifras literales que actualizar (G11). R-deuda: se salda `D-F8.6-ii-a`; ninguna deuda viva bloquea, y no queda ningún objetivo abierto.»
...
- **D-proyeccion-sin-duracion** (S148, VIVA, TÉCNICA REAL, BLOQUEANTE desde S168, sede `O-aceptación`, condición 3) — LA
  ...
  y se salda dentro de él; el arreglo incluye que el oráculo compruebe todos los tramos que cubre cada sesión.
