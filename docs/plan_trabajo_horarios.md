...
- L3156 — ### Deuda consciente CERRADA (histórico)
- L3358 — ### Notas técnicas validadas en Fase 0
- L3371 — ### Notas técnicas validadas en Fase 6
- L3416 — ### Notas técnicas validadas en Fase 9
- L3425 — ### Notas técnicas validadas en Fase 11
- L3565 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3574 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3601 — ### Registro detallado de sesiones S10–S31
- L3610 — ## Señales globales de alerta
...
### Sesión 158 — O-curso (H4), C-alcance-curso: **H4 ABRE SU SEGUNDO OBJETIVO, `O-curso`, CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 9.** Duplicar curso deja de ser un texto de fase y pasa a nueve condiciones verificables; se descarta el selector de curso al iniciar, y la ejecución de la cadena entera de §1 sale a un objetivo propio.
  ...
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (12 defectos inyectados, los 12 detectados). Índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 9 y 19 entradas descuadradas a 0 y 0** sobre 32 y 68 encabezados; el descuadre lo produjo esta misma sesión al insertar dos filas en §4, dos entradas de deuda en el plan, archivar el bloque de S156 y escribir esta entrada. Insertar esta línea de R4 los descuadra otra vez —desplaza en uno todo lo que va detrás—, así que se regeneraron DESPUÉS de escribirla, y las cifras que constan son las de esa última corrida. **Censo: 283 tokens distintos y 39 con UNA sola aparición; 1 sólo en la entrada de sesión; 0 extinciones respecto de HEAD.** Movimientos respecto de los 281/40/1/0 medidos tras `2fbdab4`: (a) +2 distintos, que son los dos tokens que nacen hoy, `D-esquema-sin-version` y `D-playwright-cita-seed-difunto`, ambos con 2 apariciones —sus dos sedes— y por eso fuera de la lista de sospechosos. (b) 40 → 39 porque sale `C-alcance-curso`, y la causa está MEDIDA y no es que nadie lo cite: su segunda aparición vive en el ÍNDICE GENERADO del plan, que reproduce la cabecera de esta entrada. El censo retira la entrada de sesión del corpus (30 líneas: la viva y la previa degradada) pero NO su reflejo en el índice, de modo que la cabecera rescata a los tokens que nombra. Es el mismo modo de fallo que S157 pagó con `D-censo-r4-cuenta-menciones`, por una puerta que aquella sesión no miró; queda MEDIDO aquí y SIN FICHA, a decisión del arquitecto. (c) El «sólo en la entrada» sigue siendo 1 y sigue siendo `O-nombre`. En una corrida intermedia fueron 2: esta entrada nombraba el token de la deuda que el guion preveía para la comprobación 3, que NO nace, y escribir un identificador que nadie va a usar es exactamente lo que S157 retiró con `O-nombre`; se reescribió la frase sin token y volvió a 1. (d) Cero extinciones: nada de lo que esta sesión movió perdió su última cita. (e) Los 39 de una aparición son los 40 de `2fbdab4` menos `C-alcance-curso`, y siguen sin revisar por R-terminado: ninguna ficha del alcance pedía vaciar esa lista. (f) HIGIENE/MÉTODO: **11 fichas**, sin cambio respecto de S157, contadas por la columna de sede de §4 con el criterio de S142; un contador que casara «Higiene/Método» en cualquier columna daría 12, porque `D-tokens-inexistentes` la nombra en su disposición y su sede es otra. **R-deuda aguanta con `O-curso` activo:** de las deudas vivas las únicas que bloquean son las dos que el objetivo absorbe —`D-horario-id-a-fuego` (condición 4) y `D-e2e-aislamiento` (condición 8)—, y ninguna de las dos que nacen hoy bloquea nada.»
...
- **D-F8.6-ii-a** (S81, VIVA, AMPLIADA Y RECLASIFICADA EN S109 a TÉCNICA REAL TRANSVERSAL) — EL `reason` DE
  ...
  **INSTANCIA de S158:** Medido en el M0 de S158: `DELETE /api/actividades/20` responde 409 con cuerpo
  ...
  al cliente por esa vía. La causa no se investigó. Importa a `O-curso` porque su condición 3 exige que el
  mensaje de la guarda de solo lectura llegue al cliente, y esa guarda puede caer en el mismo mecanismo: su M2
  empieza por esta causa. No bloquea por sí misma.
