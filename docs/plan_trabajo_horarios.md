...
- L753 — ### Sesión 158 — O-curso (H4), C-alcance-curso: **H4 ABRE SU SEGUNDO OBJETIVO, `O-curso`, CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 9.** Duplicar curso deja de ser un texto de fase y pasa a nueve condiciones verificables; se descarta el selector de curso al iniciar, y la ejecución de la cadena entera de §1 sale a un objetivo propio.
- L818 — ### Bloques de Fase 2
...
### Sesión 158 — O-curso (H4), C-alcance-curso: **H4 ABRE SU SEGUNDO OBJETIVO, `O-curso`, CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 9.** Duplicar curso deja de ser un texto de fase y pasa a nueve condiciones verificables; se descarta el selector de curso al iniciar, y la ejecución de la cadena entera de §1 sale a un objetivo propio.
  TIPO Y RITUAL: APERTURA DE OBJETIVO — M0 + seis mediciones de solo lectura sobre copias de los bancos + alta documental con corrida en seco + M1. Sin código de la aplicación. Precedente: `C-alcance-instalacion` (S151).
  M0. Cambio `C-alcance-curso`, Objetivo `O-curso`, Hito H4. Toca por el orden decidido en S157 (Fase 10 antes que Fase 12) y porque el paso 6 es el único eslabón de §1 que falla; la condición 7 de `O-instalación` ya resolvió la dependencia de la carpeta de datos. Higiene no se reabre, aunque el disparador saltó en S157: serían dos sesiones de Higiene seguidas sin producto entre medias. R-invalidación: nada planificado rehace este trabajo.
  MEDICIÓN (jar de `b154bb9`, bancos copiados a `/tmp/educhronos-s158`, repo intacto). (1) `DELETE /api/actividades/20` → 409. La única FK hacia el horario es `sesion.horario_id`, con CASCADE; los pines cuelgan de (actividad, indice) y la guarda del 409 los cuenta; no hay DELETE ni listado de horarios. (2) Con `open_mode=1` no arranca (HikariCP llama a `setReadOnly` y el driver lo rechaza); con `chmod 444` arranca y lee, y da 500 genérico al escribir. 60 handlers: 26 GET, todos de lectura, y 34 no-GET. (3) El candado es de la carpeta de datos y vive hasta que muere el proceso; `main` no conserva el contexto; arranque de unos 6 s; D18 reserva altura, no ancho. (4) Dos specs e2e, sin mecanismo de siembra. (5) `journal_mode=delete`. (6) Cuatro cambios de `schema.sql`, sin migración, `user_version` 0.
  CRITERIO. `O-curso` escrito en §3 con nueve condiciones: duplicar con el año académico como nombre, curso nuevo editable, anterior en solo lectura, horario archivado accesible, cambio de curso desde la barra, actualización sin pérdida, desarrollo intacto, e2e del eslabón 6 y Windows. **0 de 9.** `C-alcance-curso` ✔ HECHO (`2fbdab4`). Sustituye a los cuatro criterios de la Fase 10, con nota en el plan.
  DECISIONES. Del usuario: (A) los pines no pasan al curso nuevo; (B) el curso archivado muestra su último horario generado; (C) el e2e no genera horario; (D) el año académico nombra el curso, no el horario, y la copia elimina los horarios igualmente, porque la guarda del 409 no mira etiquetas. De la medición: (E) el selector vive dentro de la aplicación; (F) el solo lectura lo impone la aplicación. (G) La ejecución de la cadena entera de §1 sale a un objetivo propio de aceptación, posterior; su orden respecto a la Fase 12 se decide al cerrar `O-curso`.
  DEUDA. Pasan a bloquear `D-horario-id-a-fuego` (condición 4) y `D-e2e-aislamiento` (condición 8). Medido que NO bloquean `D-horario-irreversible` ni `D-e2e-centro-minimo-rojo`. Nacen **`D-esquema-sin-version`** (sin objetivo asignado) y **`D-playwright-cita-seed-difunto`** (sede `O-curso`, condición 8) y **`D-censo-r4-rescate-por-indice`**, con sede Higiene/Método. El 409 sin `message` NO nace como token nuevo: `D-F8.6-ii-a` está viva y describe el mismo síntoma, así que entra allí como INSTANCIA de S158, en las dos sedes. Tampoco nace la deuda que el guion preveía por si la comprobación 3 del verificador diera un OK falso: resultó CORRECTA POR DISEÑO, así que no se le pone token (criterio de S157 con `O-nombre`: un identificador que nadie va a usar no se escribe). Instancia de `D-deuda-sin-sede-en-el-plan`: `D-horario-irreversible` no tiene entrada en el plan. Defectos de instrumento: (1) de Claude Code, el inventario de mappings v1 casaba el literal del `@PathVariable` y publicó rutas inexistentes; corregido en v2 y contrastado con un recuento independiente (60 = 60); (2) de Claude Code, un `grep -q` sobre el árbol de dependencias salió vacío, repetido sin `-q`; (3) del asistente, el guion del alta revisó el diff ANTES de regenerar los índices y se commiteó otro diff: comprobado en este cierre contra `2fbdab4`, TODAS las diferencias entre lo revisado y lo commiteado caen dentro de los bloques de índice generados —cero fuera—, así que el cuerpo revisado es el que se commiteó y el defecto queda en el procedimiento, sin daño. Con `O-curso` activo, la excepción de §6 no aplica; Higiene sigue siendo candidata al cerrar `O-curso`, como dejó escrito S157.
  CORRECCIONES. (a) Del asistente: en la apertura afirmó que `D-horario-irreversible` pasaba a bloquear el objetivo; la medición lo desmintió dentro de la sesión. (b) La comprobación 3 del verificador: CORRECTA POR DISEÑO, porque sus tres marcadores dicen hasta dónde llega la bitácora y no en qué sesión estamos —el M1-bis archiva con dos sesiones de retraso y los mueve juntos, como midió el commit `89a8be0` al pasarlos de S154 a S155—, así que S155 era el valor correcto en S158 antes de archivar S156. (c) Del asistente: la instancia de S158 en `D-F8.6-ii-a` decía que la causa no se investigó, cuando la propia ficha la tiene medida desde S118; corregido en el cierre, señalado por Claude Code.
  ...
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos en el cierre: INTACTOS. Las mediciones trabajaron sobre copias en `/tmp`.
  LIMPIEZA (M1-bis): archivada S156 a `bitacora-sesiones.md`; degradada S157 a «Última sesión registrada (previa):»; S158 queda como única cabecera H3 viva.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (12 defectos inyectados, los 12 detectados). Índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados dos veces —tras el cierre y tras la enmienda que corrige la instancia de `D-F8.6-ii-a` y da de alta la deuda del censo—, la última **de 9 y 9 entradas descuadradas a 0 y 0** sobre 32 y 68 encabezados; el descuadre lo produce la propia sesión al insertar filas en §4, entradas de deuda en el plan, archivar el bloque de S156 y escribir esta entrada. Esta línea sustituye a otra línea, así que no vuelve a descuadrarlos: las cifras que constan son las medidas DESPUÉS de escribirla. **Censo: 284 tokens distintos y 39 con UNA sola aparición; 1 sólo en la entrada de sesión; 0 extinciones respecto de HEAD.** Movimientos respecto de los 281/40/1/0 medidos tras `2fbdab4`: (a) +3 distintos, que son los tres tokens que nacen hoy —`D-esquema-sin-version`, `D-playwright-cita-seed-difunto` y `D-censo-r4-rescate-por-indice`—, los tres con 2 apariciones, sus dos sedes, y por eso fuera de la lista de sospechosos. (b) 40 → 39 porque sale `C-alcance-curso`, y la causa está MEDIDA y no es que nadie lo cite: su segunda aparición vivía en el ÍNDICE GENERADO del plan, que reproduce la cabecera de esta entrada. El censo retira la entrada de sesión del corpus (30 líneas: la viva y la previa degradada) pero NO su reflejo en el índice, de modo que la cabecera rescata a los tokens que nombra. Es el mismo modo de fallo que S157 pagó con `D-censo-r4-cuenta-menciones`, por una puerta que aquella sesión no miró; nace `D-censo-r4-rescate-por-indice` (Higiene/Método). Hoy `C-alcance-curso` cuenta 4 apariciones, porque la ficha de esa deuda nueva lo cita en sus dos sedes como caso medido; lo que lo sacó de la lista fue la línea del índice, y ése es el hecho que la deuda registra. (c) El «sólo en la entrada» sigue siendo 1 y sigue siendo `O-nombre`. En una corrida intermedia fueron 2: esta entrada nombraba el token de la deuda que el guion preveía para la comprobación 3, que NO nace, y escribir un identificador que nadie va a usar es exactamente lo que S157 retiró con `O-nombre`; se reescribió la frase sin token y volvió a 1. (d) Cero extinciones: nada de lo que esta sesión movió perdió su última cita. (e) Los 39 de una aparición son los 40 de `2fbdab4` menos `C-alcance-curso`, y siguen sin revisar por R-terminado: ninguna ficha del alcance pedía vaciar esa lista. (f) HIGIENE/MÉTODO: **12 fichas**, una más que en S157 por la deuda anterior, contadas por la columna de sede de §4 con el criterio de S142; **el disparador por recuento NO está activo: 12 de 20**. Un contador que casara «Higiene/Método» en cualquier columna daría 13, porque `D-tokens-inexistentes` la nombra en su disposición y su sede es otra. **R-deuda aguanta con `O-curso` activo:** de las deudas vivas las únicas que bloquean son las dos que el objetivo absorbe —`D-horario-id-a-fuego` (condición 4) y `D-e2e-aislamiento` (condición 8)—, y ninguna de las tres que nacen hoy bloquea nada.»

Última sesión registrada (previa): Sesión 157 — Higiene/Método, entre objetivos (excepción de §6): **EL CENSO DE R4 DEJA DE CONTAR MENCIONES, VE LOS OBJETIVOS Y LAS EXTINCIONES, Y LOS ÍNDICES PASAN A FALLO DURO; LAS NORMAS DE GUION SE REÚNEN EN `metodo.md`.** Fichas con sede Higiene/Método o script de R4: **de 19 a 11**. Primera sesión abierta por la excepción de S152.
  ...
  DEUDA. Ocho cerradas, ninguna nueva. Un defecto de instrumento, del asistente: el ancla de la fila de tipos se escribió desde la copia de `metodo.md` del Project, anterior a S152; la guarda de ancla única la paró sin escribir, costó una ronda y dio la precisión de la norma 3. Un segundo, también del asistente: el guion de cierre pidió ancla única para los encabezados «### Deuda consciente VIVA» y «### Deuda consciente CERRADA (histórico)», que el índice generado cita literalmente (M-doc, punto 6); la primera corrida en seco abortó sin escribir y Claude Code contó por línea exacta. Dos defectos de instrumento con trabajo medible en la misma sesión: **el disparador de §6 SALTA** por su segunda condición; como la excepción sólo abre Higiene entre objetivos, vuelve a ser candidata al cerrar el próximo objetivo.
  ...
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (**12** defectos inyectados, los 12 detectados, recuento calculado y no escrito a mano). Índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 5 y 10 entradas descuadradas a 0 y 0** sobre 31 y 68 encabezados; el descuadre lo produjo esta misma sesión al mover cuatro entradas de deuda, archivar el bloque de S155 e insertar esta entrada, y se corrigió regenerando —el retoque final no descuadró nada: 0 y 0 medidos antes de regenerar—. Desde S157 un índice descuadrado o ausente SUMA a los fallos duros, así que esta línea ya no puede salir en verde sin regenerar. **Censo: 279 tokens distintos y 39 con UNA sola aparición; 1 sólo en la entrada de sesión; 1 extinción respecto de HEAD.** Primera medición con el instrumento nuevo, y la serie NO es comparable con la de S141–S156 (ver DECISIONES, 4). (a) La extinción es `O-nombre`, y es CORRECTA y buscada: era el ejemplo literal de encabezado de objetivo de `metodo.md`, reescrito con `<nombre>` para que dejara de ser un identificador falso. Que salga en 2c y no en 2a es el resultado que la sesión perseguía al pagar `D-censo-r4-ciego-a-la-extincion`: un token que pierde su última cita ya no desaparece en silencio. (b) El 1 de «sólo en la entrada de sesión» es ese mismo `O-nombre`, nombrado en esta línea y en ninguna otra parte del corpus vivo; la categoría es de S157 y sirve justamente para que una mención en una entrada que se archivará no se confunda con un citante vivo. La cita que SÍ lo rescataba estaba en §4, en el cierre de su propia ficha, y se retiró en el retoque: de ahí que la corrida anterior diera 40 y 0, y ésta 39 y 1. (c) Esta línea de R4 no rescata a nadie: vive dentro de la entrada de sesión del plan, que el censo retira del corpus (34 líneas retiradas). Es la comprobación en vivo de lo que la sesión pagó con `D-censo-r4-cuenta-menciones`. (d) Los 33 tokens de una aparición preexistentes siguen sin revisar, por R-terminado: ninguna ficha del alcance pedía vaciar esa lista. (e) HIGIENE/MÉTODO: **11 fichas** por la columna de sede de §4 con el criterio de S142, la cifra que el M0 fijó como criterio de terminado; las once están en «Transversal, con la sesión de Higiene/Método», y las otras dos sedes —«Transversal, sesión de Higiene/Método» y «Transversal, con el script de R4 pendiente desde S101»— quedan a CERO: las seis fichas que las poblaban se cierran y `D-arranque-no-literal` se muda a la primera. **El disparador de la excepción a R-apertura NO está activo por recuento: 11 de 20**, pero **el de §6 SÍ salta** por su segunda condición (dos defectos de instrumento con trabajo medible en la misma sesión, ver DEUDA). **R-deuda aguanta**: no hay objetivo activo, y ninguna de las once fichas vivas de Higiene bloquea nada.»
...
- **D-playwright-cita-seed-difunto** (S158, VIVA, DE DOCUMENTACIÓN, no bloqueante, sede `O-curso` condición 8)
