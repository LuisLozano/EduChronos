...
- L659 — ## FASE 9 — Exportación
- L662 — ### Criterios de verificación
- L670 — ## FASE 10 — Gestión de cursos académicos
- L679 — ### Entregable
- L685 — ### Criterios de verificación
- L694 — ## FASE 11 — Empaquetado y distribución Windows
- L697 — ### Criterios de verificación
- L706 — ## FASE 12 — CI/CD con GitHub Actions
- L709 — ### Criterios de verificación
- L716 — ## Registro de progreso
- L718 — ### Sesión 145 — O-ajuste-cierre (H1), C-gesto-rejilla: **ARRASTRAR YA REUBICA, Y EL PIN DEJÓ DE COLGAR DEL ARRASTRE PARA TENER GESTO PROPIO.** Tres condiciones del criterio pasan a CUMPLIDAS —(1) entera, (2) entera y con su precisión resuelta, y (4)—, y la (5) vuelve a ser alcanzable tras una regresión que el propio Cambio introdujo y reparó en la misma sesión. El M4 sobre el centro real ejecutó cinco pasadas más una complementaria, y el verde del intercambio se demostró DOS veces: primero sobre un par degenerado y después sobre uno multiplaza entre actividades distintas, que es donde reagrupar mal sí se nota. vitest 441 -> 477.
- L786 — ### Bloques de Fase 2
- L794 — ### Bloques de Fase 5
- L817 — ### Bloques de Fase 6
- L862 — ### Bloques de Fase 8
- L1071 — ### Fases completadas
- L1175 — ### Cierre del modelo — Sesión 8
- L1223 — ### Decisiones permanentes (no reabrir sin razón de peso)
- L1269 — ### Método de trabajo (procedimiento vigente)
- L1302 — ### Deuda consciente VIVA
- L3063 — ### Deuda consciente CERRADA (histórico)
- L3161 — ### Notas técnicas validadas en Fase 0
- L3171 — ### Notas técnicas validadas en Fase 6
- L3205 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3214 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3241 — ### Registro detallado de sesiones S10–S31
- L3250 — ## Señales globales de alerta
...
- **D-spa-sin-fallback-de-rutas** (S145, VIVA, no bloqueante) — el jar no reenvía las rutas profundas de la SPA
  al `index.html`. Nace MEDIDA en el M4 y es PREEXISTENTE: no la introduce este Cambio.
  `http://localhost:8080/horario/1` devuelve 404 con la Whitelabel Error Page; hay que entrar por `/` y navegar
  desde la landing. Dos daños: cualquier M4 futuro que intente abrir una URL profunda pierde el tiempo antes de
  entenderlo, y no se pueden compartir enlaces a una vista concreta. **Sede H4**, porque lo que falla es el
  empaquetado del jar servido, que es su superficie; H4 no está descompuesto en objetivos todavía, así que la
  deuda espera a su apertura. Queda además anotado en la nota de arranque de la bitácora: todo M4 entra por `/`.
