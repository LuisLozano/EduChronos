...
- L675 — ## FASE 10 — Gestión de cursos académicos
- L684 — ### Entregable
- L690 — ### Criterios de verificación
- L699 — ## FASE 11 — Empaquetado y distribución Windows
- L702 — ### Criterios de verificación
- L711 — ## FASE 12 — CI/CD con GitHub Actions
- L714 — ### Criterios de verificación
- L721 — ## Registro de progreso
- L723 — ### Sesión 146 — O-ajuste-cierre (H1), C-prevalidacion-s8: **LA PREVALIDACIÓN COMPRUEBA S8 Y DEJA DE DECLARAR «CATÁLOGO SANO». O-ajuste-cierre CUMPLE SU CRITERIO Y CIERRA, y con él CIERRA H1: segundo hito terminado del proyecto.** S8 entra como cuarta regla de `PrevalidacionService` REUTILIZANDO la comprobación del solver y con severidad AVISO, primer productor de AVISO de la aplicación. Demostrada sobre el centro real por mutación de tutorías sobre copia y por la vía legítima, con la predicción escrita antes de medir y un oráculo SQL independiente: 12 de 12 predicciones. solver 91 -> 92, app 334 -> 339, vitest 477 -> 478.
- L789 — ### Bloques de Fase 2
- L797 — ### Bloques de Fase 5
- L820 — ### Bloques de Fase 6
- L865 — ### Bloques de Fase 8
- L1074 — ### Fases completadas
- L1178 — ### Cierre del modelo — Sesión 8
- L1226 — ### Decisiones permanentes (no reabrir sin razón de peso)
- L1272 — ### Método de trabajo (procedimiento vigente)
- L1305 — ### Deuda consciente VIVA
- L3055 — ### Deuda consciente CERRADA (histórico)
- L3154 — ### Notas técnicas validadas en Fase 0
- L3164 — ### Notas técnicas validadas en Fase 6
- L3198 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3207 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3234 — ### Registro detallado de sesiones S10–S31
- L3243 — ## Señales globales de alerta
...
### Sesión 146 — O-ajuste-cierre (H1), C-prevalidacion-s8: **LA PREVALIDACIÓN COMPRUEBA S8 Y DEJA DE DECLARAR «CATÁLOGO SANO». O-ajuste-cierre CUMPLE SU CRITERIO Y CIERRA, y con él CIERRA H1: segundo hito terminado del proyecto.** S8 entra como cuarta regla de `PrevalidacionService` REUTILIZANDO la comprobación del solver y con severidad AVISO, primer productor de AVISO de la aplicación. Demostrada sobre el centro real por mutación de tutorías sobre copia y por la vía legítima, con la predicción escrita antes de medir y un oráculo SQL independiente: 12 de 12 predicciones. solver 91 -> 92, app 334 -> 339, vitest 477 -> 478.
  M0. Cambio `C-prevalidacion-s8`, creado en este M0: la ficha lo nombraba «la prevalidación S8 de la condición (3)» sin token. Objetivo **O-ajuste-cierre**, Hito **H1**; último Cambio del objetivo. R-invalidación: ninguna. Tipo Desarrollo. Recuento de Higiene/Método por la columna de sede de §4 con el criterio de S142: **16** —13 más 3—, **CUARTA reproducción consecutiva**; los dos falsos positivos del grep (`D-tokens-inexistentes`, `D-javadoc-plazas-caducado`) no entran por su sede. Ninguna bloquea la condición (3). **LA DEMOSTRACIÓN SE DECIDIÓ ANTES DE ESCRIBIR UNA LÍNEA**, contra la trampa que el prompt de apertura dejaba escrita —sobre este centro la lista seguiría vacía—: mutación sobre copia por el endpoint de tutoría, predicción escrita antes de medir, y cinco criterios de aborto (A1, S8 lee la solución; A2, ninguna mutación legítima da un conjunto no vacío; A3, conjunto distinto de la predicción; A4, falso positivo sin mutar o restaurado; A5, prevalidación y diagnóstico discrepan). **Y EL PROMPT TRAÍA UN DATO CADUCADO**: «los 7 tutores de Bachillerato son falsos pero portantes» dejó de ser cierto en S137, que los reescribió con la lista oficial; lo arrastraban S142, dos frases de la ficha de §3 y el M0 de S145. La conclusión práctica aguantaba —la lista vacía—, pero no su lectura: el vacío es la respuesta CORRECTA sobre un catálogo correcto, y lo que faltaba era demostrar que la comprobación corre. Corregido en la ficha (R5).
  M2, SOLO LECTURA. `verificarTutorias` (`VerificadorSolucion`, privado) NO lee la solución, ni directa ni indirectamente, así que A1 no dispara. Su semántica es un EXISTS anclado en la MISMA plaza: basta UN profesor de una plaza que sea TUTOR_PRINCIPAL de un grupo cubierto por esa plaza, y CO_TUTOR no vale. La guarda de `generar()` (`GeneradorHorarioService:196`) sólo aborta ante ERROR. `PUT /api/grupos/{id}/tutoria` es reemplazo total y permite dejar un grupo sin tutor principal —I4 sólo exige «como mucho uno»—, así que A2 tampoco dispara. Datos, sobre copia de la referencia: 16 actividades con `requiereTutor` y las 16 cumpliendo; 21 grupos con exactamente una actividad tutorial y los 7 de Bachillerato con ninguna; 7 de 7 tutores oficiales; las 6 de bloque a 0. El literal «Catálogo sano» no lo aseveraba ningún test. Línea base medida corriendo: 91 + 334 + 477 en 52.
  DECISIONES, cada una con su razón. (a) **Reutilización, no espejo**: `VerificadorSolucion` gana un público `verificarTutorias(ProblemaHorario)` que delega en el privado; se descartó llamar a `verificar()` con solución vacía y filtrar, que devolvería un `INSTANCIA_SIN_COLOCAR` por instancia y dependería de que las otras cuatro comprobaciones toleren el vacío. (b) **Severidad AVISO, contra el criterio escrito de `Severidad`**, que reservaba AVISO a las reglas que pueden sobrestimar la demanda —S8 es exacta y por ese criterio sería ERROR—. Decidido por el usuario con dos argumentos: S8 no depende de la colocación, así que el horario generado es válido en todo lo demás y se corrige cambiando el tutor SIN regenerar; y con la lista de tutores pendiente —más de veinte sesiones en este centro, S115–S137— un ERROR habría bloqueado la generación sobre el catálogo honesto, dejando como única salida falsear datos, la misma clase de dato inventado que el Hallazgo E metió y S137 desmontó. El javadoc de `Severidad` se amplía con esa segunda vía. (c) Mapeo: entidad = código de la ACTIVIDAD —unidad de deduplicación, nunca nula; el `recursoCodigo` del grupo puede serlo—, **demanda 1 / disponible 0** para respetar el «se emite sólo si demanda > disponible» sin tocar el contrato, y al final del orden. (d) El estado vacío NO enumera las reglas —sería un espejo en el cliente—: dice que no hay hallazgos y que eso no garantiza horario en el tiempo previsto. **UNA CORRECCIÓN DEL ARQUITECTO A SÍ MISMO**, que se registra porque un método que sólo anota los aciertos no mide nada (S138): el argumento de que desde S145 un AVISO se ve siempre en el diálogo era FALSO —`horario-view.ts:571` sólo pasa los ERROR— y se retiró; la decisión se sostuvo con los otros dos. No se amplió el diálogo (R-terminado): nace `D-aviso-fuera-del-dialogo`.
  COBERTURA Y MUTACIÓN. +1 en solver (la vía sin solución coincide con `verificar()` filtrada, como multiconjunto y con guarda de no vacío), +5 en app (P1-P3, E1, G1; G1 con guarda anti-tautología: comprueba antes que la prevalidación emite el AVISO) y +1 en vitest (caso (33), el literal con `toBe`). **Ocho mutantes, ocho muertos**: S1 (5 víctimas), A1 (4), A2 —AVISO -> ERROR— (4, las previstas), A3 (2), A4 (1), A5 (1), A6 —la guarda de `generar()` abortando ante cualquier hallazgo— (1) y F1 (1). **El protocolo que pidió el arquitecto FALLABA:** `mvn test` a secas detiene el reactor en `solver` y las víctimas de `app` no se miden —S1 dio una víctima cuando eran cinco—; el ejecutor lo cazó y pasó a `-Dmaven.test.failure.ignore=true`. Instancia del asistente de la familia `D-guion-exit-enmascarado`: un instrumento que mide otra cosa distinta de la que se cree. **Y una explicación falsa del ejecutor, corregida por él mismo al verificarla:** atribuyó el 400 de A6 a que el harness no monta un advice; la causa real es una guarda de dominio —`PrevalidacionFallidaException` exige al menos un ERROR para construirse—, de modo que la decisión AVISO queda protegida dos veces. No entró en ningún documento sin medir.
  **M4 SOBRE EL CENTRO REAL: 12 DE 12 PREDICCIONES, escritas en el turno anterior al de medir.** Copia `educhronos-s146-m4.db` desde `s144-m2`; el jar verificado con `javap` —lleva el público nuevo— y la base servida por `/proc/<pid>/fd`. **Oráculo SQL independiente sobre `/tmp` antes de tocar la base servida**, con la semántica de reemplazo del PUT: violan exactamente TUT1-1ºA y TUT2-2ºB, y TUT3-3ºA+3ºADi cumple. Sin mutar: `GET /api/prevalidacion` = `[]`, diagnóstico sin S8 y el panel con el literal nuevo. Tres PUT de tutoría —1ºA a `[]`, 2ºB con su tutor degradado a `CO_TUTOR` y 3ºA a `[]`—: la prevalidación devolvió **exactamente dos hallazgos, AVISO, `TUTORIA_SIN_TUTOR`, 1/0, {TUT1-1ºA, TUT2-2ºB}**, y **TUT3 ausente**, que es la parte que discrimina: basta un grupo cubierto, y el PUT al padre no se propaga al PDC (3ºADi intacto). `/diagnostico` dio el mismo conjunto, nombrando el grupo donde la prevalidación nombra la actividad. En el navegador, entrando por `/`: errores 0, «otros hallazgos» 2, marco sin `es-error` y panel colapsado al cargar. **Es el primer AVISO de la aplicación visto en vivo**: el diseño de S92 sólo se había ejercitado con datos sintéticos. Restauración: `[]`, diagnóstico sin S8, `profesor_tutoria` idéntica, `.dump` ordenado idéntico y el crudo difiriendo sólo en el orden de tres filas de esa tabla. Ningún clic en «Generar horario» y ningún presupuesto de 600 s gastado.
  CRITERIO. Condición **(3) CUMPLIDA**. Con (1), (2), (4) y (5) ya cumplidas, **O-ajuste-cierre CUMPLE SU CRITERIO Y CIERRA** tras cinco sesiones (S142–S146) y cinco Cambios —`C-alcance-ajuste`, `C-mover-sesion-backend`, `C-intercambiar-instancias`, `C-gesto-rejilla` y `C-prevalidacion-s8`—, por debajo del ocho de §7: el disparador de replanteo no se disparó nunca. **Con él CIERRA H1**, segundo hito terminado tras H2 (S141). El paso 4 del guion de §1 deja de fallar como eslabón; la cadena entera sigue sin ejecutarse, porque H3 está al 0 % y H4 sin verificar. Los criterios 1-4 de Fase 8 NO se marcan: se cumplió el criterio que los sustituye y no su redacción literal (nota actualizada en la Fase 8). **Siguiente: H3, que todavía no tiene ningún objetivo escrito en §3.**
  DEUDA. **`D-F8.4-A-c` CERRADA por su propia condición escrita** —«hasta que exista un productor de `AVISO`»—: lo es S8, y la severidad discrimina, medido por el mutante A2; su texto pasa a la sección de deuda cerrada. Nace **`D-aviso-fuera-del-dialogo`** en «Mejora futura», **sin sede** por el precedente de S141, porque nace el día que su objetivo cierra. Se anotan sin cerrarse las dos fichas cuyo texto ha cambiado de verdad: `D-prevalidacion-ciega-a-holgura-cero` (se paga la mitad S8 de la condición 3; la ceguera a holgura cero sigue viva y el texto nuevo la declara en vez de ocultarla) y `D-meta-invariantes-a-mano` (la red de S8 deja de estar después de los diez minutos de solver; sigue después de las 816 escrituras). **Las 13 fichas vivas con sede O-ajuste-cierre pasan a «O-ajuste-cierre (cerrado S146)»**, con el precedente de `D-tutor-pdc-desincronizado`. Por el argumento escrito en S132 y S141, eso equivale a no pagarlas mientras nadie las reasigne, y no se reasignan aquí porque hacerlo exige leer cada una. **Costura R5 pagada en el camino: `referencia-codigo-solver.md` estaba desfasada desde el cambio que describe S118** (commit `1f447c4`, la excepción del solver conserva el estado de CP-SAT), que tocó `solver/src/main` sin regenerarla. S146 la regenera cubriendo los dos cambios, y el `git diff dc2ab65` delimita la regeneración a cuatro ficheros. **Deuda bloqueante del proyecto: 0.**
  R4 / COSTURA. `verificar-cierre.py`: 0 comprobaciones duras con fallo tras el cierre; índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, con 0 y 0 entradas descuadradas. **Censo de tokens sospechosos: 34**, frente a **33** en S145: el primer cierre que puede compararse, porque S145 dejó su número escrito. una entrada y ninguna salida respecto a la corrida previa al cierre, que también dio 33 porque nada tocó el corpus vivo entre los dos cierres; la entrada la causa el archivado de S144, que retira del corpus vivo la segunda cita de un token, y no una degradación nueva; ningún token de los que esta sesión crea, cierra o anota cae a cero apariciones en el corpus vivo.
  SUITES. solver **91 -> 92**, app **334 -> 339**, vitest **477 -> 478** (52 ficheros). `solver/src/main` TOCADO: `referencia-codigo-solver.md` regenerada en `bd197d7`. Código en `ad6865a`.
  BANCOS. Referencia `educhronos-s137-centro-completo.db` en `64d671fe…`, verificada al abrir y al cerrar: **INTACTA**. `s142-m2`, `s143-m4` y `s144-m2` en `5f02ef43…` —el M2 midió que son el MISMO fichero byte a byte—, `s144-m4` en `73dcabd2…` y `s145-m4` en `95050aaa…`, intactos. Nueva copia `educhronos-s146-m4.db`, que cierra en **`e8d7d45f504023ccbe8327a3df973c85`** con el contenido lógico idéntico al de partida y tres filas de `profesor_tutoria` reordenadas por el DELETE+INSERT del reemplazo: un md5 no prueba la no-mutación de una base servida; el `.dump` ordenado sí.
  LIMPIEZA (M1-bis): archivada S144 a `bitacora-sesiones.md` (promovida a `### Sesión 144`, al final, cuerpo íntegro); degradada S145 a «Última sesión registrada (previa):»; S146 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S143 a S144. M1.5: no se condensa nada; la ficha de un objetivo cerrado se conserva como registro, como las de O-demo y O-particiones.

Última sesión registrada (previa): Sesión 145 — O-ajuste-cierre (H1), C-gesto-rejilla: **ARRASTRAR YA REUBICA, Y EL PIN DEJÓ DE COLGAR DEL ARRASTRE PARA TENER GESTO PROPIO.** Tres condiciones del criterio pasan a CUMPLIDAS —(1) entera, (2) entera y con su precisión resuelta, y (4)—, y la (5) vuelve a ser alcanzable tras una regresión que el propio Cambio introdujo y reparó en la misma sesión. El M4 sobre el centro real ejecutó cinco pasadas más una complementaria, y el verde del intercambio se demostró DOS veces: primero sobre un par degenerado y después sobre uno multiplaza entre actividades distintas, que es donde reagrupar mal sí se nota. vitest 441 -> 477.
