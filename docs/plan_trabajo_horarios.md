...
- L754 — ### Sesión 165 — O-disponibilidad (H2), C-dura-completa: **LA INDISPONIBILIDAD DEL PROFESOR PASA A SER REGLA DURA DEL VERIFICADOR POR TRAMO OCUPADO, Y LA PREVALIDACIÓN RECHAZA EL PIN SOBRE UN TRAMO DURA Y EL BLOQUE CON DISPONIBILIDAD. EL SOLVER QUEDA PENDIENTE: 0 DE 7.**
...
- L3173 — ### Deuda consciente CERRADA (histórico)
- L3430 — ### Notas técnicas validadas en Fase 0
- L3443 — ### Notas técnicas validadas en Fase 6
- L3488 — ### Notas técnicas validadas en Fase 9
- L3497 — ### Notas técnicas validadas en Fase 11
- L3639 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3648 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3675 — ### Registro detallado de sesiones S10–S31
- L3684 — ## Señales globales de alerta
...
### Sesión 165 — O-disponibilidad (H2), C-dura-completa: **LA INDISPONIBILIDAD DEL PROFESOR PASA A SER REGLA DURA DEL VERIFICADOR POR TRAMO OCUPADO, Y LA PREVALIDACIÓN RECHAZA EL PIN SOBRE UN TRAMO DURA Y EL BLOQUE CON DISPONIBILIDAD. EL SOLVER QUEDA PENDIENTE: 0 DE 7.**
  ...
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 22 y 19 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD (`a43a5e7`) los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo este cierre, en gestión al reescribir la condición 5 y la línea de estado y añadir las Decisiones (S165), y en el plan al insertar la entrada de S165 y archivar S163; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 40** sobre 303 tokens distintos en el corpus vivo, frente a **40** sobre 303 en HEAD, medido corriendo el verificador sobre un worktree de HEAD: sale `C-dura-completa` y entra `C-alcance-disponibilidad`. (a) Los tres tokens nuevos de la sesión, en el corpus vivo: `D-indisp-solo-tramo-de-inicio` con 5 apariciones —su ficha del plan, las fichas del plan de las otras dos deudas, la línea de estado de la ficha de `O-disponibilidad` y su fila de §4—, `D-ventanas-consecutivas-ciegas-a-bloques` con 2 —su ficha del plan y su fila de §4— y `D-modelo-no-determinista-entre-jvm` con 3 —su ficha del plan, la de `D-indisp-solo-tramo-de-inicio` y su fila de §4—; ninguno está en la lista. (b) `C-dura-completa` sale con 7 apariciones: la fila de H2 en §2, los Cambios de la ficha, la decisión (F), la fila de §4 de `D-indisp-solo-tramo-de-inicio` (dos) y su ficha del plan, que escribe este cierre, y la entrada del ÍNDICE GENERADO que reproduce la cabecera de S165, séptima reproducción medida de `D-censo-r4-rescate-por-indice`; sin ella tendría 6 y saldría igual. `C-alcance-disponibilidad` entra con 1, los Cambios de la ficha: perdió la aparición del índice al degradarse la cabecera de S164, que lo nombraba; es el mismo mecanismo en sentido inverso, y el token se aceptó sin acción en S164. (c) La línea de R4 no rescata nada: la entrada de sesión sale del corpus (38 líneas retiradas, con la S164 degradada). (d) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S163** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S163 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 17 líneas; título idéntico). (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en HEAD, porque las deudas nuevas no tienen esa sede; el filtro mecánico por la columna de sede de §4 da 18 filas, 5 tachadas, igual en HEAD y ahora; **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S164 degradada y S165 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda aguanta: ninguna deuda viva bloquea `O-disponibilidad`.»

Última sesión registrada (previa): Sesión 164 — O-disponibilidad (H2), C-alcance-disponibilidad: **H2 ABRE `O-disponibilidad` CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 7.** La medición encuentra que la DURA sólo veta el tramo de inicio de cada sesión y que el verificador no la comprueba; su arreglo entra en el criterio. Se descarta inferir la disponibilidad de los horarios del centro.
  TIPO Y RITUAL: APERTURA DE OBJETIVO — M0 + M2 de solo lectura + alta documental con revisión del diff + M1. Sin código de la aplicación. Precedente: `C-alcance-curso` (S158).
  M0. Cambio `C-alcance-disponibilidad`, Objetivo `O-disponibilidad`, Hito H2: el paso 2 de §1 es el único eslabón que falla, y el orden lo decidió S163. Sin R-invalidación: la aceptación usará la rejilla y no la rehará, la Fase 12 no la toca y `O-diseño` está terminado. Ninguna deuda bloquea. Higiene no compite: su disparador no está activo.
  DECISIÓN DE MAPA (usuario). El centro no entregará el formulario en papel. El usuario propuso inferir la disponibilidad de los horarios del centro; el arquitecto lo desaconsejó y el usuario lo aceptó: un hueco en un horario es holgura del reparto y no una imposibilidad; como DURA forzaría el horario anterior, como BLANDA lo favorecería, y en los dos casos sería un dato inventado, contra la definición de «presentable» de S136. La dependencia externa de la ficha desaparece: el centro introducirá los datos con la rejilla.
  MEDICIÓN (M2, HEAD `fc9076d`, Claude Code, solo lectura). (1) El PUT de restricciones valida profesor, tramo lectivo por `(dia, ordenEnDia)`, tipo y duplicados; el recreo no tiene `ordenEnDia` y da 400; `peso` no viaja y `motivo` sí, sin validar; sin UNIQUE ni índices. (2) `PROFESOR_SOBRECARGADO` ya compara la carga con los tramos libres tras las DURA. (3) DURA y BLANDA sólo restringen el tramo de INICIO; un bloque de más de un tramo puede ocupar uno vetado (por lectura; los bancos no tienen actividades multi-tramo). (4) El verificador no tiene regla de indisponibilidad, y el movimiento manual valida con él. (5) Nada detecta un pin sobre un tramo DURA antes del solver; el estado de CP-SAT en ese caso, NO DETERMINADO. (6) La guarda de solo lectura cubre el PUT sin configuración. (7) Una sola restricción congela la jornada (409). (8) Frontend: nada alcanza el endpoint, la rejilla del horario no sirve, no hay librería de iconos ni tema oscuro.
  CRITERIO. `O-disponibilidad` escrito en §3 con siete condiciones: entrada desde Profesores, tres estados que se distinguen sin depender del color y con leyenda, pincel por celda, día y tramo, guardar con los rechazos visibles, la DURA entera con regla en el verificador, el pin contradictorio rechazado en la prevalidación y la verificación sintética en navegador. **0 de 7.** `C-alcance-disponibilidad` ✔ HECHO (`8461fe6`). Siguen `C-dura-completa` (backend, primero) y `C-rejilla-disponibilidad` (maqueta y frontend).
  DECISIONES. Del usuario: el marcado en bloque, por día y por tramo, entra en el criterio. Del arquitecto, aceptadas: pincel y no rotación por clic; el recreo no es editable —el usuario lo proponía para las guardias de recreo, pero en el recreo no se programa nada y una guardia es una asignación, no disponibilidad; la gestión de guardias queda en «Fuera»—; los hallazgos (3) a (5) entran en el criterio y no son deuda; dirección de los símbolos (celda vacía, prohibido sobre fondo de error, «−» sobre fondo de aviso), que cierra la maqueta. Fuera del criterio: e2e y Windows.
  DEUDA. Nace `D-jornada-congelada-por-disponibilidad` (técnica real, sede aceptación). No se salda ninguna. `D-F8.5-E-b` sigue sin bloquear: la rejilla escribe por el mismo PUT. R-deuda: ninguna deuda bloquea `O-disponibilidad`.
  TOKENS DE UNA APARICIÓN. `C-alcance-disponibilidad` y `C-dura-completa` nacen citados sólo en los Cambios de la ficha. Decisión del arquitecto: se aceptan sin acción, porque son Cambios planificados que citarán sus propias sesiones.
  DEFECTOS DE INSTRUMENTO. (1) Del asistente: el guion del M2 pidió sha256 para unos hashes registrados como md5; Claude Code midió los dos y los bancos resultaron intactos. (2) De Claude Code: dos citas del informe del M2 (el número de casos de un test y una línea de la rejilla) se corrigieron después de escribirlo.
  SUITES: sin cambio de código, no se corrieron: **solver 92, app 534, vitest 513 en 55 ficheros**, y el recuento estático del M2 cuadra con las tres; e2e 3 tests en 3 ficheros, con `centro-minimo` rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…` (md5), medidos en el M2 antes y después de las consultas de solo lectura y otra vez en el cierre: INTACTOS.
  LIMPIEZA (M1-bis): archivada S162 a `bitacora-sesiones.md`; degradada S163 a «Última sesión registrada (previa):»; S164 queda como única cabecera H3 viva. Condensación: no hay frente cerrado acumulado.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 0 y 19 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD (`8461fe6`) los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo esta misma sesión en el plan, al insertar la cabecera de S164 y archivar S162; gestión no se toca en el cierre, porque el alta de `8461fe6` ya dejó su índice regenerado; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 40** sobre 300 tokens distintos en el corpus vivo, frente a **41** sobre 300 en HEAD, medido corriendo el verificador sobre el árbol de HEAD antes de escribir: sale `C-alcance-disponibilidad` y no entra ninguno. (a) Los cuatro tokens nuevos de la sesión, en el corpus vivo: `C-alcance-disponibilidad` con 2 apariciones —los Cambios de la ficha de `O-disponibilidad` y la entrada del índice del plan—, `C-dura-completa` con 1 —los Cambios de la ficha—, `C-rejilla-disponibilidad` con 2 —la decisión (E) y los Cambios de la ficha— y `D-jornada-congelada-por-disponibilidad` con 3 —el hallazgo (7) de la ficha, su fila de §4 y su ficha del plan—, así que `D-deuda-sin-sede-en-el-plan` no engorda. `C-dura-completa` sigue en la lista, aceptado sin acción (ver TOKENS DE UNA APARICIÓN). (b) La salida de `C-alcance-disponibilidad` NO se debe a una cita: es la entrada del ÍNDICE GENERADO que reproduce la cabecera de S164, **sexta reproducción medida de `D-censo-r4-rescate-por-indice`** tras las de S159 a S163; sin ella el token tendría 1 y seguiría en la lista. Esta vez el rescate sí cambia la lista, pero no una decisión, porque el token ya se aceptaba sin acción. Los otros tres no lo sufren: la cabecera no los nombra. (c) La línea de R4 no rescata nada, porque desde S157 la entrada de sesión sale del corpus: las cifras que constan son las medidas antes de escribirla y se vuelven a comprobar después. (d) Extinción respecto de HEAD (`D-censo-r4-ciego-a-la-extincion`, ya instrumentada): **NINGÚN token cae a 0**, ni entre los citados en el bloque de S162 que esta sesión archiva. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S162** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S162 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 16 líneas). (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en S163 porque hoy no nace ni se cierra ninguna de esa sede —la deuda nueva cuelga de la aceptación—; el filtro mecánico por la columna de sede de §4 da 18 filas, 5 tachadas, igual en HEAD y ahora; **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S163 degradada y S164 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda aguanta: ninguna deuda viva bloquea `O-disponibilidad`.»
