...
### Sesión 146 — O-ajuste-cierre (H1), C-prevalidacion-s8: **LA PREVALIDACIÓN COMPRUEBA S8 Y DEJA DE DECLARAR «CATÁLOGO SANO». O-ajuste-cierre CUMPLE SU CRITERIO Y CIERRA, y con él CIERRA H1: segundo hito terminado del proyecto.** S8 entra como cuarta regla de `PrevalidacionService` REUTILIZANDO la comprobación del solver y con severidad AVISO, primer productor de AVISO de la aplicación. Demostrada sobre el centro real por mutación de tutorías sobre copia y por la vía legítima, con la predicción escrita antes de medir y un oráculo SQL independiente: 12 de 12 predicciones. solver 91 -> 92, app 334 -> 339, vitest 477 -> 478.
  ...
  **M4 SOBRE EL CENTRO REAL: 12 DE 12 PREDICCIONES, escritas en el turno anterior al de medir.** Copia `educhronos-s146-m4.db` desde `s144-m2`; el jar verificado con `javap` —lleva el público nuevo— y la base servida por `/proc/<pid>/fd`. **Oráculo SQL independiente sobre `/tmp` antes de tocar la base servida**, con la semántica de reemplazo del PUT: violan exactamente TUT1-1ºA y TUT2-2ºB, y TUT3-3ºA+3ºADi cumple. Sin mutar: `GET /api/prevalidacion` = `[]`, diagnóstico sin S8 y el panel con el literal nuevo. Tres PUT de tutoría —1ºA a `[]`, 2ºB con su tutor degradado a `CO_TUTOR` y 3ºA a `[]`—: la prevalidación devolvió **exactamente dos hallazgos, AVISO, `TUTORIA_SIN_TUTOR`, 1/0, {TUT1-1ºA, TUT2-2ºB}**, y **TUT3 ausente**, que es la parte que discrimina: basta un grupo cubierto, y el PUT al padre no se propaga al PDC (3ºADi intacto). `/diagnostico` dio el mismo conjunto, nombrando el grupo donde la prevalidación nombra la actividad. En el navegador, entrando por `/`: errores 0, «otros hallazgos» 2, marco sin `es-error` y panel colapsado al cargar. **Es el primer AVISO de la aplicación visto en vivo**: el diseño de S92 sólo se había ejercitado con datos sintéticos. Restauración: `[]`, diagnóstico sin S8, `profesor_tutoria` idéntica, `.dump` ordenado idéntico y el crudo difiriendo sólo en el orden de tres filas de esa tabla. Ningún clic en «Generar horario» y ningún presupuesto de 600 s gastado.
  ...
  R4 / COSTURA. `verificar-cierre.py`: 0 comprobaciones duras con fallo tras el cierre; índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, con 0 y 0 entradas descuadradas. **Censo de tokens sospechosos: 34**, frente a **33** en S145: el primer cierre que puede compararse, porque S145 dejó su número escrito. 1 entradas y 0 salidas respecto a la corrida previa al cierre, que dio 33; ningún token de los que esta sesión crea, cierra o anota cae a cero apariciones en el corpus vivo.
