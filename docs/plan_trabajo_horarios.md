...
- L2956 — ### Deuda consciente CERRADA (histórico)
- L3048 — ### Notas técnicas validadas en Fase 0
- L3058 — ### Notas técnicas validadas en Fase 6
- L3092 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3101 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3128 — ### Registro detallado de sesiones S10–S31
- L3137 — ## Señales globales de alerta
...
Sesión 122, y la de S121 en la Sesión 123, y la de S122 en la Sesión 124, y la de S123 en la Sesión 125, y la de S124 en la Sesión 126, y la de S125 en la Sesión 127, y la de S126 en la Sesión 128, y la de S127 en la Sesión 129, y la de S128 en la Sesión 130, y la de S129 en la Sesión 131, y la de S130 en la Sesión 132, y la de S131 en la Sesión 133, y la de S132 en la Sesión 134, y la de S133 en la Sesión 135, y la de S134 en la Sesión 136.
El plan conserva ahora S135 (degradada a formato compacto) y S136 como única cabecera H3 viva. (Esta frase
...
- **D-hallazgo-E-refutado** (S134, VIVA, no bloqueante) — el Hallazgo E de `modelo_datos_fase1.md` afirma
  ...
  grupo es tutor dejaría el hueco sin sustituto. Se paga cuando `D-tutores-bachillerato` traiga el dato. **S136 mide su huella en la BASE, que hasta ahora sólo se sabía documental:** el `requiere_tutor = 1` de las 6 actividades `Bloque-PTEV/PTVE_Relig` desciende de este Hallazgo, es falso en sí —Bachillerato tiene tutor y NO tiene hora de tutoría, luego ninguna de sus actividades debe exigir tutor— y su corrección a 0 NO depende de saber quién es el tutor real. Lo que sí depende es el orden: bajar el flag sin poner tutores deja el dato ausente, y poner tutores sin bajar el flag rompe S8.
- **D-tutores-bachillerato** (S134, VIVA, LIMITACIÓN CONOCIDA, no bloqueante) — faltan los tutores de los
  ...
  sistema y no en ningún PDF de los entregados. Deuda de REQUISITOS: se cierra preguntando, no midiendo. **MEDIDA EN S136, y lo medido cambia el coste:** las 7 filas de `profesor_tutoria` que hay hoy son PORTANTES —cada actividad 40–45 (`Bloque-PTEV/PTVE_Relig`) se sostiene sobre una de ellas para pasar S8—, así que la corrección es un PAR indivisible: reescribir las 7 tutorías Y bajar `requiere_tutor` a 0 en esas 6 actividades. Lo segundo está bloqueado por `exigirSinDependientes` (409, 12 sesiones sobre esas plazas) mientras exista el horario, luego exige base sin horario y REGENERAR. La ausencia no es cobertura parcial del PDF: trae 80 profesores frente a los 59 del catálogo. Y la pregunta ya estaba escrita desde S115 en la ambigüedad A5, sin cola que la sostuviera (`D-preguntas-sin-cola`).
...
- **D-catalogo-meta-enganosa** (S136, VIVA, DEUDA DE DOCUMENTACIÓN, no bloqueante) — el `_meta` de
  `docs/horario-referencia/catalogo-derivado.json` declara «NO es un fichero que la aplicacion lea. No hay
  importador». Es literalmente cierto —la app no lo lee— y prácticamente engañoso: `cargar-centro.py` lo lee y
  con él escribe el centro entero por la API REST, y S136 midió que es la SEDE ÚNICA de la estructura (el
  cargador abre exactamente ese fichero y `nombres-derivados.json`, y ninguno más; `verificar-conservacion.py`
  lee los volcados `grupo-*.json`, que son la capa de extracción y no de estructura). Un lector concluye que el
  fichero es inerte cuando es la fuente de la que sale la base, y por tanto la sede donde aterriza cualquier
  corrección de catálogo. Familia de `D-tokens-inexistentes`: documento con estado falso. → sesión de
  Higiene/Método.
