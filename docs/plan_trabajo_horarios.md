...
Última sesión registrada (previa): Sesión 160 — O-curso (H4), C-selector-curso: **EL CURSO SE CAMBIA DESDE LA BARRA SIN CERRAR LA APLICACIÓN, Y EL DUPLICADO Y EL SOLO LECTURA LLEGAN A LA INTERFAZ: `O-curso` EN 6 DE 9.**
  ...
  DECISIONES. Cambio de curso EN CALIENTE: un datasource estable (`BaseConmutable`, subclase del envoltorio de claves foráneas) delega en un pool de Hikari que se sustituye, y el contexto de Spring no se reconstruye. Motivos: el e2e corre con URL explícita y tiene que poder cambiar de curso; «Salir» y la bandeja guardan el contexto y no tienen tests. Esto PRECISA la decisión E de S158: el selector vive dentro de la aplicación y el contexto no se rehace. Generar, duplicar y cambiar de curso se excluyen entre sí por transiciones atómicas de `EstadoCurso`; dos generaciones pueden coincidir (contador). Mientras dura el cambio, la guarda rechaza también los GET (503 `CURSO_CAMBIANDO`). Del usuario: al duplicar se abre el curso nuevo en el acto. En la interfaz, la marca de solo lectura va en la barra y no desactiva los botones de las pantallas (R-terminado). Eliminar cursos NO entra: se registra como mejora futura.
  FASE A (`254e9d2`). `BaseConmutable`, `FabricaDeBases` y `BaseConmutableConfig` (autoconfiguración anterior a la de Boot); `GET /api/cursos` (lista de la carpeta de la base abierta, más la base de arranque) y `POST /api/cursos/abrir`; duplicar abre el curso nuevo; se puede duplicar un archivado cuando no queda ningún curso activo (requisito b). +32 tests; 9 mutantes, todos cazados. M4 por HTTP en modo escritorio y en desarrollo, sobre copias: escrituras en el fichero correcto, md5 del archivado intacto, puntero conservado al relanzar, 409 al cambiar durante un solve, instancia única tras un cambio, nada escrito en la carpeta de datos con URL explícita.
  CORRECCIÓN (`e6b15ae`). Generar no se bloqueaba durante un duplicado: una petición que pasaba la guarda antes del duplicado podía acabar escribiendo su horario en el curso archivado. La tabla de exclusiones queda simétrica. Se documenta en el Javadoc la ventana de escritura en vuelo del cambio de curso, en la misma línea que las ventanas de S159. +1 test; m10 cazado.
  UNICIDAD (`bc0355a`). El nombre del curso nuevo se compara con el nombre interno de todos los cursos de la carpeta, no solo con los nombres de fichero; lo destapó el M4 al duplicar 2025/2026 teniendo `educhronos.db` con ese nombre. m11 cazado.
  FASE B (`7825c38`). `curso.service.ts`, componente de barra con el nombre del curso abierto y la marca «Solo lectura», y diálogo que lista, abre y duplica (pide el nombre del curso actual si la base no lo tiene); tras abrir o duplicar, recarga entera en `/`. M3 por mutación en la lógica real (regla de habilitar «Duplicar», campo de nombre actual, recarga solo en éxito, marca). M4 con el usuario, en modo escritorio con bandeja y navegador reales: U1–U9 correctos tras corregir la separación en la barra, los ejemplos en gris de los campos y la unicidad del nombre.
  CRITERIO. `O-curso` **6 de 9** en Linux: cumplidas la 1, la 2, la 3, la 5, la 6 y la 7. Quedan la 4 (horario vigente), la 8 (e2e) y la 9 (Windows). `C-selector-curso` ✔ HECHO.
  DEUDA. Nacen **`D-verificar-cierre-ciego-a-la-ventana`** (Higiene/Método: `verificar-cierre.py` no comprueba la frase de ventana del plan; el cierre de S158 la dejó desfasada y lo detectó a mano el de S159) y **`D-curso-sin-borrado`** (mejora futura, sede `O-curso`: un curso no se puede eliminar desde la aplicación; se revisa al cerrar el objetivo). Nota de S160 en `D-horario-id-a-fuego`: el id 1 se repite entre ficheros de curso. Sin ficha, por el criterio de S157: la ventana de escritura en vuelo (queda en el Javadoc) y el sondeo de la segunda instancia al puerto 8080 fijo (el bundle no permite cambiar el puerto).
  DEFECTOS DE INSTRUMENTO. Del asistente: (1) descartó el hallazgo M4.d del M2 y reapareció como defecto en el M4 de la fase B; (2) la espera por conexiones activas no cubre la escritura en vuelo (documentado, no cubierto); (3) dio la fase B por commiteada en el balance del M1 antes de que lo estuviera, y el prompt de S161 llegó a Claude Code como si fuera para él; lo paró la precondición y Claude Code. De Claude Code: (4) justificó que generar no se excluyera de duplicar porque «la guarda ya rechaza», un comprobar-aquí-actuar-allá contrario al invariante I2; (5) informó de 40 tests nuevos en la fase A cuando eran 32; (6) citó «condición 8» sin decir de qué objetivo; (7) puso el `gap` de la barra en un contenedor flex con un solo hijo, donde no separa nada, y hicieron falta varios intentos hasta llevarlo al `:host` del componente.
  SUITES: **solver 92, app 531, vitest 508 en 55 ficheros**, medidas en el cierre; app pasa de 496 a 531 y vitest de 482 en 52 a 508 en 55. e2e no corrido (`D-e2e-centro-minimo-rojo`, sede Fase 12).
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos en el cierre: INTACTOS. Todo M4 trabajó sobre copias en `/tmp`.
