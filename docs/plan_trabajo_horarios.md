...
- L754 — ### Sesión 170 — O-aceptación (H4), C-exportacion-bloques: **UNA ACTIVIDAD DE VARIOS TRAMOS APARECE EN TODOS LOS QUE OCUPA EN LA REJILLA, EL CSV Y LOS TRES PDF, Y EL ORÁCULO LO COMPRUEBA CON UNA MUTACIÓN QUE LO DEMUESTRA. `O-aceptación` 2 DE 4; SALDADA `D-proyeccion-sin-duracion`.**
...
- L3219 — ### Deuda consciente CERRADA (histórico)
- L3490 — ### Notas técnicas validadas en Fase 0
- L3503 — ### Notas técnicas validadas en Fase 6
- L3548 — ### Notas técnicas validadas en Fase 9
- L3557 — ### Notas técnicas validadas en Fase 11
- L3700 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3709 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3736 — ### Registro detallado de sesiones S10–S31
- L3745 — ## Señales globales de alerta
...
### Sesión 170 — O-aceptación (H4), C-exportacion-bloques: **UNA ACTIVIDAD DE VARIOS TRAMOS APARECE EN TODOS LOS QUE OCUPA EN LA REJILLA, EL CSV Y LOS TRES PDF, Y EL ORÁCULO LO COMPRUEBA CON UNA MUTACIÓN QUE LO DEMUESTRA. `O-aceptación` 2 DE 4; SALDADA `D-proyeccion-sin-duracion`.**
...
Última sesión registrada (previa): Sesión 169 — O-aceptación (H4), C-windows-generacion: **EL BUNDLE GENERA EN WINDOWS 11 CON CUENTA ESTÁNDAR: OPTIMAL Y 0 VIOLACIONES DURAS, Y EL NATIVO DE OR-TOOLS CARGA CON LAS DLL DE VISUAL C++ QUE TRAE EL PROPIO BUNDLE. `O-aceptación` 1 DE 4.** Es la primera vez que la aplicación genera un horario en Windows.
  TIPO Y RITUAL: DESARROLLO reducido, calcado de S156 y S163 — M0 + M2 de solo lectura + entrega y predicción en Linux + construcción en la máquina virtual + M4 con el usuario en la cuenta estándar + oráculo en Linux + M1. Sin M3: ningún cambio de producto.
  M0. Cambio `C-windows-generacion`, Objetivo `O-aceptación` (condición 2), Hito H4; el primero por la decisión E. R-invalidación: los guiones de empaquetado se usan tal cual (los rehará la Fase 12), y el centro sintético es un vehículo desechable, porque el versionado lo crea la condición 1. R-deuda: `D-proyeccion-sin-duracion` bloquea la 3, no la 2.
  DECISIÓN (arquitecto, por análisis). El centro se siembra en Linux y viaja como base sin horario: la condición 2 mide la generación en Windows, no el origen del centro, y sembrarlo por pantalla es la condición 4. Linux y Windows ejecutan el MISMO jar.
  MEDICIÓN (M2, HEAD `65222f0`, Claude Code, solo lectura). (1) `jniortools.dll`, única DLL del jar de nativos de Windows, importa `MSVCP140`, `VCRUNTIME140` y `VCRUNTIME140_1`; el jar no las trae y el runtime del bundle sí. El cargador de OR-Tools extrae la DLL a una carpeta temporal y la carga por ruta absoluta, así que no se podía dar por hecho, leyendo, que Windows las encontrara. (2) La generación no escribe ninguna línea de log y la interfaz no muestra `estadoSolver` ni `objetivo`: el oráculo sale de la API y de la base. (3) El bundle de la máquina virtual era de `d85c79b` (S163). (4) La siembra de S167 sobrevivía en /tmp. El primer `grep "DLL Name"` dio 0 coincidencias porque objdump imprime en español; Claude Code lo trató como dato (norma 2 de M-guion) y repitió con `LC_ALL=C`.
  ENTREGA Y PREDICCIÓN (Linux). `empaquetar-linux.sh --salida` desde `65222f0`, jar `3f4d7008…`; la entrega de S163 se movió a /tmp, porque el guion borra `build/`. Centro de S167 sembrado por REST con ese jar en modo servidor: un grupo, dos profesores y una actividad B de 2 tramos, con P1 en 28 DURA y 1 BLANDA. Base `centro-s169.db` (`bfa5bcef…`), sin horario, en `banco/` de la carpeta compartida. Predicción sobre una copia: OPTIMAL, objetivo 1.0, 0 violaciones, la BLANDA en L1 y el bloque en lunes 1–2, que es la única colocación factible y no depende de la máquina.
  CONSTRUCCIÓN (máquina virtual, `educhronos-admin`, procedimiento sin tocar). Huellas coincidentes; carpeta de 231.769.614 B, CUMPLE con 18,2 MB de margen, así que `O-instalación` no ha retrocedido; zip `133329D2…`; humo «NO DISCRIMINANTE» sobre base vacía, como se esperaba.
  M4 CON EL USUARIO (cuenta estándar `prueba`; los datos de S163 apartados como `Educhronos-antes-s169`). Ninguna de las tres DLL de Visual C++ existe en `System32`. Generación por pantalla en `/horario/1`: proyección OPTIMAL con objetivo 1.0 y diagnóstico con 0 violaciones y la BLANDA en L1, las dos con captura. Con la aplicación abierta, el proceso que escucha en el 8080 (`Educhronos.exe`) tiene cargadas las tres DLL desde `…\Educhronos\runtime\bin\` y `jniortools.dll` desde `%TEMP%\ortools-java…`: **el bundle se basta solo**, también para la condición 4. Cierre por «Salir».
  ORÁCULO (Claude Code, sobre copias en /tmp/educhronos-s169/f4). Base traída `1aba867a…`: un horario OPTIMAL y una sesión en lunes orden 1; esquema idéntico al de `centro-s169.db` y sus 20 tablas de configuración iguales en los dos sentidos (EXCEPT). Diagnóstico recalculado en Linux con el mismo jar: JSON idéntico al de Windows y al de la predicción. Log de Windows: un arranque sobre `C:\Users\prueba\AppData\Local\Educhronos\educhronos.db`, una parada por «Salir», 0 ERROR y la WARN conocida; la generación no deja línea.
  CRITERIO. **Condición 2 CUMPLIDA (S169). `O-aceptación` 1 de 4.** `C-windows-generacion` ✔ HECHO. H4 sigue abierto.
  OBSERVACIONES, SIN DEUDA NUEVA. (a) La rejilla del navegador pinta el bloque sólo en su tramo de inicio y deja vacío el segundo: primera medición en navegador de `D-proyeccion-sin-duracion` (condición 3), anotada en su ficha. (b) La cabecera muestra «Curso sin nombre», porque la base se sembró por REST sin nombrar el curso; no toca ninguna condición. (c) El humo de `empaquetar-windows.ps1` dice que la carga del nativo la cierra «la condicion 3, con el banco», que nunca generó: texto caducado de un guion que rehará la Fase 12 (precedente S156, sin deuda).
  DEUDA. No nace ni se salda ninguna. Higiene/Método sigue en 14 de 20.
  DEFECTOS DE INSTRUMENTO. (1) Del asistente: el guion de la fase 2 preveía que `empaquetar-linux.sh` mezclara o se negara ante una salida existente, no que borrara `build/`; Claude Code movió la entrega vieja por el espíritu de la orden, sin daño. (2) Del asistente: pidió `GET /api/horarios/{id}`, escrita de memoria cuando el M2 ya listaba las rutas (norma 3 de M-guion); da 404, y Claude Code usó `/proyeccion`. (3) Del asistente: afirmó, sin mirar el modelo, que la colocación no se podía predecir; en este centro es la única factible. (4) De Claude Code: una consulta sqlite con columnas supuestas, repetida tras leer el esquema. Ninguno costó una ronda: el disparador de §6 no salta.
  SUITES: sin cambio de código, no se corrieron: **solver 116, app 550, vitest 558 en 58 ficheros**; e2e 3 tests en 3 ficheros, con `centro-minimo` rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…` (md5), medidos en el M2, en la fase 2, en el oráculo y en el cierre: INTACTOS.
  MATERIAL SIN VERSIONAR. /tmp/educhronos-s169/ (M2, siembra, predicción y oráculo) puede no sobrevivir. `centro-s169.db` y lo traído de Windows quedan en la carpeta compartida (`banco/` y `medidas/s169/`); `entrega/` y `bundle/` son ya de `65222f0`. En la máquina virtual, la cuenta `prueba` conserva los datos de S163 en `%LOCALAPPDATA%\Educhronos-antes-s169`.
  LIMPIEZA (M1-bis): archivada S167 a `bitacora-sesiones.md`; degradada S168 a «Última sesión registrada (previa):»; S169 queda como única cabecera H3 viva. Condensación: ninguna; `O-aceptación` sigue abierto y no deja frente cerrado.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 0 y 19 entradas descuadradas a 0 y 0** sobre 34 y 68 encabezados —en HEAD (`65222f0`) los dos estaban a 0 con esos mismos 34 y 68: gestión no se descuadró porque sus cuatro añadidos van dentro de líneas existentes y «Cambios que agrupa» cambia tres líneas por tres, y el plan se descuadró al insertar la entrada de S169 y la nota del nativo de OR-Tools y retirar S167; el «antes» se midió importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación, tras escribir y antes de regenerar—. **Censo de tokens sospechosos: 38** sobre 311 tokens distintos en el corpus vivo, frente a **37** sobre 310 en HEAD (`65222f0`), medido corriendo el verificador sobre el árbol limpio antes de editar. (a) Entra en la lista `C-alcance-aceptacion`, de 2 a 1, y NO porque se retire una cita: su segunda aparición en HEAD era la entrada del ÍNDICE GENERADO que reproducía la cabecera de S168; al degradarse S168 el índice reproduce la de S169, y el token queda sólo en «Cambios que agrupa» de la ficha de `O-aceptación`. Es el mecanismo de `D-censo-r4-rescate-por-indice` en sentido inverso, como `C-alcance-disponibilidad` en el cierre de S165. (b) Token nuevo: `C-windows-generacion`, con 4 apariciones vivas —la fila de H4 en §2, la condición 2 y «Cambios que agrupa» de la ficha de `O-aceptación` en §3, y la entrada del índice del plan que reproduce la cabecera de S169—; no está en la lista. (c) Cambia de cuenta sin tocar la lista `O-aceptación`, de 26 a 28, por las notas de S169 en §1 y en la fila de H4; en el índice sigue con 2, porque las cabeceras de S168 y de S169 lo nombran dos veces cada una. Ningún otro token cambia de cuenta. (d) La entrada de sesión sale del corpus (36 líneas retiradas, con la S168 degradada), así que esta línea no rescata nada; dentro de ella entran `C-windows-generacion` y `O-instalación`. (e) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (f) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión. (g) Los tres censos coinciden en **S167** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S167 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 21 líneas de cuerpo; título idéntico). (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S168 degradada y S169 como única cabecera H3 viva, y el plan tiene exactamente una (`grep -c "^### Sesión"` da 1). (i) El diff de costura toca, en gestión, la frase de estado de §1, la fila de H4 en §2 —cinco barras antes y después, sin celda de más (`D-verificar-cierre-ciego-a-las-tablas`)—, la condición 2 y «Cambios que agrupa» de la ficha de `O-aceptación`, y no el índice; en el plan, el índice, el registro de progreso (entrada de S169, S168 degradada y S167 retirada), la crónica, la frase de ventana, la ficha de `D-proyeccion-sin-duracion` y dos notas de entorno —la de la máquina virtual y la nueva del nativo de OR-Tools—; en la bitácora, los dos censos y S167 al final. (j) HIGIENE/MÉTODO: con el filtro mecánico por la columna de sede de §4 (sede con «Higiene» o con el script de R4), **14 vivas**, las mismas que en HEAD: 22 filas con 8 tachadas. **El disparador por recuento NO está activo: 14 de 20.** R-deuda: `D-proyeccion-sin-duracion` bloquea la condición 3 de `O-aceptación` y se salda dentro de él; ninguna otra deuda viva bloquea.»
