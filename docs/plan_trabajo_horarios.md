...
### Sesión 172 — O-aceptación (H4), C-corrida-aceptacion: **LA CADENA ENTERA SE EJECUTA EN WINDOWS POR UN HUMANO. `O-aceptación` TERMINADO, 4 DE 4, CON SALVEDAD POR DECISIÓN DEL USUARIO.**
  TIPO Y RITUAL: DESARROLLO REDUCIDO, sin M3 porque no hubo cambio de producto (precedentes S169 y S171) — M0 + construcción (A) + preparación de Windows (B) + corrida guiada por el arquitecto (C) + oráculos y acta (D) + M1. Dirigida desde un solo chat.
  M0. Cambio `C-corrida-aceptacion`, Objetivo `O-aceptación` (condición 4, la última), Hito H4. R-invalidación: los guiones de empaquetado se usan tal cual, sin mejorarlos (los rehará la Fase 12). R-deuda: ninguna deuda viva bloquea. El usuario propuso aplazar la prueba de Windows («si es Java y va en Linux, irá en Windows»); se mantuvo con argumento medido: el riesgo vive en lo que Java no abstrae —el nativo de OR-Tools y sus DLL de Visual C++, `jpackage` sin construcción cruzada y dos bundles distintos, y las diferencias de sistema de las notas técnicas—.
  DECISIONES. (G) y (H) de la ficha de `O-aceptación`: Windows limpio = cuenta estándar nueva `prueba2` en la VM de S163 con la ausencia comprobada; actas en `docs/actas-aceptacion/`, capturas fuera del repo con su sha256; corrida aceptada con salvedad contra la recomendación del arquitecto. Automatizar la corrida se descartó: §1 la define como ejecución humana y el e2e es de la Fase 12.
  FASE A (Claude Code). Commit candidato `8d9a74a` (cierre de S171); sin código desde `4c07553`, así que cubren las suites de S170. Bancos intactos. `bundle/` de `65222f0` apartado a `bundle-65222f0-s169/`. Entrega construida: jar `7b65bd48…`, guardado en `/home/luis/educhronos-aceptacion/s172/`; carpeta prevista 231.769.576 B.
  FASE B (VM). `empaquetar-windows.ps1` con `-HuellaJar`: huellas OK, carpeta 231.770.158 B (condición 2 de `O-instalación` con 18,2 MB de margen), zip `76e363d8…`, humo NO DISCRIMINANTE como es de esperar. `prueba2` estándar, sin Java, Node, `JAVA_HOME` ni DLL de Visual C++ en `System32`, sin datos previos; el zip llega con la misma huella.
  FASE C (corrida). Pasos 1 a 5 como el guion; 4a LEN-3ºB #3 a V6, 4b ING-3ºA a L2, 4c TEC-3ºB a M3, horario final 2. Desviaciones: PDC creado antes que el tutor de 3ºA (tutor puesto a mano); P2 y P4 tecleados como «Profesor»; CSV abierto con el Bloc de notas (sin Excel en la VM; Excel verificado en S156); y en el paso 6 el ejecutor pulsó «Generar horario» en el curso nuevo tras ver el mensaje.
  FASE D. §7.2, §7.3 y §7.4 pasan enteros (siete rc=0). §7.5 falla tal como está escrito: el curso nuevo tiene `horario_generado` 1 y `sesion` 24 por esa generación; el duplicado es correcto (17 tablas sin diferencias, id 1, sin pines). Cuatro capturas con discrepancia: 3b (muestra 3ºA), 4a-1 (falta), 4a-2 (sin la celda tras F5) y 4d (sin proyección ni diagnóstico). Acta `f0cc718`, `docs/actas-aceptacion/acta-s172.md`, veredicto «PASA CON SALVEDAD, POR DECISIÓN DEL USUARIO», con la recomendación contraria del arquitecto escrita.
  CRITERIO. **Condición 4 CUMPLIDA CON SALVEDAD (S172). `O-aceptación` TERMINADO, 4 de 4**, en cinco sesiones (S168–S172) y cinco Cambios. H4 sigue abierto: queda la Fase 12 (CI), sin ficha. No queda ningún objetivo abierto.
...
Última sesión registrada (previa): Sesión 171 — O-aceptación (H4), C-guion-aceptacion: **EL GUION DE ACEPTACIÓN EXISTE CON SU CENTRO, Y UN HUMANO LO HA RECORRIDO ENTERO POR PANTALLA EN LINUX: LOS PASOS 2 A 6 PASAN SU ORÁCULO. `O-aceptación` 3 DE 4; NACE `D-vista-horario-estado-rancio`.**
