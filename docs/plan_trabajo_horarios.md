...
- L754 — ### Sesión 166 — O-disponibilidad (H2), C-dura-completa: **C-DURA-COMPLETA HECHO: EL SOLVER CUMPLE LA DISPONIBILIDAD EN TODOS LOS TRAMOS QUE OCUPA CADA SESIÓN Y EL 422 DEL PIN SOBRE DURA DICE POR QUÉ. O-DISPONIBILIDAD, 2 DE 7.**
...
- L3186 — ### Deuda consciente CERRADA (histórico)
- L3445 — ### Notas técnicas validadas en Fase 0
- L3458 — ### Notas técnicas validadas en Fase 6
- L3503 — ### Notas técnicas validadas en Fase 9
- L3512 — ### Notas técnicas validadas en Fase 11
- L3654 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3663 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3690 — ### Registro detallado de sesiones S10–S31
- L3699 — ## Señales globales de alerta
...
### Sesión 166 — O-disponibilidad (H2), C-dura-completa: **C-DURA-COMPLETA HECHO: EL SOLVER CUMPLE LA DISPONIBILIDAD EN TODOS LOS TRAMOS QUE OCUPA CADA SESIÓN Y EL 422 DEL PIN SOBRE DURA DICE POR QUÉ. O-DISPONIBILIDAD, 2 DE 7.**
...
Última sesión registrada (previa): Sesión 165 — O-disponibilidad (H2), C-dura-completa: **LA INDISPONIBILIDAD DEL PROFESOR PASA A SER REGLA DURA DEL VERIFICADOR POR TRAMO OCUPADO, Y LA PREVALIDACIÓN RECHAZA EL PIN SOBRE UN TRAMO DURA Y EL BLOQUE CON DISPONIBILIDAD. EL SOLVER QUEDA PENDIENTE: 0 DE 7.**
  TIPO Y RITUAL: DESARROLLO — M0 + M2 + fases 0, 1 y 3 (una línea cada una, abajo; la fase 2 del plan, el solver, no se hizo: ver CAMBIO DE PLAN) + M3 + M1. La sesión se dirigió desde DOS chats de arquitectura (ver DEFECTOS DE INSTRUMENTO).
  FASE 0 — línea base y medición de impacto, sin código de producción: supuesto de indexación de tramos, línea base del solver con hash del modelo y solves, e impacto en los datos reales de los dos bancos.
  FASE 1 — verificador (`INDISPONIBILIDAD_PROFESOR` y BLANDA por tramo ocupado), cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE` en la prevalidación, tests de endpoint de movimiento e intercambio, y comprobación sobre una copia del horario de s137 (0 violaciones de la regla nueva, `indispBlanda` 0).
  FASE 3 — prevalidación del pin sobre DURA (`PIN_SOBRE_TRAMO_DURA`), con `VerificadorSolucion.tramosOcupados` público y estático; y registro de las deudas nuevas.
  M0. Cambio `C-dura-completa`, Objetivo `O-disponibilidad`, Hito H2. Sin R-invalidación: `C-rejilla-disponibilidad` consume este backend y la aceptación no lo rehará. Ninguna deuda bloqueaba.
  MEDICIÓN (M2 y fase 0). M2, cuatro tests desechables en `MedicionS165Test`, 7 casos en verde, borrados después: T1 REPRODUCIDO —con la DURA en el segundo tramo de un bloque de 2, el solver arranca en L1 y ocupa L1 y L2; con la DURA en el primero, INFEASIBLE—; T2 REPRODUCIDO —una instancia sobre un tramo DURA da `violaciones=[]`; el control con dos instancias del mismo profesor da `SOLAPE_PROFESOR`—; T3 DETERMINADO —el pin sobre DURA acaba en `HorarioInfactibleException`, estado `INFEASIBLE`, tras un solve—; T4 REPRODUCIDO —BLANDA en el segundo tramo del bloque: penalización 0 y objetivo 0.0; en el primero, 1 y 1.0—. Fase 0: (0.2) SÍ, los índices de tramo son contiguos por día y crecen con `ordenEnDia` en los dos bancos y en los 44 fixtures, sin huecos ni duplicados; el recreo es una fila `es_lectivo=false` que `CatalogoMapper` excluye, y `ProblemaHorario` no lo valida. (0.3) El SHA-256 del `CpModelProto` es estable dentro de una JVM y cambia entre JVM en 13 de 44 fixtures con los mismos recuentos; una huella canónica sale idéntica en 3 JVM. Línea base en `/tmp/baseline-s166.txt`, fuera del repo: 6b, 6c y d13 con estado y objetivo idénticos en 3 corridas y menos de 0,2 s; `fusion-instituto-completo` con 600 s y semilla 42, factibilidad en 248,9 / 61,7 / 79,4 s y optimización al límite de 600 s con objetivo 230 / 217 / 224 y cota 3 / 1 / 1. (0.4) Los dos bancos: 0 DURA, 0 BLANDA, 0 pines y 0 actividades de más de un tramo; ninguno de los 23 `app/*.db` tiene DURA, BLANDA ni actividades de más de un tramo; margen mínimo por profesor 11 (PAU1, FIS4, ECO: demanda 19 de 30).
  CAMBIO DE PLAN. Cita: «Aprobada la fase 0. Cambio de plan: el solver NO se toca en este ciclo. Motivo: la interfaz no permite actividades de duración > 1, y con duración 1 el solver ya trata bien DURA y BLANDA (control T1 de S165, tests 6b/6c). El objetivo ahora es dejar el backend listo para que la interfaz pueda crear restricciones horarias con seguridad.» Se dejó de arreglar el solver para bloques (la fase 2 del plan) y en su lugar el verificador cuenta por tramo ocupado y la prevalidación rechaza el bloque con disponibilidad. La premisa se midió en el cierre y es FALSA: el formulario de actividad tiene «Duración (tramos)» editable con mínimo 1 y sin máximo (`actividad-form.html:30-35`, `actividad-form.ts:157` y `:404`), fijado por el test de `actividad-form.spec.ts:542`; el backend sólo exige ≥ 1 (`ActividadService.java:47`).
  CÓDIGO `836ef49` — `ReglaDura.INDISPONIBILIDAD_PROFESOR`; el verificador cuenta DURA y BLANDA por tramo ocupado (`contarPenalizacionIndisponibilidadBlanda` y `atribuirBlandas`); 13 tests en `VerificadorSolucionIndisponibilidadTest`.
  CÓDIGO `6a39f36` — cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE` en `PrevalidacionService`; 6 tests unitarios.
  CÓDIGO `d625508` — 3 tests de endpoint: mover y intercambiar hacia un tramo DURA dan 409 con `INDISPONIBILIDAD_PROFESOR`, y una indisponibilidad preexistente no bloquea un movimiento ajeno.
  CÓDIGO `c15f9af` — `PIN_SOBRE_TRAMO_DURA` en `PrevalidacionService`; `VerificadorSolucion.tramosOcupados` pasa a público y estático; 7 tests unitarios y 1 de endpoint.
  CÓDIGO `f9e59e2` — renumeración a S165 de referencias en comentarios, datos de test y documentos, sin cambio de comportamiento.
  CÓDIGO, en suma. Solver: `ReglaDura.INDISPONIBILIDAD_PROFESOR`; `VerificadorSolucion` cuenta DURA y BLANDA por tramo ocupado y expone `tramosOcupados`. App: cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE` (profesor con cualquier restricción que imparte una actividad de más de un tramo) y `PIN_SOBRE_TRAMO_DURA` (todos los tramos que ocuparía la sesión), los dos con 422 por la única llamada al solver de la app; la línea de comandos del solver no pasa por la prevalidación.
  M3. 21 mutantes en cuatro frentes, 21 cazados, cada uno de una línea con su diff comprobado antes de correr. Verificador (7): sólo inicio en DURA → `dura_bloqueConTramoInteriorVetado…` y `dura_bloqueQueCubreDosTramosVetados…`; sin deduplicar → `dura_dosRestriccionesIgualesDelMismoProfesorYTramo_cuentanUna`; sin mirar al profesor → `dura_coDocencia…`; sin saltar el bloque imposible → `dura_bloqueImposible…`; contando BLANDA como DURA → `dura_restriccionBlanda_noEsViolacionDura`; recuento y atribución BLANDA sólo en el inicio → `blanda_bloqueConTramoInteriorVetado_penalizaUno` y `blanda_bloqueQueCubreDosTramosVetados…`. Cortafuegos (5): sin filtro de duración → `restriccionConActividadDeUnTramo…` y `restriccionYBloqueDeProfesoresDistintos…` más 2 tests previos con DURA y duración 1; sin mirar quién imparte → `restriccionYBloqueDeProfesoresDistintos…`; sin mirar restricciones → `bloqueSinRestriccionesHorarias…` y el anterior; sólo DURA → `profesorConBlandaQueImparteUnBloque…`; detrás de S8 → `conCortafuegosYUnaS8…`. Movimiento (2): verificador sin la regla → los 3 tests de endpoint nuevos; veredicto por filtro en vez de por diferencia → `unaIndisponibilidadPreexistenteAjenaAlMovimientoNoLoRechaza` más 3 tests previos de violación preexistente. Pin (7): regla desactivada → 5 unitarios y el de endpoint; sólo el tramo del pin → `pinDeUnBloqueCuyoTramoInteriorEsDura…`; sin mirar al profesor → `pinEnCoDocencia…`; contando BLANDA → `pinSobreTramoBlanda…`; sin deduplicar → `pinSobreDuraRepetida…`; antes del cortafuegos → `pinDeUnBloqueCuyoTramoInteriorEsDura…`; bloque imposible sin tramo → `pinDeUnBloqueImposibleSobreDura…`. Restauración: cada mutante sobre una copia del original, restaurado con `cp` sin `-p` (mtime nueva, Maven recompila) y `diff` contra el original vacío («RESTAURADO»); después, suite completa en verde.
  CRITERIO. **0 de 7.** La 5 queda a medias: verificador y movimiento manual por tramo ocupado, hechos; el solver sigue vetando sólo el inicio (`D-indisp-solo-tramo-de-inicio`, bloqueante) y el cortafuegos cubre el hueco entre tanto. El cierre aceptó reescribir la condición (decisión F) y la revocó al medir la premisa: la interfaz sí crea bloques, y con la reescritura un centro con sesiones dobles y disponibilidad no podría generar. La 6, construida y sin contar: falta el cuerpo HTTP del 422 (`D-F8.6-ii-a`). El otro chat había dejado «0 de 7 plenas, 1 con alcance limitado»; ninguna regla del método lee un cumplimiento parcial.
  DEUDA. Nacen `D-indisp-solo-tramo-de-inicio` (técnica real, BLOQUEANTE, se salda en `C-dura-completa`), `D-ventanas-consecutivas-ciegas-a-bloques` (técnica real, sin objetivo asignado, no bloquea) y `D-modelo-no-determinista-entre-jvm` (técnica real, con sede en `D-generacion-no-reproducible`, no bloquea). No se salda ninguna. R-deuda: `D-indisp-solo-tramo-de-inicio` bloquea la condición 5 y se salda dentro del Cambio en curso, sin sesión propia; `D-F8.6-ii-a` queda como riesgo de la condición 6.
  DEFECTOS DE INSTRUMENTO. (1) La sesión se dirigió desde dos chats: las respuestas del M2 fueron a un chat distinto del que lo escribió, porque los ficheros del Project seguían en S164 y un segundo chat reabrió S165. Ese chat llamó sesión a cada fase (S166, fase 0) y Claude Code dedujo S167 para las siguientes, en commits, código y documentos. Se unificó en `f9e59e2` con `filter-branch` sobre los asuntos (árbol idéntico, verificado); los CUERPOS de los mensajes conservan referencias a S166 y a «S165 T1/T4», limitación aceptada. Lección: el número de sesión lo pone el usuario, y los ficheros del Project se actualizan tras cada push. (2) La regla «una fase por sesión» no es del método: una sesión tiene fases. (3) El cambio de plan de la fase 0 se apoyó en una premisa no medida («la interfaz no permite actividades de duración > 1»), y el cierre la dio por buena al reescribir la condición 5; se midió al final, era falsa y la reescritura se revocó. Lección: una premisa que decide alcance se mide antes de decidir (M2).
  SUITES: **solver 105, app 551, vitest 513 en 55 ficheros** (+13 del verificador, +17 de la app: 6 del cortafuegos, 3 de movimiento e intercambio, 7 unitarios del pin, 1 de endpoint). e2e no se corrió: 3 tests en 3 ficheros, `centro-minimo` rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos en este cierre: INTACTOS.
  LIMPIEZA (M1-bis): archivada S163 a `bitacora-sesiones.md`; degradada S164 a «Última sesión registrada (previa):»; S165 queda como única cabecera H3 viva. Condensación: no hay frente cerrado acumulado.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 22 y 19 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD (`a43a5e7`) los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo este cierre, en gestión al reescribir la condición 5 y la línea de estado y añadir las Decisiones (S165), y en el plan al insertar la entrada de S165 y archivar S163; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 40** sobre 303 tokens distintos en el corpus vivo, frente a **40** sobre 303 en HEAD, medido corriendo el verificador sobre un worktree de HEAD: sale `C-dura-completa` y entra `C-alcance-disponibilidad`. (a) Los tres tokens nuevos de la sesión, en el corpus vivo: `D-indisp-solo-tramo-de-inicio` con 5 apariciones —su ficha del plan, las fichas del plan de las otras dos deudas, la línea de estado de la ficha de `O-disponibilidad` y su fila de §4—, `D-ventanas-consecutivas-ciegas-a-bloques` con 2 —su ficha del plan y su fila de §4— y `D-modelo-no-determinista-entre-jvm` con 3 —su ficha del plan, la de `D-indisp-solo-tramo-de-inicio` y su fila de §4—; ninguno está en la lista. (b) `C-dura-completa` sale con 7 apariciones: la fila de H2 en §2, los Cambios de la ficha, la decisión (F), la fila de §4 de `D-indisp-solo-tramo-de-inicio` (dos) y su ficha del plan, que escribe este cierre, y la entrada del ÍNDICE GENERADO que reproduce la cabecera de S165, séptima reproducción medida de `D-censo-r4-rescate-por-indice`; sin ella tendría 6 y saldría igual. `C-alcance-disponibilidad` entra con 1, los Cambios de la ficha: perdió la aparición del índice al degradarse la cabecera de S164, que lo nombraba; es el mismo mecanismo en sentido inverso, y el token se aceptó sin acción en S164. (c) La línea de R4 no rescata nada: la entrada de sesión sale del corpus (38 líneas retiradas, con la S164 degradada). (d) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S163** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S163 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 17 líneas; título idéntico). (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en HEAD, porque las deudas nuevas no tienen esa sede; el filtro mecánico por la columna de sede de §4 da 18 filas, 5 tachadas, igual en HEAD y ahora; **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S164 degradada y S165 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda aguanta: ninguna deuda viva bloquea `O-disponibilidad`. **CORREGIDO en S166:** falso al escribirse. La línea DEUDA de esta misma entrada y la fila de §4 clasifican `D-indisp-solo-tramo-de-inicio` como BLOQUEANTE de la condición 5; la frase quedó de antes de revocar la decisión F.»
...
- **D-curso-sin-borrado** (S160, VIVA, MEJORA FUTURA, no bloqueante, sede objetivo de aceptación desde S163) — UN CURSO NO SE PUEDE
