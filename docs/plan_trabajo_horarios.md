...
- L754 — ### Sesión 169 — O-aceptación (H4), C-windows-generacion: **EL BUNDLE GENERA EN WINDOWS 11 CON CUENTA ESTÁNDAR: OPTIMAL Y 0 VIOLACIONES DURAS, Y EL NATIVO DE OR-TOOLS CARGA CON LAS DLL DE VISUAL C++ QUE TRAE EL PROPIO BUNDLE. `O-aceptación` 1 DE 4.** Es la primera vez que la aplicación genera un horario en Windows.
...
- L3218 — ### Deuda consciente CERRADA (histórico)
- L3477 — ### Notas técnicas validadas en Fase 0
- L3490 — ### Notas técnicas validadas en Fase 6
- L3535 — ### Notas técnicas validadas en Fase 9
- L3544 — ### Notas técnicas validadas en Fase 11
- L3687 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3696 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3723 — ### Registro detallado de sesiones S10–S31
- L3732 — ## Señales globales de alerta
...
### Sesión 169 — O-aceptación (H4), C-windows-generacion: **EL BUNDLE GENERA EN WINDOWS 11 CON CUENTA ESTÁNDAR: OPTIMAL Y 0 VIOLACIONES DURAS, Y EL NATIVO DE OR-TOOLS CARGA CON LAS DLL DE VISUAL C++ QUE TRAE EL PROPIO BUNDLE. `O-aceptación` 1 DE 4.** Es la primera vez que la aplicación genera un horario en Windows.
...
Última sesión registrada (previa): Sesión 168 — O-aceptación (H4), C-alcance-aceptacion: **H4 ABRE SU TERCER OBJETIVO, `O-aceptación`, CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 4.** La cadena entera de §1 tenía nombre pero no tenía hito ni ficha. La medición encuentra que los pasos 2, 3 y 4 no se han ejecutado nunca en el bundle de Windows, y que el oráculo de exportación da OK sobre un PDF que pinta una clase de dos horas como si fuera de una.
  TIPO Y RITUAL: APERTURA DE OBJETIVO — M0 + M2 de solo lectura + alta documental con revisión del diff + M1. Sin código de la aplicación. Precedente: `C-alcance-disponibilidad` (S164). Dirigida desde un solo chat.
  FASE 1 — M2 de solo lectura (Claude Code) en cuatro puntos: centro mínimo frente al paso 2, oráculos, pasos vistos en Windows y tres deudas.
  FASE 2 — decisiones A a F.
  FASE 3 — alta documental con revisión del diff y una corrección antes del commit.
  M0. Cambio `C-alcance-aceptacion`, Objetivo `O-aceptación`, Hito H4. La aceptación no tenía hito: se cuelga de H4 (usuario, a propuesta del arquitecto) por su valor, la condición de entrega, y porque corre sobre el bundle; un H5 contradiría la definición de hito de §2, porque no da al usuario ninguna capacidad nueva. El nombre de H4 no la describe, y su fila lo dice. R-invalidación: la Fase 12 rehará la construcción y es la dueña del e2e, así que la aceptación usa los guiones de empaquetado tal como están. R-deuda: ninguna fila de §4 bloqueaba al abrir. Higiene no compite: 13 de 20.
  MEDICIÓN (M2, HEAD `4d3b39c`, Claude Code; guiones y salidas en /tmp/educhronos-s168/, sin versionar). (1) El e2e crea por la interfaz 5 de los 10 elementos del paso 2; faltan desdobles, agrupamientos, PDC, tutores y disponibilidad. No hay ningún centro sintético versionado: los de S166 y S167 viven en /tmp y el de S120 sólo en la bitácora. (2) Los oráculos son genéricos en los datos: `VerificadorSolucion`, por `GET /api/horarios/{id}/diagnostico`, y `oraculo-exportacion.py` y `verificar-leyenda-pdf.py`, que sacan lo esperado de la base. Los dos scripts fijan la maqueta del PDF (5 días, un recreo), y el primero sólo mira el tramo de inicio (`:72-74`). (3) En el bundle de Windows se han visto los pasos 1, 5 y 6, y nunca el 2, el 3 ni el 4: allí no se ha generado nunca un horario y el nativo de OR-Tools para Windows no se ha cargado nunca (en Linux sí, S152). (4) Por HTTP: con una restricción DURA, incluso un PUT de jornada idéntico da 409, también en el curso duplicado. Con un bloque de 2 tramos, el CSV sale con una fila, el PDF pinta sólo el tramo de inicio y el oráculo da OK sobre los dos ficheros. Por lectura: con una sola pestaña, el paso 6 no pasa por el estado de `D-curso-pestanas-desfasadas`; con dos, sí (deducido).
  DECISIONES (usuario, a propuesta del arquitecto), en la ficha: (A) centro de aceptación nuevo, pequeño y versionado. (B) El guion incluye una actividad de 2 tramos y `D-proyeccion-sin-duracion` pasa a bloquear; se descarta declararlo como limitación. (C) Los oráculos se usan tal cual, y el de exportación aprende la duración. (D) Tres deudas quedan fuera del criterio. (E) El primer Cambio es generar en el bundle de Windows. (F) Un fallo repite la cadena entera, y la corrida válida deja acta.
  CRITERIO. `O-aceptación` escrito en §3 con cuatro condiciones: guion versionado, generación en Windows, bloques en la exportación y cadena completa. **0 de 4.** `C-alcance-aceptacion` HECHO.
  DOCUMENTACIÓN `b912d22` — ficha de `O-aceptación`; nota de S168 en §1, con la del paso 2 que el cierre de S167 no escribió; fila y ESTADO de H4 en §2; sede `O-aceptación` para tres deudas; ficha nueva de `D-proyeccion-sin-duracion` en el plan. De paso, tres filas de la tabla «Mejora futura» de §4 tenían una celda de más y no mostraban su nota al renderizar; se corrigen.
  DEUDA. `D-proyeccion-sin-duracion` pasa a BLOQUEANTE (condición 3) y recibe ficha en el plan. `D-jornada-congelada-por-disponibilidad` y `D-curso-pestanas-desfasadas` suman su medición de S168 y quedan fuera del criterio, igual que `D-curso-sin-borrado`. Nace `D-verificar-cierre-ciego-a-las-tablas` (técnica real, sede Higiene/Método, no bloquea). `D-deuda-sin-sede-en-el-plan` suma `D-jornada-msg409` y `D-jornada-asimetria`. No se salda ninguna.
  DEFECTOS DE INSTRUMENTO. (1) Del asistente: el texto del alta contaba como hallazgos nuevos dos deudas vivas desde S107 (`D-jornada-msg409` y `D-jornada-asimetria`); se corrigió antes del commit. (2) Del asistente: el guion de la fase 3 decía «PARA» ante un ancla repetida y, a la vez, daba la regla para desambiguarla. Claude Code siguió la regla y lo informó, sin daño. (3) Del asistente: las decisiones del objetivo se numeraron D1–D6, que casan con el patrón de las deudas antiguas D1–D6 y engordaban su censo (D4 de 6 a 13). Renombradas a (A)–(F) después del cierre, en el commit de corrección; los precedentes usan letras.
  SUITES: sin cambio de código, no se corrieron: **solver 116, app 550, vitest 558 en 58 ficheros**; e2e 3 tests en 3 ficheros, con `centro-minimo` rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…` (md5), medidos al inicio y al final del M2 y otra vez en el cierre: INTACTOS.
  LIMPIEZA (M1-bis): archivada S166 a `bitacora-sesiones.md`; degradada S167 a «Última sesión registrada (previa):»; S168 queda como única cabecera H3 viva. Condensación: ninguna en esta sesión.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 13 y 19 entradas descuadradas a 0 y 0** sobre 34 y 68 encabezados —en `b912d22` ya estaban a 13 y 9, con 33 entradas para 34 encabezados en gestión y 68 para 68 en el plan: la fase 3 descuadró gestión al insertar la ficha de `O-aceptación`, cuyo encabezado quedó fuera del índice, y el plan al insertar la ficha de `D-proyeccion-sin-duracion`; este cierre sumó en el plan la entrada de S168, la retirada de S166 y la ficha nueva. En `4d3b39c` los dos estaban a 0 con 33 y 68. El «antes» se midió importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 37** sobre 310 tokens distintos en el corpus vivo, frente a **38** sobre 309 en `b912d22`, medido corriendo el verificador sobre el árbol limpio antes de editar, y 40 sobre 307 en `4d3b39c`, el cierre de S167. (a) Sale de la lista `C-alcance-aceptacion`, de 1 a 2, y NO por una cita nueva: la segunda aparición es la entrada del ÍNDICE GENERADO que reproduce la cabecera de S168, el mecanismo de `D-censo-r4-rescate-por-indice`; sin ella seguiría en la lista. Entre `4d3b39c` y `b912d22` ya habían salido `D-jornada-msg409`, `D-jornada-asimetria` y `D-proyeccion-sin-duracion` (de 1 a 3, 2 y 5 con el alta) y había entrado `C-alcance-aceptacion`. (b) Tokens nuevos de la sesión: `O-aceptación`, con 26 apariciones vivas, 3 de ellas del índice —dos en la entrada que reproduce la cabecera de S168 y una en la del encabezado de su ficha de §3—; `C-alcance-aceptacion`, con 2, una del índice; y `D-verificar-cierre-ciego-a-las-tablas`, con 2 —su ficha del plan y su fila de §4—. (c) Cambian de cuenta sin tocar la lista, respecto de `b912d22`: `D-curso-sin-borrado` de 5 a 7, `D-curso-sin-renombrar`, `D-bundle-sobre-techo-550` y `D-gh6-tutor-contradictorio` de 2 a 3, y `D-configuracion-monolitica` de 6 a 7, porque los nombra la ficha nueva, igual que a `D-guion-pkill-casa-su-propio-envoltorio`, de 4 a 5; `D-jornada-msg409` de 3 a 5, `D-jornada-asimetria` de 2 a 4, y `D-jornada-congelada-por-disponibilidad` y `D-proyeccion-sin-duracion` de 5 a 7, por las instancias de S168 de `D-deuda-sin-sede-en-el-plan` en su ficha y en su fila; `C-rejilla-disponibilidad` de 7 a 6 y `O-disponibilidad` de 40 a 39, porque el índice deja de reproducir la cabecera de S167. Respecto de `4d3b39c`, las etiquetas (D1) a (D6) de las decisiones de `O-aceptación` casan con el patrón `Dnn` y suben la cuenta de las deudas homónimas D1 a D6 (D4 de 6 a 13), sin tocar la lista. **CORREGIDO tras el cierre:** decisiones renombradas a (A)–(F); D1–D6 vuelven a su cuenta de 4d3b39c. (d) La entrada de sesión sale del corpus (40 líneas retiradas, con la S167 degradada), así que esta línea no rescata nada. (e) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (f) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión. (g) Los tres censos coinciden en **S166** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S166 en la bitácora es idéntico al que sale del plan (diff vacío, 23 líneas; título idéntico). (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S167 degradada y S168 como única cabecera H3 viva, y el plan tiene exactamente una. (i) El diff de costura toca, en gestión, el índice y tres filas de §4 —la nueva de `D-verificar-cierre-ciego-a-las-tablas`, la instancia de `D-deuda-sin-sede-en-el-plan` y la tachada de `D-guion-pkill-casa-su-propio-envoltorio`, cuya barra sin escapar dentro de código en línea partía la celda y se escapa aquí (la regla del M1 exige que la tabla tocada quede con tantas celdas por fila como su cabecera)—; en el plan, el índice, el registro de progreso (entrada de S168, S167 degradada y S166 retirada), la crónica, la frase de ventana y dos fichas de deuda —la nueva y la instancia—; en la bitácora, los dos censos y S166 al final. (j) HIGIENE/MÉTODO: con el filtro mecánico por la columna de sede de §4 (sede con «Higiene» o con el script de R4), **14 vivas**, 13 en `b912d22`: entra `D-verificar-cierre-ciego-a-las-tablas`; son 22 filas con 8 tachadas. El mismo filtro sobre `60a33d9` da 21 filas y 8 tachadas frente a las 18 y 5 que declaró S166, con las mismas 13 vivas: el total no se reproduce y las vivas sí. **El disparador por recuento NO está activo: 14 de 20.** R-deuda: `D-proyeccion-sin-duracion` bloquea la condición 3 de `O-aceptación` y se salda dentro de él; ninguna otra deuda viva bloquea.»
...
- **D-proyeccion-sin-duracion** (S148, VIVA, TÉCNICA REAL, BLOQUEANTE desde S168, sede `O-aceptación`, condición 3) — LA
  ...
  y se salda dentro de él; el arreglo incluye que el oráculo compruebe todos los tramos que cubre cada sesión. **Medido en S169 (bundle de `65222f0`, Windows 11):** la rejilla del navegador pinta el bloque B sólo en lunes 1 y deja vacío lunes 2 (captura del M4). Sustituye a «la rejilla en navegador no se midió».
