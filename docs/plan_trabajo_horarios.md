...
- L754 — ### Sesión 165 — O-disponibilidad (H2), C-dura-completa: **LA INDISPONIBILIDAD DEL PROFESOR PASA A SER REGLA DURA DEL VERIFICADOR POR TRAMO OCUPADO, Y LA PREVALIDACIÓN RECHAZA EL PIN SOBRE UN TRAMO DURA Y EL BLOQUE CON DISPONIBILIDAD. LA CONDICIÓN 5 SE REESCRIBE Y SE CUMPLE: 1 DE 7.**
...
### Sesión 165 — O-disponibilidad (H2), C-dura-completa: **LA INDISPONIBILIDAD DEL PROFESOR PASA A SER REGLA DURA DEL VERIFICADOR POR TRAMO OCUPADO, Y LA PREVALIDACIÓN RECHAZA EL PIN SOBRE UN TRAMO DURA Y EL BLOQUE CON DISPONIBILIDAD. LA CONDICIÓN 5 SE REESCRIBE Y SE CUMPLE: 1 DE 7.**
  ...
  CRITERIO. Condición 5 REESCRITA y cumplida (decisión del usuario, F): el bloque con disponibilidad se rechaza en vez de resolverse; motivo: el propósito de la decisión (D) de S164 queda satisfecho y ningún banco tiene actividades de más de un tramo (0 de 219). Condición 6 construida y no contada: falta el cuerpo HTTP del 422 (`D-F8.6-ii-a`). **1 de 7.** El otro chat había dejado «0 de 7 plenas, 1 con alcance limitado»; el cierre lo sustituye porque ninguna regla del método lee un cumplimiento parcial.
  DEUDA. Nacen `D-indisp-solo-tramo-de-inicio` (LIMITACIÓN CONOCIDA desde el cierre, sin objetivo), `D-ventanas-consecutivas-ciegas-a-bloques` (técnica real, sin objetivo asignado, no bloquea) y `D-modelo-no-determinista-entre-jvm` (técnica real, con sede en `D-generacion-no-reproducible`, no bloquea). No se salda ninguna. R-deuda: ninguna bloquea `O-disponibilidad`; `D-F8.6-ii-a` queda como riesgo de la condición 6.
  DEFECTOS DE INSTRUMENTO. (1) La sesión se dirigió desde dos chats: las respuestas del M2 fueron a un chat distinto del que lo escribió, porque los ficheros del Project seguían en S164 y un segundo chat reabrió S165. Ese chat llamó sesión a cada fase (S166, fase 0) y Claude Code dedujo S167 para las siguientes, en commits, código y documentos. Se unificó en `f9e59e2` con `filter-branch` sobre los asuntos (árbol idéntico, verificado); los CUERPOS de los mensajes conservan referencias a S166 y a «S165 T1/T4», limitación aceptada. Lección: el número de sesión lo pone el usuario, y los ficheros del Project se actualizan tras cada push. (2) La regla «una fase por sesión» no es del método: una sesión tiene fases.
