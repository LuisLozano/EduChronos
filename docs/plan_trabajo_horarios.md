...
- L754 — ### Sesión 167 — O-disponibilidad (H2), C-rejilla-disponibilidad: **C-REJILLA-DISPONIBILIDAD HECHO: EL CENTRO MARCA LA DISPONIBILIDAD DE CADA PROFESOR EN UNA REJILLA Y EL HORARIO LA CUMPLE. O-DISPONIBILIDAD TERMINADO, 7 DE 7; H2 CERRADO DE NUEVO. LOS MENSAJES DE ERROR DEL BACKEND LLEGAN A LA INTERFAZ.**
- L837 — ### Bloques de Fase 2
- L845 — ### Bloques de Fase 5
- L868 — ### Bloques de Fase 6
- L913 — ### Bloques de Fase 8
- L1125 — ### Fases completadas
- L1229 — ### Cierre del modelo — Sesión 8
- L1277 — ### Decisiones permanentes (no reabrir sin razón de peso)
- L1323 — ### Método de trabajo (procedimiento vigente)
- L1356 — ### Deuda consciente VIVA
- L3208 — ### Deuda consciente CERRADA (histórico)
- L3467 — ### Notas técnicas validadas en Fase 0
- L3480 — ### Notas técnicas validadas en Fase 6
- L3525 — ### Notas técnicas validadas en Fase 9
- L3534 — ### Notas técnicas validadas en Fase 11
- L3676 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3685 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3712 — ### Registro detallado de sesiones S10–S31
- L3721 — ## Señales globales de alerta
...
### Sesión 167 — O-disponibilidad (H2), C-rejilla-disponibilidad: **C-REJILLA-DISPONIBILIDAD HECHO: EL CENTRO MARCA LA DISPONIBILIDAD DE CADA PROFESOR EN UNA REJILLA Y EL HORARIO LA CUMPLE. O-DISPONIBILIDAD TERMINADO, 7 DE 7; H2 CERRADO DE NUEVO. LOS MENSAJES DE ERROR DEL BACKEND LLEGAN A LA INTERFAZ.**
...
Última sesión registrada (previa): Sesión 166 — O-disponibilidad (H2), C-dura-completa: **C-DURA-COMPLETA HECHO: EL SOLVER CUMPLE LA DISPONIBILIDAD EN TODOS LOS TRAMOS QUE OCUPA CADA SESIÓN Y EL 422 DEL PIN SOBRE DURA DICE POR QUÉ. O-DISPONIBILIDAD, 2 DE 7.**
  TIPO Y RITUAL: DESARROLLO — M0 + M2 + fases 1 a 5 + M3 + M4 + M1, dirigida desde un solo chat.
  FASE 1 — medición de la condición 6, sin código.
  FASE 2 — 422 de prevalidación con cuerpo propio y motivo en la vista.
  FASE 3 — puerta de regresión: huella, predicción, tests en rojo y lectura de la invariante de tramos.
  FASE 4 — invariante en `ProblemaHorario` y arreglo del modelo.
  FASE 5 — retirada del cortafuegos y verificación de punta a punta.
  M0. Cambio `C-dura-completa`, Objetivo `O-disponibilidad`, Hito H2. Sin R-invalidación: `C-rejilla-disponibilidad` consume este backend y `D-ventanas-consecutivas-ciegas-a-bloques`, que tocará `ModeloCpSat`, no tiene objetivo asignado. R-deuda: `D-indisp-solo-tramo-de-inicio` bloquea la 5 y se salda dentro del Cambio; la 6 se midió primero porque su resultado decidía el alcance (lección (3) de S165).
  MEDICIÓN (fase 1, HTTP real sobre copia de s137: pin de AMO-1FPB #3 en X1 y DURA de PAU2 en X1). El 422 de `POST /api/horarios` salía por `ResponseStatusException` (`HorarioController:93-94`) con el cuerpo genérico de Spring, sin regla, profesor ni tramo, en 0,116 s y sin solver. El motivo sólo llegaba por `GET /api/prevalidacion` al diálogo previo; tras «Generar de todos modos» la vista decía «Esta configuración no tiene solución. Revisa el catálogo.», confirmado en navegador. El test de endpoint de S165 asertaba `reason()`, el canal mudo de `D-F8.6-ii-a`: no contar la 6 en S165 fue correcto. `D-F8.6-ii-a` pasó a bloquear la 6.
  DECISIONES (usuario, a propuesta del arquitecto). (H) No se reescribe la condición 6, que sería repetir F: el controlador construye el cuerpo del 422 de prevalidación reutilizando `respuestaDeFallo`/`FalloGeneracionDTO` de S118, con causa nueva `PREVALIDACION_FALLIDA`; no se paga la clave global, `D-F8.6-ii-a` sigue viva. (I) La invariante de indexación se valida en el constructor de `ProblemaHorario` (opción A): tramos ordenados por (día, ordenEnDia) y sin dos con el mismo par; se admiten huecos en ordenEnDia. El modelo calcula los tramos que cubre un inicio como t..t+d−1, la misma definición que los intervalos del no-solape. (J) Se retira el cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE`, en un commit posterior al arreglo del solver, como exigía su deuda.
  PUERTA (fase 3). La huella canónica se calcula en dos variantes, `construir()` y `construirConObjetivo()`: la de S165 sólo cubría la primera, que no contiene el término BLANDA. Estable en dos JVM y sensible a mutantes en los 5 fixtures con restricciones. Predicción previa: ningún fixture combina bloque y restricción del mismo profesor, así que la huella debía quedar idéntica en los 88 (44 × 2), y quedó idéntica. T1 y T4 pasaban con el solver viejo por azar y se reforzaron con un desempate. La invariante ya era un supuesto del modelo (intervalos [t, t+d) en los cuatro no-solapes) sin validar, y nadie rechazaba dos tramos con el mismo (día, ordenEnDia).
  CÓDIGO `7f27985` — el 422 de la prevalidación lleva su motivo en el cuerpo (`PREVALIDACION_FALLIDA`); los tres tests del endpoint asertan el cuerpo, no `reason()`; +1 caso con dos errores.
  CÓDIGO `e3415c4` — `mensajeGeneracion` muestra el motivo; el diálogo deja de decir que el servidor «no detallará la causa».
  CÓDIGO `3738a23` — `ProblemaHorario` valida el orden de los tramos; +4 tests de dominio y +1 del cargador JSON.
  CÓDIGO `1638ee0` — DURA: se prohíben los inicios v−k (k en 0..d−1) que sean válidos; BLANDA: un literal por tramo cubierto sobre el conjunto de inicios que lo cubren. Con d = 1, construcción idéntica. `SolverHorarioIndisponibilidadBloqueTest`, 6 tests que fallaban antes del arreglo. El modelo no usa código del verificador (decisión G).
  CÓDIGO `206b754` — retirada del cortafuegos: 4 tests borrados, 2 adaptados y 1 invertido como guarda (profesor con DURA y BLANDA que imparte un bloque: la prevalidación no da ERROR).
  M3. 12 mutantes, 12 cazados: 4 de la fase 2 (rama del controlador, mensaje nulo, sólo el primer error, frontend que ignora el cuerpo), 7 de la fase 4 (DURA y BLANDA sólo en k = 0, DURA sin el propio inicio, BLANDA una vez por inicio, y tres del constructor) y 1 de punta a punta en la fase 5 (el modelo de `3738a23`).
  M4. Fases 1 y 2 en navegador, antes y después. Fase 5 por HTTP real sobre base vacía: centro mínimo por REST, bloque de 2 tramos con DURA en L3 y BLANDA en L1 del lunes, y DURA en el resto de inicios. Tabla de costes escrita antes de generar. Modelo nuevo: OPTIMAL, objetivo 1.0, bloque en L1-L2, diagnóstico con 0 violaciones e `indispBlanda` 1. Modelo viejo: bloque en L2-L3, `INDISPONIBILIDAD_PROFESOR` en L3.
  CRITERIO. **2 de 7: se cumplen la 5 y la 6.** `C-dura-completa` HECHO. Queda `C-rejilla-disponibilidad` (condiciones 1 a 4 y 7).
  DEUDA. Se salda `D-indisp-solo-tramo-de-inicio`. `D-F8.6-ii-a` suma la instancia de S166, esquivada y viva. Nace `D-post-generacion-sin-sesiones` (técnica real, sin objetivo asignado, no bloquea). Precisiones a `D-modelo-no-determinista-entre-jvm` y `D-ventanas-consecutivas-ciegas-a-bloques`.
  OBSERVACIONES, sin deuda. El mensaje empieza por «Pre-validacion fallida:», sin tilde, y ahora lo ve el usuario; no cambia el criterio (R-terminado). En el diálogo de confirmación, la columna «1 / 0» se parte por falta de ancho; ya pasaba antes. Crear un grupo por la API no crea su subgrupo `-Completo`; el fixture de repositorio sí lo hace, y no se ha medido si es un hueco de la interfaz. La fase 2 usó `mvn -pl app test` para mostrar un rojo: allí era válido porque el solver no había cambiado, y los guiones de las fases 3 a 5 lo prohibieron.
  SUITES: **solver 116, app 548, vitest 516 en 55 ficheros** (solver +11: 4 del constructor, 1 del cargador y 6 del bloque; app +1 en la fase 2 y −4 netos en la fase 5; vitest +3). e2e no se corrió: 3 tests en 3 ficheros, `centro-minimo` rojo preexistente.
  LIMPIEZA (M1-bis): archivada S164 a `bitacora-sesiones.md`; degradada S165 a «Última sesión registrada (previa):»; S166 queda como única cabecera H3 viva. Condensación: no hay frente cerrado acumulado.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre (código de salida 0, capturado inmediatamente tras el comando), con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 13 y 19 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD (`0baba1a`) los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo este cierre, en gestión al reescribir los Cambios y añadir las Decisiones (S166) y la línea de estado, y en el plan al insertar la entrada de S166, archivar S164 y añadir las fichas; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo sobre una copia con el bloque de índice de HEAD y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 40** sobre 304 tokens distintos en el corpus vivo, frente a **40** sobre 303 en HEAD, medido corriendo el verificador sobre el árbol limpio en `206b754` antes de editar: la lista es idéntica. (a) El token nuevo de la sesión, `D-post-generacion-sin-sesiones`, entra con 2 apariciones —su ficha del plan y su fila de §4— y no está en la lista. (b) Cambian de cuenta sin tocar la lista: `C-dura-completa` de 7 a 10 —la celda de H2, la fila tachada de §4 y la línea CERRADA de su ficha del plan; la entrada del ÍNDICE GENERADO cambia la cabecera de S165 por la de S166 y se queda igual—, `C-rejilla-disponibilidad` de 2 a 4 —la celda de H2 y el ESTADO en S166—, `O-disponibilidad` de 31 a 33 —la celda de H2 y la ficha nueva—, `D-indisp-solo-tramo-de-inicio` de 5 a 6 —la precisión de `D-modelo-no-determinista-entre-jvm`— y `D-F8.6-ii-a` de 31 a 32 —las Decisiones (S166); la instancia de S166 de su ficha no nombra el token—. (c) La línea de R4 no rescata nada: la entrada de sesión sale del corpus (48 líneas retiradas, con la S165 degradada). (d) Extinción respecto de HEAD: **NINGÚN token cae a 0**. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S164** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S164 en la bitácora es idéntico al que sale del plan en HEAD (diff vacío, 13 líneas; título idéntico). (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en HEAD, porque la deuda nueva no tiene esa sede; el filtro mecánico por la columna de sede de §4 da 18 filas, 5 tachadas, igual en HEAD y ahora; **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S165 degradada y S166 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda: se salda `D-indisp-solo-tramo-de-inicio`, la única fila de §4 que bloqueaba `O-disponibilidad`; `D-F8.6-ii-a` queda esquivada en la 6 y su fila dice que no bloquea. Ninguna deuda viva bloquea `O-disponibilidad`.»
...
- **D-deuda-sin-sede-en-el-plan** (S132, VIVA, DE MÉTODO, no bloqueante) — CINCO DEUDAS NACIDAS EN S130 Y S131
  ...
  **INSTANCIA de S158:** `D-horario-irreversible` (S111) tiene fila en §4 y ninguna entrada propia en el plan;
  ...
  crea la entrada en S158 (R-deuda).
