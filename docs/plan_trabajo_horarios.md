...
- L3169 — ### Deuda consciente CERRADA (histórico)
- L3426 — ### Notas técnicas validadas en Fase 0
- L3439 — ### Notas técnicas validadas en Fase 6
- L3484 — ### Notas técnicas validadas en Fase 9
- L3493 — ### Notas técnicas validadas en Fase 11
- L3635 — ### Por qué OR-Tools sobre Timefold (no reabrir)
- L3644 — ### Hallazgos del análisis de PDFs (datos reales del centro)
- L3671 — ### Registro detallado de sesiones S10–S31
- L3680 — ## Señales globales de alerta
...
### Sesión 164 — O-disponibilidad (H2), C-alcance-disponibilidad: **H2 ABRE `O-disponibilidad` CON EL CRITERIO ESCRITO SOBRE MEDICIÓN: 0 DE 7.** La medición encuentra que la DURA sólo veta el tramo de inicio de cada sesión y que el verificador no la comprueba; su arreglo entra en el criterio. Se descarta inferir la disponibilidad de los horarios del centro.
...
Última sesión registrada (previa): Sesión 163 — O-curso (H4), C-windows-curso: **LAS CONDICIONES 1 A 5 SE CUMPLEN EN EL BUNDLE, EN WINDOWS 11 CON CUENTA ESTÁNDAR, CONSTRUIDO EN UNA MÁQUINA VIRTUAL NUEVA: `O-curso` TERMINADO, 9 DE 9.**
  TIPO Y RITUAL: DESARROLLO reducido, calcado de S156 — M0 + M2 de solo lectura + construcción + M4 con el usuario en la máquina virtual + oráculos en Linux + M2 de solo lectura sobre la propuesta del nombre + M1. Sin M3: ningún cambio de producto.
  M0. Cambio `C-windows-curso`, Objetivo `O-curso`, Hito H4; sin R-invalidación: la Fase 12 automatizará la construcción, no esta verificación; la guarda de solo lectura es genérica ante `O-disponibilidad`; la aceptación volverá a verificar, no rehará. Ninguna deuda bloquea.
  DECISIONES. Del usuario: se construye y se prueba en una máquina virtual nueva (VirtualBox, Windows 11 Pro), no en la de S156, que sigue disponible pero es de peor acceso; así se prueba además el procedimiento en una máquina nueva, que pasa a ser la de construcción habitual. El administrador construye y la cuenta estándar `prueba` prueba. Del arquitecto: la edición Pro vale, porque la condición 9 no fija edición y los diálogos de seguridad eran de `O-instalación`.
  MEDICIÓN (M2, HEAD `d85c79b`, Claude Code, solo lectura). (1) El JDK viaja en la entrega (Temurin 17.0.20.1, por huella) y el `.ps1` no lo busca en el PATH: en el invitado no se instala nada. (2) La prueba de humo escribe `%LOCALAPPDATA%\Educhronos` y su log en la cuenta que construye. (3) No hay guion versionado para las condiciones 1 y 3: se reutiliza la consulta `attach` + `EXCEPT` de `DuplicadorCursoTest`. (4) El md5 de referencia del archivado no es el del banco, porque el arranque añade la tabla `curso`. (5) El banco no tiene `curso`: entra por la vía de la condición 6. Hallazgo de texto: `D-ps1-palanca-de-linux`.
  CONSTRUCCIÓN. Entrega de Linux en la carpeta compartida; en el invitado, `empaquetar-windows.ps1` con `-HuellaJar` y prueba de humo, sin tocar el procedimiento ni los guiones: arranque OK en `127.0.0.1:8080`, parada limpia, carpeta de 231.761.706 B (46.221 B más que en S154), jar `d575594f…`, zip `6a2a50c6…`. Solver «NO DISCRIMINANTE» (base vacía): en esta máquina NO está verificado que cargue el nativo de OR-Tools, y ninguna condición de `O-curso` genera horario.
  M4 CON EL USUARIO (cuenta estándar, banco copiado como `educhronos.db`). C1 duplicar sin nombre: pide `2025/2026`, rechaza repetirlo y crea `2026/2027`. C2 «Salir» y copia. C3 relanzar abre `2026/2027`; se edita y se borra `Biol-3ºA`. C4 cambio en caliente al archivado, «Solo lectura» y escritura rechazada con su mensaje. C5 `/horario/1` con las tres vistas y las cuatro descargas. C6 segundo lanzamiento: ninguna segunda instancia, abre una pestaña. C7 «Salir» y relanzar abre `2025/2026`. C8 copia final.
  ORÁCULOS (Claude Code, sobre copias en `/tmp/s163`). (1) 21 tablas en el banco y 22 en el curso nuevo (la diferencia es sólo `curso`); las 17 de configuración, 0/0 en los dos sentidos y con las mismas cuentas; `horario_generado` y `sesion` a 0. (2) Borrada la actividad 4 (`Biol-3ºA`), con su plaza en cascada; en el banco tiene 2 sesiones, así que en el origen su borrado daba 409 igual que la 20 (guarda replicada por SQL). La edición la observó el usuario. (3) md5 del archivado `F88BFA7D…`, igual antes y después de escribir en el activo. (4) Grupo 28 p. 1285/0/0, profesor 59 p. 835/0/0, aula 44 p. 819/0/0 con 0 páginas vacías, CSV OK. (5) Log, copiado después de C8, donde no llegó: tres arranques (abren `educhronos.db`, `curso-2026-2027.db` y `educhronos.db`) y tres paradas por «Salir», sin ERROR; el segundo lanzamiento no deja líneas, por diseño.
  LA PROPUESTA DEL NOMBRE. El usuario informó de que no se proponía el año siguiente. M2 de solo lectura: está implementada (`GET /api/curso` → `propuestaSiguiente`, `NombreCurso.siguiente`), con tests en vitest y en la app, y el jar entregado la devuelve sobre la base traída de Windows. Una repetición con captura mostró `2027/2028`. Causa probable: se observó en C1, donde el campo sale vacío por diseño (condición 6), o se leyó el texto de ayuda fijo «como 2026/2027», que confunde junto a un campo que propone `2027/2028` (anotado, sin deuda).
  CRITERIO. `O-curso` **9 de 9, TERMINADO**, en seis sesiones (S158–S163) y seis Cambios. Se marcan los criterios 1–4 de la Fase 10. H4 sigue abierto con la Fase 12. `C-windows-curso` ✔ HECHO.
  DECISIONES DE MAPA (usuario, a propuesta del arquitecto). Orden: `O-disponibilidad` → objetivo de aceptación → Fase 12 (el motivo está en §2 de gestión). `D-curso-sin-borrado` revisada: sigue como mejora futura y cuelga de la aceptación.
  DEUDA. Nacen `D-curso-pestanas-desfasadas` (técnica real, sede aceptación) y `D-ps1-palanca-de-linux` (técnica real de texto, sede Fase 12). No se salda ninguna. R-deuda: el objetivo siguiente, `O-disponibilidad`, no tiene deuda bloqueante (`D-F8.5-E-b` cuelga de él y no bloquea).
  RIESGO RESIDUAL (heredado de S156). La prueba se hizo en una máquina virtual del usuario, no en un equipo gestionado del centro.
  DEFECTOS DE INSTRUMENTO, todos del asistente. (1) Recomendó instalar un JDK portable en el invitado sin haber leído el `.ps1`; el M2 midió que viaja en la entrega. (2) Presentó como hipótesis probable, sin haber leído el código, que la propuesta del nombre no estaba implementada y que S160 la había dado por cumplida; el M2 la refutó. (3) Pidió comprobar la propuesta como un añadido verbal de C3, sin punto propio ni captura, y la observación ambigua costó un M2 y una repetición: una observación de interfaz que decide una condición se pide con captura.
  SUITES: sin cambio de código, no se corrieron: **solver 92, app 534, vitest 513 en 55 ficheros**; e2e 3 tests en 3 ficheros, con `centro-minimo` rojo preexistente.
  BANCOS. `educhronos-s137.db` en `dfa4c077…` y `educhronos-s137-centro-completo.db` en `64d671fe…`, medidos en el cierre: INTACTOS. Los oráculos trabajaron sobre copias en `/tmp/s163`, y el M4 sobre otra copia del banco dentro de la máquina virtual.
  LIMPIEZA (M1-bis): archivada S161 a `bitacora-sesiones.md`; degradada S162 a «Última sesión registrada (previa):»; S163 queda como única cabecera H3 viva. Condensación: no. `O-curso` se cierra hoy, pero no deja frente acumulado en el plan: su material vivo es la sección de la Fase 10, que esta sesión actualiza, y las fichas de sus deudas saldadas ya están en «Deuda consciente CERRADA»; la única que seguía colgada de él, `D-curso-sin-borrado`, pasa a la aceptación.
  R4 / COSTURA. «`verificar-cierre.py`: **0 comprobaciones duras con fallo** tras el cierre, con su autoprueba en verde (12 defectos inyectados, los 12 detectados); índices de `gestion_proyecto.md` y `plan_trabajo_horarios.md` regenerados, **de 14 y 24 entradas descuadradas a 0 y 0** sobre 33 y 68 encabezados —en HEAD los dos estaban a 0 con esos mismos 33 y 68: el descuadre lo produjo esta misma sesión al insertar en gestión la línea de estado de S163 en la ficha de `O-curso` y las dos filas de §4, y en el plan la cabecera de S163, el archivado de S161, la línea de la NOTA de la Fase 10, las dos fichas de deuda y las dos notas técnicas; la medición del «antes» se hizo importando `regenerar-indice.py` como módulo y llamando sólo a su comprobación—. **Censo de tokens sospechosos: 39** sobre 296 tokens distintos en el corpus vivo, y la lista es IDÉNTICA a la de HEAD (39 sobre 293), medida corriendo el verificador sobre el árbol de HEAD antes de escribir: ningún token entra ni sale. (a) Los tres tokens nuevos **no los marca el censo**: `C-windows-curso` nace con 3 apariciones vivas —la fila de H4, los Cambios de la ficha de `O-curso` y la entrada del índice—, `D-curso-pestanas-desfasadas` con 3 —su fila de §4, la nota de `C-windows-curso` y su ficha del plan— y `D-ps1-palanca-de-linux` con 2 —su fila de §4 y su ficha—, así que `D-deuda-sin-sede-en-el-plan` no engorda. (b) Una de las 3 de `C-windows-curso` NO es una cita: es la entrada del ÍNDICE GENERADO que reproduce la cabecera de S163, **quinta reproducción medida de `D-censo-r4-rescate-por-indice`** tras las de S159, S160, S161 y S162; sin ella el token tendría 2 y seguiría fuera de la lista, así que el rescate no cambia ninguna decisión, sólo el número. Las dos deudas nuevas no lo sufren: la cabecera no las nombra. (c) La línea de R4 no rescata nada, porque desde S157 la entrada de sesión sale del corpus: las cifras que constan son las medidas antes de escribirla y se vuelven a comprobar después. (d) Extinción respecto de HEAD (`D-censo-r4-ciego-a-la-extincion`, ya instrumentada): **NINGÚN token cae a 0**, ni entre los citados en el bloque de S161 que esta sesión archiva. (e) `O-nombre` sigue siendo el único token que vive SÓLO en entradas de sesión, sin cambio respecto de HEAD. (f) Los tres censos coinciden en **S161** —cabecera de la bitácora, línea de orden y crónica del plan—, comprobado por el verificador, y el cuerpo de S161 en la bitácora es idéntico al que sale del plan (diff vacío). (g) HIGIENE/MÉTODO: **13 fichas** vivas, las mismas que en S162 porque hoy no nace ni se cierra ninguna de esa sede —las dos nuevas cuelgan de la aceptación y de la Fase 12—; el filtro mecánico por la columna de sede de §4 da 18 filas, 5 tachadas, igual en HEAD y ahora (S162 anotó 20 con un filtro que esta sesión no ha reproducido); **el disparador por recuento NO está activo: 13 de 20**. (h) La frase de ventana del plan se comprobó A MANO (`D-verificar-cierre-ciego-a-la-ventana`): dice S162 degradada y S163 como única cabecera H3 viva, y el plan tiene exactamente una. R-deuda aguanta al pasar a `O-disponibilidad`: ninguna deuda viva lo bloquea.»
...
- **D-indisp-solo-tramo-de-inicio** (S165, VIVA, TÉCNICA REAL, sede `O-disponibilidad` condición 5) — EL SOLVER SÓLO APLICA LA INDISPONIBILIDAD AL TRAMO DE INICIO DE CADA SESIÓN. Medido en S165 con tests desechables (Claude Code, HEAD `7bd70a6`): con `duracionTramos = 2`, una DURA sobre el tramo INTERIOR no impide colocar el bloque (T1; control con la DURA en el inicio: INFEASIBLE) y una BLANDA sobre el interior penaliza 0 (T4; control en el inicio: 1). Sitio: `ModeloCpSat.restriccionIndisponibilidadProfesor` y `objetivoIndisponibilidadBlandaProfesor` sólo restringen `tramoIndex`. Con duración 1 es correcto (control de T1 y tests 6b/6c). S167 NO toca el solver —decisión del usuario: la interfaz no permite actividades de más de un tramo— y levanta en su lugar el CORTAFUEGOS `RESTRICCION_HORARIA_CON_BLOQUE` en `PrevalidacionService` (`7bb65be`): ERROR, y por tanto 422 al generar, por cada profesor con alguna restricción horaria que imparte una actividad de duración > 1. **El cortafuegos SÓLO se retira junto con el arreglo del solver para duración > 1 (S165 T1/T4), en el mismo cambio, NUNCA por separado:** retirarlo antes devuelve horarios que parecen válidos y meten al profesor en su tramo vetado. Arreglo previsto: vetar todo inicio cuyo bloque ocupe un tramo DURA (`{t−k : k en 0..d−1}` intersecado con los inicios válidos de `iniciosValidosDeBloque`) y penalizar todo inicio que cubra el tramo BLANDA, sin variables nuevas por tramo; primero, fixtures de bloque + DURA/BLANDA que fallen contra el solver actual. **Puerta de regresión:** la huella canónica del modelo idéntica en los 44 fixtures (S166 0.3, ver `D-modelo-no-determinista-entre-jvm`) y la invariante de indexación —los índices de `problema.tramos()` son contiguos dentro de cada día y crecen con `ordenEnDia`, cierto por construcción en `CatalogoMapper.renumerarLectivos` y en `ProblemaHorarioMapper`, medido en los dos bancos y en los 44 fixtures, pero NO validado por el constructor de `ProblemaHorario`— (S166 0.2), que el arreglo debe comprobar o hacer cumplir. No se comparan objetivos ni tiempos de escala: no son reproducibles (`D-generacion-no-reproducible`). **El arreglo del modelo NO debe reutilizar `VerificadorSolucion.tramosOcupados`** (público y estático desde S167, sólo para la prevalidación del pin sobre DURA): el verificador tiene que seguir siendo un oráculo INDEPENDIENTE del modelo, y si los dos compartieran la definición de «tramos ocupados», un error en ella pasaría los tests de concordancia sin que nada avisara (decisión del usuario, S167). El oráculo ya existe: el verificador cuenta por tramo ocupado desde S167 (`INDISPONIBILIDAD_PROFESOR` y la BLANDA, `2e44c5f`), así que con bloques el recuento BLANDA del verificador y el término del modelo divergen hasta el arreglo. Relación con el criterio: la condición 5 de `O-disponibilidad`, escrita en S164, pide que la DURA vete todos los tramos que ocupa cada sesión; con esta deuda viva sólo se cumple porque el cortafuegos impide la combinación. Decisión del usuario (S167): la condición 5 cuenta como cumplida con alcance limitado a duración 1, no a secas.
- **D-ventanas-consecutivas-ciegas-a-bloques** (S165, VIVA, TÉCNICA REAL, no bloqueante, sin objetivo asignado) — VENTANAS Y CONSECUTIVAS NO VEN LOS TRAMOS INTERIORES DE UN BLOQUE, NI EN EL SOLVER NI EN EL VERIFICADOR. Por lectura (S165 y S167, Claude Code): en `ModeloCpSat`, los literales `instEnT`/`ocupa` del término de ventanas y `instEnTC`/`ocupaC` del de consecutivas significan «la instancia EMPIEZA en t»; en `VerificadorSolucion`, `contarVentanasProfesor`, `contarPenalizacionConsecutivasProfesor` y los deltas de ventanas y consecutivas de `atribuirBlandas` sólo añaden el `ordenEnDia` del tramo de inicio. Un bloque de 2 cuenta como una sola clase y su segundo tramo parece hueco. Como solver y verificador comparten el punto ciego, los tests de concordancia no lo pueden detectar. A diferencia de `D-indisp-solo-tramo-de-inicio`, NO depende de las restricciones horarias: afecta hoy a todo catálogo con actividades de más de un tramo (fixtures de FPB y `fusion-instituto-completo`; ningún banco del centro, medido en S166 0.4). No se implementa nada hasta que el usuario cierre la semántica —si un bloque ocupa cada uno de sus tramos a efectos de ventanas, y si cuenta como N sesiones seguidas o como una a efectos de consecutivas—; la primera sesión será de análisis y cuantificará el impacto en los objetivos de escala. S167 dejó los dos términos intactos a propósito.
- **D-modelo-no-determinista-entre-jvm** (S166, VIVA, TÉCNICA REAL, no bloqueante, sede con `D-generacion-no-reproducible`) — EL MODELO CP-SAT DE UN MISMO PROBLEMA NO SALE IGUAL EN DOS PROCESOS. Medido en S166 0.3 (Claude Code, test desechable, HEAD `7bd70a6`, tres JVM): el SHA-256 de `CpModelProto.toByteArray()` es el mismo al construir dos veces en una JVM, pero cambia entre JVM en 13 de los 44 fixtures —`3-cierre-cyr-refmt`, `5-escala-1bach`, `5-escala-4ESO`, `5-escala-4ESO-Di`, `5-escala-instituto`, `5-fusion-3-4-eso`, `5-fusion-eso-completa`, `5-fusion-instituto-completo`, los tres `8-2b-pin-aula-*`, `aulaCandidata-factible` y `aulaCandidata-mixta`— con los mismos recuentos de variables y restricciones en todos: cambia el ORDEN, no el contenido. Causa probable, por lectura y NO probada: el orden de iteración de `Map.copyOf`/`Set.copyOf`, que lleva una sal aleatoria por JVM y alimenta las opciones de aula de `InstanciaProgramada` y los conjuntos de `Plaza`. Una huella canónica —variables por nombre y dominio, restricciones con las referencias sustituidas por nombres y sus listas ordenadas, objetivo ordenado— sale idéntica en las tres JVM, sin nombres de variable duplicados; que detecte un cambio real está aún por demostrar con un mutante. Consecuencias: (1) un hash crudo del modelo no sirve de puerta de regresión, sí la huella (`D-indisp-solo-tramo-de-inicio`); (2) por inferencia, fijar `num_search_workers`, uno de los caminos de `D-generacion-no-reproducible`, podría no bastar para repetir una generación entre dos arranques, porque el orden del modelo puede orientar la búsqueda. En la misma medición, `fusion-instituto-completo` con 600 s y semilla 42 dio objetivo 230, 217 y 224 en tres corridas: es evidencia de `D-generacion-no-reproducible`, no de esta.
