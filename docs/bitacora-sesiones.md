...
Registro detallado e histórico de las sesiones de trabajo S10–S145. Archivado
...
Orden: cronológico ascendente (S10 → S145). Los formatos difieren según la época
...
### Sesión 130 — O-diseño (transversal): **C-revisión, TRAMO 2 APLICADO**. Los 195 valores de espaciado de las hojas de componente pasan a la escala `--e1..--e6` con los 24 redondeos DECLARADOS uno a uno, y el radio se unifica en dos niveles sobre 36 reglas: `--radio-s` sube de 3 a 4 px y estrena 21 usos, `--radio-m` cubre los 15 contenedores. Al buscar el precedente de declaración de S128 resulta que NO EXISTE y que su recuento era falso: nace `D-declarado-sin-artefacto`. El M4 en navegador destapa que la jerarquía de acciones que `styles.css` declara «aplicada» NO lo está (M0 + M2 + M4 en navegador + M1, sin M3: el tramo no toca un solo `.ts`). Cinco commits. **NO cierra O-diseño**: el tramo 3 queda a medias y su trabajo restante pasa a S131.
  ...
  **(d) EL PADDING LATERAL DE `.app__contenido` NO CRUZA EL CRITERIO 4.** La regla es `padding:
  ...
  anterior.
  **AMPLIADO en S145, medido y no supuesto:** (a) para un M4 con navegador hay que ENTRAR POR `/` y
  navegar desde la landing — la ruta profunda `http://localhost:8080/horario/1` devuelve 404 con la
  Whitelabel Error Page, porque el jar no reenvía las rutas de la SPA al `index.html`
  (`D-spa-sin-fallback-de-rutas`, sede H4). (b) el corolario del `package` vale igual para TS y no
  sólo para CSS: `spring-boot:run` no reconstruye el frontend, así que tocar cualquiera de los dos
  durante un M4 obliga a repetir `mvn -pl app package -DskipTests` o el navegador seguirá sirviendo
  el bundle anterior.
...
### Sesión 137 — O-demo (H2): **EL CENTRO ENTREGA LA LISTA OFICIAL DE TUTORES Y O-DEMO CIERRA. Las TRES patas del criterio 5 de Fase 8 quedan demostradas a escala real: centro completo por las vías legítimas del producto, horario válido sobre él y ningún dato identificativo falso en pantalla.** La corrección entra como PAR INDIVISIBLE —7 tutorías reescritas y `requiereTutor` a 0 en las 6 actividades de bloque— por el catálogo, y la base se rehace desde vacío. Se cierran `D-tutores-bachillerato` y `D-hallazgo-E-refutado`; nacen `D-meta-invariantes-a-mano` y `D-catalogo-candidatos-huerfano`. Tres commits, ni un fichero de `app/` ni de `solver/` tocado.
  TIPO Y RITUAL: Saneamiento/herramienta — M0 + M2 en pasadas + M1. Sin M3 (no se muta lógica de producto: lo escrito es un guion de `tools/` y el catálogo derivado) y sin M4 (no cambia ninguna vista). Tiene fila en la tabla de Tipos, igual que S135, así que la observación de S133/S134/S136 sobre sesiones sin tipo no suma instancia.
  M0. Cambio `C-presentable`, Objetivo O-demo, Hito H2. **El Cambio NO hubo que crearlo**: existe desde el M0 de S136 y estaba EN CURSO, que es la primera vez en cuatro sesiones que una apertura de O-demo no tiene que inventar su pieza. R-invalidación: NINGUNA. El M0 midió el estado contra la documentación y descartó con argumento los otros tres candidatos vivos del prompt: la NOTA DE ALCANCE de O-demo (el M0 de S120 ya midió que no avanza el criterio de terminado, y su mitad restante —el DESCUBRIMIENTO del modelado— no la demuestra ningún ejercicio guionizado por el arquitecto, luego la sesión no podría mover nada); O-particiones (no está bloqueado técnicamente —la base de S135 es exactamente el punto de partida que su ficha pide— pero §2 escribe que va DESPUÉS de la demo, y al menos dos de sus cuatro preguntas de dominio son para el jefe de estudios, con lo que abrirlo mientras se espera al centro tiene alta probabilidad de acabar esperando otra vez); e Higiene/Método por R-deuda, aunque por primera vez su lista tiene un tema y no sólo tamaño. La bifurcación se resolvió por un dato que no estaba en la documentación: si el centro había contestado. Contestó.
  LA LISTA, Y EL CONTRASTE QUE LE DA CRÉDITO. La entrega del centro cubre **28 grupos exactos** —23 ordinarios y 5 PDC—, ni sobra ni falta ninguno, y usa el nombre `3º ESO PDC` que ya casaba con la nomenclatura del catálogo. Lo decisivo no es que lo diga el centro sino que **casa 16/16 con el cruce que S136 hizo contra `Horarios de profesores.pdf`**, nombre a nombre y código a código: acierta en todo lo verificable, que es lo que permite fiarse de las 7 filas que no se pueden contrastar contra nada. Los siete: **1B-A/LEN7 Moreno Mesa, 1B-B/MAT7 Carbonell Coronado, 1B-C/LEN2 García Gómez, 1B-D/LEN1 Moreno Araujo, 2B-A/MAT2 Mesa Guarnido, 2B-B/GH3 Vela Montero, 2B-C/GH5 López Escudero.** La lista confirma además, oficialmente, las cinco co-tutorías de ORI1 Guerrero Serrano sobre los grupos Di que S136 había visto en el volcado.
  LA MEDICIÓN QUE PODÍA TUMBAR EL PLAN, HECHA ANTES DE PROMETER NADA. `D-censo-profesores-80-59` dejaba abierto que alguno de los siete tutores oficiales fuera de los 21 profesores que el PDF trae y el catálogo no: si así fuera, no habría a quién asignar la tutoría y la sesión cambiaba de forma. **Los siete códigos existen entre los 59.** Y el cotejo con la base da **7 de 7 discrepando por CONTENIDO y no por ausencia**, con el patrón que delata el relleno: FIL2 figuraba como principal de cinco grupos de Bachillerato a la vez, y GH6 y EFI3 estaban duplicados con sus tutorías reales de ESO (1ºA y 4ºD/4ºDDi).
  **EL PAR INDIVISIBLE DEJA DE SER CITA Y PASA A SER MEDIDA, y es el mejor resultado metodológico de la sesión.** S136 lo dedujo; S137 lo recalculó recorriendo plazas → subgrupos → grupos → tutor en los TRES estados: (A) actual con tutores viejos, 22 con `requiereTutor` y 22 cumpliendo S8, cero fallos; (B) sólo las 7 tutorías, sin bajar el flag, 22 con flag y **16 cumpliendo, 6 fallando**; (C) S137 completo, 16 y 16, cero fallos. El escenario B rompe **exactamente las seis actividades de bloque, ni una más ni una menos**, así que el instrumento DISCRIMINA y no se limita a decir «cero». **Y el detalle de los fallos destapa el mecanismo, que vale por sí solo:** `FALLA Bloque-PTVE_Relig-1B-A grupos=['1B-A'] tutores=['LEN7'] profesores=['GH6','REL1']`. El «S8: OK» se sostenía porque **el tutor inventado ERA el profesor de la plaza**: GH6, EFI3 y FIL2 no cubrían el invariante por ser tutores sino por impartir la actividad. El Hallazgo E no sólo fabricó tutores, fabricó a la vez la evidencia que los hacía parecer consistentes. Un invariante puede estar en verde por la misma causa que lo vuelve falso.
  LA VÍA SE DECIDE POR MEDICIÓN Y NO POR PRUDENCIA. La opción barata —recargar encima de `educhronos-s135-centro-completo.db`— queda descartada POR CONSTRUCCIÓN: `cargar-centro.py` envía las tutorías siempre (PUT de reemplazo total) pero se SALTA las actividades que ya existen, y `requiereTutor` viaja únicamente en el POST de alta. Una recarga incremental propagaría media corrección, que es literalmente el escenario B y sus seis violaciones. Hace falta base nueva desde fichero vacío con la carga completa de 816 escrituras.
  LO ESCRITO, Y EL ROUND-TRIP QUE CONVIERTE EL GUION EN UN EDITOR FIEL. `tools/carga-centro/aplicar-tutores-oficiales.py` (`f73144c`), mismo patrón que `aplicar-regla-aulas-fpb.py` de S135, para que el dato de entrevista tenga procedencia ejecutable y no sea edición a mano de un JSON de 219 actividades. Cuatro bloques con guarda propia y aborto sin escribir: (1) las 7 tutorías, (2) `requiereTutor` a 0 en las 6 de bloque con el total pasando de 22 a 16, (3) la advertencia de `_meta`, (4) los invariantes declarados en `_meta`. **`indent=1`, medido del original en tiempo de ejecución, reproduce el fichero BYTE A BYTE**, y por eso se retiró el `f.write("\n")` final que el original no tiene: con esa propiedad, cada línea del diff es un cambio semántico. Diff del catálogo: **16 inserciones y 16 borrados, un fichero, cero ruido de formato** (`2fd5e55`). Dos defectos los cazó Claude Code en corrida EN SECO antes de tocar el repo, y el primero es de la familia peor: el bloque (3) anunciaba pagar `D-catalogo-meta-enganosa` y **sólo evaluaba e imprimía**, con un `True` que se lee como «hecho» y significa «pendiente» —declarar sin artefacto, `D-declarado-sin-artefacto`—; se reescribió con asignación real y con aborto si la frase no está donde se espera, en vez de un `replace` a ciegas. El segundo se ve más abajo.
  SE PAGA DE PASO `D-catalogo-meta-enganosa`, y CERRADA. La frase del `_meta` —«NO es un fichero que la aplicacion lea. No hay importador»— pasa a decir que la aplicación no lo lee pero `cargar-centro.py` sí, y que es la SEDE ÚNICA de la estructura del centro y donde aterriza cualquier corrección de catálogo. Verificado antes de escribirla: el guion existe, está trackeado desde `e09ba3a`, lee el catálogo y su propio docstring dice «NO hay segunda fuente». Se paga AQUÍ y no en Higiene porque cae de lleno en el camino —es el fichero que se está parcheando— y porque había mordido en esta misma sesión: esa frase es la que llevó a plantear editar las `.db` a mano.
  Y SE ARRASTRA LO QUE EL CAMBIO VUELVE FALSO, que es la otra mitad de terminar una edición. `_meta.verificacionInvariantes` declaraba «S8: OK - 22 actividades requiereTutor» y «I4: 28/28 derivados (ver ambiguedad A5)». Dejarlo intacto habría metido una afirmación numérica falsa DENTRO del mismo objeto que se estaba corrigiendo por engañoso, y en la sesión cuyo criterio es «nada de lo mostrado es falso». Pasan a 16 y a «28/28, 21 derivados y 7 oficiales del centro (A5 resuelta: el centro entrego la lista oficial)». Verificación de cierre sobre el fichero ya escrito, no sobre la simulación: **S8 da 16/16 sin fallos**, así que lo que `_meta` declara es cierto y está medido.
  EL RESIDUO DE `candidatos`, Y POR QUÉ SE VACÍA EN VEZ DE RELLENARSE (`89a1d36`). El diff dejó a la vista que las 7 entradas conservaban `candidatos: ["GH6"]`, `["FIL2"]`, `["EFI3"]`: exactamente los nombres inventados que la sesión acababa de desterrar, y 7/28 incoherentes con su propio `tutorPrincipal`. Medido que **nadie consume el campo** —ni `cargar-centro.py` ni `ESPECIFICACION-CATALOGO.md` lo mencionan—, así que es residuo documental del derivador. Se decide **vaciar y no sustituir**, con dos argumentos: lo que hay dentro procede del Hallazgo E refutado y dejarlo vivo mantiene el material inventado a un `grep` de distancia; y poner `["LEN7"]` sería una deducción sobre un campo SIN SEMÁNTICA ESCRITA, es decir el mismo error que la sesión está deshaciendo. Vaciar no es una tercera invención, es una retracción, y es literalmente la segunda mitad del criterio —lo que no tiene fuente aparece como ausente y no como inventado— aplicada al catálogo y no sólo a la pantalla. Diff: 7 inserciones y 21 borrados, las listas colapsando a `[]`. Nace `D-catalogo-candidatos-huerfano`.
  LAS BASES, con la decisión de arquitecto tomada ANTES de cargar para no destruir el punto de partida de O-particiones. **No se edita ninguna `.db` existente**: las dos de S135 conservan sus md5 (`e072383a…` y `dfb30839…`, verificados antes y después) y son el registro fiel de lo que S135 midió. Se crean DOS ficheros nuevos, gemelos de los de S135: `app/educhronos-s137-centro-completo.db` (`64d671fefa997b8a672fc7f042e01fc7`, centro completo y `sesion` 0, **nueva base de REFERENCIA DEL CENTRO**) y su copia `app/educhronos-s137.db` (`dfa4c077…` tras generar, **nuevo HORARIO DE REFERENCIA**). Se copió ANTES de generar, y la app se reinició apuntada a la copia: generar sobre la primera habría dejado una base llamada «centro-completo» con horario dentro y sin punto de partida limpio para O-particiones, cuya ficha exige que la prueba se haga antes de generar. No se renombra ni se borra nada, por trazabilidad con las actas que citan los nombres viejos.
  LA CARGA Y LA GENERACIÓN, LAS DOS AL PRIMER INTENTO. Arranque por `java -jar app/target/app-0.1.0-SNAPSHOT.jar --spring.datasource.url=…` (invocación verificada en el `pom.xml` y el manifiesto, no escrita de memoria), esquema creado por `schema.sql` sobre fichero inexistente, prevalidación con **0 violaciones** y carga de **816/816 escrituras** con las once familias cuadrando por GET. Estado de partida verificado antes de generar y exigido como valla: **219 actividades / 16 con `requiere_tutor` / 28 tutorías / 0 sesiones / 0 horarios**. Generación por la vía de producción con el presupuesto por defecto de 600 s: `HTTP 200` en **10 min 2 s**, `FEASIBLE`, **`sesion` 819** y un solo horario. Comprobado en la base que la corrección aterrizó entera: 7/7 tutores oficiales, 6 actividades de bloque a 0 y **cero supervivientes de GH6/FIL2/EFI3 como tutores de Bachillerato**.
  LAS TRES CAPAS, CON LAS PREDICCIONES DECLARADAS ANTES DE MEDIR. **CAPA 1: cero violaciones de regla dura sobre 819 sesiones y los 28 grupos a 30/30**, y —comprobación NUEVA de esta sesión, porque S8 pasa a exigirse sobre 16 actividades y no 22— **ningún `TUTORIA_SIN_TUTOR`**, que era la única forma en que el par indivisible podía salir mal en silencio. **CAPA 2: 526 claves, CERO divergentes, 1301 = 1301, delta 0.** Se predijo EXIGIBLE y no esperable, contra el pronóstico de que el cambio de tutores la movería: la capa cruza ternas (grupo, asignatura, profesor) sobre las PLAZAS, y el parche no tocó ni una plaza. Que se mantenga confirma que el instrumento mide lo que dice medir. **CAPA 3: 201 + 0 + 34 = 235**, la identidad recompuesta sin residuo por tercera vez. Remedida y no heredada la advertencia: `profesor_restriccion_horaria`, `sesion_bloqueada` y `aula_bloqueada` están las tres VACÍAS en esta base, luego el `indispBlanda = 0` es VACUO; las 561 penalizaciones son contrafactuales por celda.
  **EL 235 FRENTE AL 203 DE S135: VARIANZA DEL SOLVER, DEMOSTRADA Y NO CONJETURADA, y NO se corrige (R-terminado).** El argumento es de relajación y lo cerró la medición de las tres tablas vacías: ambas bases tienen la misma jornada (35 tramos, 30 lectivos), cero restricciones y cero bloqueos, y **ninguna de las 16 actividades que conservan `requiereTutor` toca un grupo de Bachillerato**, luego las siete tutorías corregidas no intervienen en ninguna restricción S8 viva. El problema de S137 es el de S135 menos seis restricciones y con idéntica función objetivo, así que toda solución factible de S135 lo es de S137 al mismo coste: **el óptimo de S137 es ≤ 203 y existe una solución de 203, porque S135 la encontró.** El solver devolvió 235 teniéndola al alcance. Un problema con menos restricciones no puede tener peor óptimo. **Lo que eso mide es la DISPERSIÓN: al menos 32 puntos a 600 s, con `FEASIBLE`, cota inferior 0.0 y sin semilla**, dato nuevo que refuerza lo que S117 y S118 caracterizaron. Se DENIEGA regenerar buscando un horario mejor —con semilla fija, más presupuesto o la mejor de varias corridas—: la calidad se mide y no se corrige, el horario de referencia se usa como oráculo de las capas 1 y 2 y **ninguna de las dos depende del coste blando**, y quedarse con la mejor de varias corridas produciría una referencia NO reproducible por la vía de producción, que es un problema nuevo (`D-generacion-no-reproducible`). El límite se escribe y no se paga: **235 no es comparable con 203 por dos razones acumuladas** —instancias distintas, como ya escribió S135, y dos corridas sin optimalidad probada ni semilla, que CP-SAT no hace comparables entre sí—.
  **CON ESTO O-DEMO CUMPLE SU CRITERIO Y CIERRA, abierto en S115 y con 22 sesiones por medio.** Las tres patas del criterio 5 de Fase 8: el centro entero creado por las vías legítimas del producto (816/816 por la API REST desde base vacía), un horario válido sobre él (819 sesiones, cero violaciones duras, conservación con delta 0 contra el horario que el centro imparte) y **presentable**, en la lectura estrecha que escribió el M0 de S136: ningún dato identificativo mostrado contradice lo que el centro dijo de sí mismo, y lo que no tiene fuente aparece como ausente. Queda VIVA y declarada la NOTA DE ALCANCE de S115, estrechada en S120: lo demostrado por UI a escala real es el centro mínimo, y siguen sin demostrarse por interfaz la ESCALA y el DESCUBRIMIENTO del modelado. **Con O-demo terminado, a H2 le queda UN objetivo: O-particiones.**
  DEUDA. **Cerradas DOS: `D-tutores-bachillerato`** —el dato llegó, se cargó y está verificado en la base; era la única cosa que separaba a O-demo de su criterio y no era trabajo técnico— **y `D-hallazgo-E-refutado`**, cuya huella en la base (el `requiere_tutor` de las 6 actividades de bloque) queda a 0 y cuya afirmación sobre GH6 queda sustituida por el dato oficial, que es lo que S134 esperaba para poder cerrarla. Cerrada también, de paso y en el camino, **`D-catalogo-meta-enganosa`**. Nacen DOS: **`D-meta-invariantes-a-mano`** y **`D-catalogo-candidatos-huerfano`**. **R-deuda ratificado por los dos lados el mismo día**: se cobran de paso las que caen en el camino del Cambio (la advertencia del `_meta`, y sus invariantes por ser la mitad restante de la misma edición) y se DENIEGA lo que estaba igual de a mano y no cae en él (el experimento de semilla y dispersión, que es calidad). Dos deudas nuevas y ninguna abre sesión. **Deuda bloqueante del proyecto: 0.**
  LA OBSERVACIÓN DE MÉTODO: **el proyecto sigue sin instrumento que verifique S8 antes del hecho.** `cargar-centro.py --prevalidar` valida I2, I4 e I7 y NO S8; `requiereTutor` sólo se mira al enviar. Y los «OK» de `_meta.verificacionInvariantes` son prosa que ningún guion recalcula. Si el par se hubiera aplicado a medias, ni la herramienta ni el fichero lo habrían delatado: la red existe, pero está en la capa 1, es decir DESPUÉS de 816 escrituras y diez minutos de solver, cuando el aserto barato podría avisar antes. Nace `D-meta-invariantes-a-mano` con esta sesión como primera instancia medida. Se registra además que la contención de S135 —gatear toda escritura con una corrida en seco revisada— se ha vuelto a pagar sola: interceptó los dos defectos del guion antes de tocar el repo.
  LIMPIEZA (M1-bis): archivada S135 a `bitacora-sesiones.md` (promovida a `### Sesión 135`, insertada al final en orden ascendente, cuerpo íntegro); degradada S136 a «Última sesión registrada (previa):»; S137 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S134 a S135. R4/costura: ni un fichero de `app/src`, `solver/src` ni `app/frontend/src` tocado en toda la sesión —los tres commits son catálogo y herramienta—, así que las suites medidas en S132 —91 + 282 de Maven y 419 de vitest— siguen midiendo este árbol y NO se recorren, con el mismo argumento explícito de S133, S135 y S136. Los dos tokens nuevos nacen con definición viva aquí y citante vivo en `gestion_proyecto.md` §4.
  **O-demo (H2, abierto en S115) ✔ TERMINADO en S137**, con `C-presentable` HECHO. O-particiones (H2, esbozado S115) pasa a ser el ÚNICO objetivo que separa a H2 de su cierre, y su punto de partida material se renueva: `app/educhronos-s137-centro-completo.db`, centro completo y sin horario. O-ajuste-cierre (H1) sigue vivo, sin deuda que lo bloquee y con su criterio pendiente de reescribir, que es trabajo de su propio M0. **Deuda bloqueante del proyecto: 0.** Commits: `f73144c` (guion), `2fd5e55` (catálogo: tutores, flags y `_meta`) y `89a1d36` (catálogo: `candidatos` vacíos). Árbol limpio al cierre en `89a1d36`, con `058d978` como antepasado.

### Sesión 138 — O-particiones (H2): **LAS CUATRO PREGUNTAS DE DOMINIO SE CIERRAN POR MEDICIÓN Y NINGUNA ERA PARA EL JEFE DE ESTUDIOS. El objetivo se reduce a la mitad: `Particion` NO hay que materializarla, porque el mecanismo de mezcla ya existe en `plaza_subgrupo` y expresa más que la plantilla que `D1` proponía.** Se reescribe el «Terminado cuando», que era inalcanzable tal como estaba redactado. Sesión de MEDICIÓN en tres pasadas de solo lectura: ni una fila escrita en ninguna base. Nacen cuatro deudas y ninguna abre sesión. Ni un fichero de `app/`, `solver/` ni `app/frontend/` tocado.
  TIPO Y RITUAL: MEDICIÓN — M0 + M2 en tres pasadas + M1. Sin M3 (no se muta lógica de producto) y sin M4 (no cambia ninguna vista). Mismo tipo que S136, que tampoco tiene fila en la tabla de Tipos de `metodo.md`: **cuarto caso en seis sesiones, y el patrón ya está consolidado** —una sesión cuyo entregable es una MEDICIÓN y una reescritura de criterio, no un cambio de producto—. Decidir si «Medición» entra como tipo con fila propia es trabajo de la sesión de Higiene/Método y no de este cierre, pero el recuento se deja escrito aquí para que esa sesión no tenga que reconstruirlo.
  M0. Cambio `C-alcance-particiones`, CREADO en este M0 (precedente: S107, S115, S135, S136). Objetivo O-particiones, Hito H2. R-invalidación: ninguna. La dependencia escrita del objetivo era O-demo, TERMINADO en S137, así que el hecho nuevo que permite abrirlo es de S137 y no de esta sesión. Se descartaron O-ajuste-cierre —cuyo M0 está igual de definido— e Higiene/Método (R-deuda: doce entradas, ninguna bloquea).
  **EL ARGUMENTO CON QUE SE ABRIÓ LA SESIÓN QUEDA REFUTADO POR LA PROPIA SESIÓN, y conviene que quede escrito.** El M0 eligió O-particiones sobre O-ajuste-cierre por un argumento de PLAZO: al menos dos de sus cuatro preguntas de dominio eran para el jefe de estudios, y formularlas pronto solapaba la espera con otro trabajo (la lección de `D-preguntas-sin-cola`, medida dos veces en S136 y S137). **La medición demostró que NINGUNA de las cuatro es para el centro**: las cuatro se contestan con el código y los datos delante. La elección fue acertada y el argumento que la sostenía era falso. Se registra porque un método que sólo anota los aciertos no mide nada.
  PASADA 1 — `Particion` NO EXISTE EN NINGÚN PLANO, y el documento de modelo lleva 90 sesiones diciendo lo contrario. Ni `CREATE TABLE particion` ni `subgrupo_particion` en `schema.sql`; las 21 tablas declaradas casan una a una con las 21 reales de la base. D-a está VERBATIM en `Subgrupo.java`, remitiendo al Bloque 4 de S48. Y `modelo_datos_fase1.md` §4.2 —que se presenta como «referencia autoritativa del esquema hasta que sea modificada explícitamente con justificación registrada»— sigue declarando las dos tablas SIN una sola mención a D-a ni a S48. Corregido en esta sesión (ver más abajo). `GrupoAdministrativo` no tiene lado inverso: cero `OneToMany`, cero `Set<`, y el 409 de borrado se calcula por consulta. Anotada de paso una asimetría de FORMA entre guardas: `ActividadService` usa un `exigirSinDependientes(id, accion)` compartido por `editar` y `borrar` —deliberadamente, para que no se pueda vaciar por PUT lo que no se puede borrar—, mientras `GrupoService.borrar` lo hace inline y `editar` no comprueba nada. En `Grupo` es coherente (editar código o nivel no rompe FK entrante), así que se registra como observación y no como deuda.
  PASADA 2 — **LA PREDICCIÓN CENTRAL FALLA, Y ES EL RESULTADO DE LA SESIÓN.** Se predijo que habría subgrupos multi-grupo en la base (el javadoc de `Subgrupo` cita la «Lectura B de Bachillerato» como caso de uso del `Set<GrupoAdministrativo>`). **Son 334 subgrupos y 334 aristas en `subgrupo_grupo`, cero huérfanos: TODOS mono-grupo, sin excepción.** La capacidad está construida y jamás estrenada. La mezcla entre grupos vive un nivel más arriba, en `plaza_subgrupo`: 189 plazas con 1 subgrupo, 47 con 2, 41 con 3, 34 con 4 y 5 con 6, es decir **127 plazas con dos o más**. `RefLe-4ºADi+4ºDDi` no es un subgrupo que abarque dos grupos: son dos subgrupos mono-grupo colgando de la misma plaza.
  **LA SEGUNDA PREDICCIÓN TAMBIÉN FALLA, Y ROMPE LA DICOTOMÍA CON QUE SE HABÍA PLANTEADO LA PREGUNTA (1).** De los 39 bloques (actividades con K>1 plazas), **29 replican el universo de grupos en todas las vías y 10 lo REPARTEN**, en tres formas distintas: (a) vías propias más vía común (`ATED_Relig-1ºA+1ºB`: P1→1ºB, P2→1ºA, P3→ambos); (b) reparto en mitades (`DIG_EXPRE_FOPP_TEC-4ºA+4ºB+4ºC+4ºD`: P2→{4ºA,4ºB}, P3→{4ºC,4ºD}, P1/P4/P5→los cuatro); y (c) **sin ningún grupo en todas** (`BIOL_Físic_Geogr_HART-2BACH`, 5 vías, universo {2B-A,2B-B,2B-C} que no aparece completo en ninguna). El caso (c) es el que decide: ese reparto no se hereda de nada, **depende de qué eligió cada alumno y cambia cada septiembre con la matrícula**. Por eso la pregunta (1) no es para el jefe de estudios ni es una regla que enunciar: es un DATO DE ENTRADA que se aporta en el alta, y por tanto UX y no requisito externo.
  PASADA 2, EL RESIDUO POR NIVEL, que es la cifra que entra en el criterio. Ninguna actividad cruza niveles, así que el recuento parte limpio: **2BACH 4 replicados / 4 de reparto (50 %), 1ESO 5/2 (29 %), 1BACH 8/2 (20 %), 2ESO 2/1 (33 %), 4ESO 6/1 (14 %), 3ESO 4/0 (0 %)**. El «1 de cada 4» global esconde la forma: **una plantilla de producto cartesiano funcionaría en 3º y 4º ESO y se rompería justo donde el horario es más difícil.** Coste actual de replicar un grupo a mano, medido: 1ºA–1ºD son 11 subgrupos y 19 aristas; 4ºA y 4ºB, 24 y 35; 4ºC, 18 y 29; 4ºD, 21 y 33. Las actividades implicadas son ~12 en todo el nivel: **el coste escala con la OPTATIVIDAD y no con la matrícula.** Y `D-itinerario-como-grupo` queda acotada: `grupo_administrativo` ya tiene `tipo` y auto-FK `grupo_padre_id` en uso (5 `DIVERSIFICACION_PDC`, 23 `ORDINARIO`, **0 `VIRTUAL_OPTATIVA`** —tercer tipo declarado y jamás instanciado—), pero visto el caso (c) el BLOQUE expresa más de lo que expresaría ese grupo virtual, así que la remodelación de 4º ESO deja de ser precedencia de este objetivo.
  PASADA 3 — LAS RUTAS DE ESCRITURA, Y LA CONDICIÓN DE OPERACIÓN QUE ENTRA EN EL CRITERIO. **No existe `PlazaService` y no es un olvido:** el javadoc de `ActividadService` declara que la actividad es la raíz y sus plazas viajan embebidas, y el censo de controladores lo confirma (`/api/grupos`, `/api/grupos/{idPadre}/pdc`, `/api/subgrupos`, `/api/actividades`, y nada de plazas). Consecuencia directa: **`plaza_subgrupo` —donde vive el reparto— sólo se escribe con `PUT /api/actividades/{id}`**, y `exigirSinDependientes` lo bloquea con 409 si hay horario, sesión bloqueada o aula bloqueada. Cambiar qué grupo va en qué vía exige borrar el horario antes. La guarda es ROMA a propósito (javadoc), porque la reconciliación posicional muta plazas vivas conservando su id, así que **no se toca**: pasa a ser condición de operación escrita en el criterio, y coincide con el «cuidado de ORDEN» que la ficha ya anotaba desde S115.
  EL CRITERIO REESCRITO, que es el entregable de la sesión. El anterior pedía «sin edición manual subgrupo a subgrupo» y es INALCANZABLE en los 10 bloques de reparto por falta de información, no de código. El nuevo separa las dos mitades: automático en los 29 replicados, y **exactamente N decisiones por pantalla en los de reparto, con N medido por nivel** (2 en 1º ESO, 1 en 4º ESO, 4 en 2º Bachillerato). Añade además dos condiciones que el anterior no recogía: base SIN horario, y **REVERSIBILIDAD** —hoy `GrupoService.borrar` rechaza con 409 un grupo que esté en algún subgrupo, así que un alta que cree 24 subgrupos produce un grupo irreversible, y septiembre es cuando el alta se hace a tientas—. Texto íntegro en la ficha de `gestion_proyecto.md` §3.
  SE CORRIGE `modelo_datos_fase1.md` (R5), y es un pago que caía de lleno en el camino. Su §4.2 declaraba `Particion` y `SubgrupoParticion` como esquema vivo, sin mención a D-a ni a S48, mientras el javadoc del código dice lo contrario. Diseñar sobre ese documento habría sido rediseñar a ciegas algo decidido hace 90 sesiones. Se anota la no materialización con la evidencia de S138, y se anota en `D1` (§8) que su forma escrita —`patron_generacion` JSON sobre `Particion`— queda descartada por medición.
  DEUDA. Nacen CUATRO y ninguna abre sesión: **`D-subgrupos-di-sin-api`** (18 subgrupos de los PDC sin vía de gestión, con un 400 que remite a una puerta donde el recurso no está), **`D-borrado-pdc-integridad-500`** (FK latente en `PdcService.borrar`, reconstruida sobre copia, hoy inalcanzable), **`D-codigo-actividad-tilde`** y **`D-i3-sin-datos`** (`asignatura_aula_compatible` con 0 filas: I3 no la ejercita ningún dato del centro real). **R-deuda ratificado por el lado del hallazgo abundante:** una sesión de medición destapa cuatro deudas de una sentada y ninguna se paga, porque ninguna bloquea el criterio del objetivo activo y ninguna cae en el camino del Cambio —el gesto de prueba es un grupo ORDINARIO y las dos de PDC no lo tocan—. Lo que SÍ se cobra de paso es la corrección de `modelo_datos_fase1.md`, que sí cae en el camino: es el documento sobre el que se estaba decidiendo. **Deuda bloqueante del proyecto: 0.**
  LIMPIEZA (M1-bis): archivada S136 a `bitacora-sesiones.md` (promovida a `### Sesión 136`, insertada al final en orden ascendente, cuerpo íntegro); degradada S137 a «Última sesión registrada (previa):»; S138 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S135 a S136. R4/costura: ni un fichero de `app/src`, `solver/src` ni `app/frontend/src` tocado en toda la sesión —los cambios son de documentación—, así que las suites medidas en S132 —91 + 282 de Maven y 419 de vitest— siguen midiendo este árbol y NO se recorren, con el mismo argumento explícito de S133, S135, S136 y S137. Los cuatro tokens nuevos nacen con definición viva aquí y citante vivo en `gestion_proyecto.md` §4.
  **O-particiones (H2) sigue ABIERTO, con `C-alcance-particiones` HECHO y criterio VÁLIDO por primera vez.** No está bloqueado ni por deuda ni por requisitos: el trabajo que le queda es de construcción y el diseño está decidido —clonar de un hermano, preguntar sólo en los bloques de reparto—. O-ajuste-cierre (H1) sigue vivo, sin deuda que lo bloquee y con su criterio pendiente de reescribir, que es trabajo de su propio M0. Bases de referencia sin movimiento, verificadas por md5 al abrir y al cerrar.

### Sesión 139 — O-particiones (H2): **EL ALTA REPLICADA EXISTE EN SERVIDOR. Un grupo nuevo hereda los bloques de un hermano por un sub-recurso propio, con CERO decisiones en los 29 bloques replicados y exactamente N en los 10 de reparto.** Primera sesión de Desarrollo y primer fichero de `app/src` tocado desde S132. **El «Terminado cuando» del objetivo se ESTRECHA a los bloques**, tras medir que clonar dentro de una actividad de plaza única no replica al grupo: lo FUNDE con su hermano. Cuatro modificaciones al código existente, tres ficheros nuevos, 11 tests y 10 mutaciones. Nace y se paga `D-javadoc-plazas-caducado`. Ningún fichero de `solver/` ni de `app/frontend/` tocado.
  TIPO Y RITUAL: Desarrollo — M0 + M2 en tres pasadas + M4 de contraste + M3 con campaña completa + M1. Sin M4 en navegador: el Cambio no cambia ninguna vista. Tiene fila en la tabla de Tipos, así que la observación de S133/S134/S136/S138 sobre sesiones sin tipo no suma instancia. **Ambigüedad del método detectada y NO resuelta aquí:** la tabla condiciona M4 a «>1 módulo», pero lo que S90 condiciona al número de módulos es el TURNO DE MEDICIÓN previo, no el contraste, que el cuerpo de M4 enuncia sin condición. Se resolvió haciendo el contraste —y pagó seis hallazgos—; decidir la redacción es trabajo de Higiene/Método.
  M0. Cambio `C-replicación-alta`, CREADO en este M0 (precedente: S107, S115, S135, S136, S138). Objetivo O-particiones, Hito H2 —el ÚNICO objetivo que le queda a H2 desde que O-demo cerró en S137—. R-invalidación: ninguna. Descartados O-ajuste-cierre (§5 pone H2 antes que H1 y no hay hecho nuevo que invierta ese orden) e Higiene/Método (R-deuda). **Recuentos hechos contra §4 y no heredados, con dos correcciones: la lista de Higiene/Método tiene 15 entradas y no doce** —el número de S138 ya venía corto: incluía `D-catalogo-candidatos-huerfano`, nacida en S137— **y las deudas SIN SEDE son 4 y no dos** (a las dos de S138 se suman `D-asignatura-sin-nivel` y `D-fuente-tercera-sin-usar`). Corregido también el encuadre de la apertura: «primera sesión de Desarrollo desde S133» mide corto, porque S133 no tocó un fichero de producto y S132 sólo tocó estilos («no se tocó un solo `.ts`»).
  EL OBJETIVO SE DESCOMPONE EN TRES CAMBIOS y esta sesión hace el primero: (1) `C-replicación-alta`, el alta replicada en servidor; (2) `C-alta-reversible`, la condición (3) del criterio; (3) `C-alta-por-pantalla`, la mitad de UI de la condición (2). El corte no es arbitrario: la pantalla no se puede diseñar sin saber qué necesita la operación como entrada, y la reversibilidad depende de saber qué crea el alta.
  **EL CRITERIO SE ESTRECHA, Y ES EL HALLAZGO DE DISEÑO DE LA SESIÓN.** La cabecera del «Terminado cuando» pedía dejar al grupo participando «en todas las actividades del nivel en las que participan sus hermanos», y eso es INCORRECTO para las 180 actividades de una sola plaza: meter el subgrupo espejo en la plaza de «MAT-1ºA» no replica nada, produce una sesión con dos grupos dentro y un solo profesor. Lo que esas materias necesitan es una actividad NUEVA, con profesor, aula y carga que no se deducen de ningún hermano —el mismo dato faltante que la matrícula, medido en S138—, y su vía es el alta de actividad por UI, que existe desde S110. La cabecera era más ancha que sus propias tres condiciones, que sólo hablaban de los 39 bloques. Se estrecha a los bloques y la exclusión queda escrita. **No vacía el objetivo:** lo que éste mata es la edición subgrupo a subgrupo —11 subgrupos y 19 aristas en 1ºA, 24 y 35 en 4ºA—, no el alta de actividades.
  M2, PASADA 2 — LOS CÓDIGOS SON DERIVABLES, MEDIDO SIN EXCEPCIÓN. Los 334 subgrupos de la base de referencia son mono-grupo Y empiezan por `{codigo_grupo}-`: 334/334 en las dos condiciones, cero excepciones. El clonado sustituye el prefijo y conserva el sufijo. El riesgo que la apertura de M2 había levantado —códigos por convención de cadena, la familia de `D-pdc-vinculo-por-cadena`— queda CERRADO por medición, y de paso se corrige el encuadre que lo levantó: `RefLe-4ºADi+4ºDDi` y `EFis-4ºA+4ºADi` son códigos de ACTIVIDAD, no de subgrupo. El clasificador de bloques se reconstruyó como instrumento de producto con su autoprueba delante (oráculo de S138 clavado nivel a nivel; mutación `== U` → `⊆ U` moviendo el resultado a 39/0), y confirmó que ningún bloque cruza niveles y que 1FPB y 2FPB no aportan ninguno.
  M2, PASADA 3 — LA FRONTERA NO ES UNA CONVENCIÓN IMPUESTA, ES UNA PROPIEDAD DE LOS DATOS. **Ninguno de los 28 subgrupos `-Completo` toca una sola plaza de bloque: 0 pares.** Los `-Completo` viven exclusivamente en actividades de plaza única, y los de optatividad exclusivamente en bloques. El estrechamiento del criterio no obliga al clonado a distinguir casos: replica todos los subgrupos del hermano, cablea donde el original esté, y el `-Completo` sale solo con cero plazas. Medido también que 50 subgrupos participan en DOS actividades (nunca en dos de reparto a la vez), lo que obliga a recorrer todas las plazas del original y no la primera. Y que el espejo escala con la optatividad: 1ºA son 11 subgrupos (10 en bloque), 4ºA son 24 (23 en bloque); en los dos, el único fuera de todo bloque es su `-Completo`.
  **NO HABÍA PUERTA, Y SE ABRE UNA NOMBRADA.** `Plaza` no tenía ningún método aditivo (verificado por búsqueda de `agregarSubgrupo|addSubgrupo|anadirSubgrupo|añadirSubgrupo`: cero coincidencias) y `Plaza.actualizar` obliga a reenviar los cinco campos. Existe una puerta trasera que compila —`getSubgrupos()` devuelve el `HashSet` vivo, sin copia defensiva— y se DESCARTA usarla: el javadoc de la clase dice «mutación de dominio nombrada en lugar de setters libres», y apoyarse en que un getter no copia fabrica un acoplamiento que nadie declaró. Se añade `Plaza.agregarSubgrupo`, que además muta la colección gestionada en vez de reemplazar la referencia del `PersistentSet`, que es lo barato para Hibernate.
  **NO SE PASA POR EL PUT, Y LA RAZÓN ES LA RECONCILIACIÓN POSICIONAL.** `reconciliarPlazas` empareja por POSICIÓN sobre las plazas vivas ordenadas por id, y `PlazaRequest` no lleva id ni código: lo único que ata una plaza del cuerpo a una plaza viva es su sitio en el array. Componer un PUT por cada una de las ~12 actividades de un nivel para añadir un subgrupo es superficie de fallo SILENCIOSO —un desajuste de orden muta la plaza equivocada conservando su id, sin error—. El servicio nuevo muta el agregado y REUTILIZA la guarda, que es lo que se quería del PUT.
  **M4 DE CONTRASTE: SEIS HALLAZGOS, Y EL PRIMERO HABRÍA MORDIDO EN LOS DOS NIVELES CON MÁS BLOQUES.** (1) La regla de sufijo escrita por el arquitecto —«lo que sigue al primer guion»— es FALSA: 7 grupos llevan guion en su propio código (1B-A…2B-C) y produciría sufijo erróneo en **107 de los 334 subgrupos**, generando `1B-E-A-Completo`; la regla correcta es quitar el prefijo `{codigoHermano}-`, y si algún subgrupo no lo lleva, ABORTAR. (2) La consulta «plazas que contienen un subgrupo» NO EXISTÍA —no hay `PlazaRepository`, `Subgrupo` no tiene lado inverso del M:N—, así que la lista de modificaciones cerrada en dos pasaba a tres. (3) La decisión no puede nombrar plazas por CÓDIGO: el único identificador expuesto está documentado como técnico interno, y además `PlazaRequest` y el ctor de `Plaza` se contradicen sobre su estabilidad; se nombra por `id`. (4) **I2 quedaba sin guarda en la ruta nueva** —`validarPlazas` sólo corre en el CRUD de actividad y `plaza_subgrupo` no lo impide—, y el arreglo no fue una validación más sino cambiar la UNIDAD DE DECISIÓN: no «a qué plazas va el grupo» sino «a qué plaza va cada espejo», con lo que I2 se vuelve inviolable por construcción. (5) Subir `exigirSinDependientes` a package-private no rompía ningún test, pero llamar así a un bean `@Transactional` no tiene precedente en el repo; se sube a `public` y la incógnita desaparece sin escribir un test que pruebe el framework. (6) El constructor de `ReferenciaEntranteException` revienta si todos los conteos son 0, así que hay que filtrar antes o el 409 se convierte en un 400 con mensaje interno.
  LO CONSTRUIDO. Sub-recurso `GET`/`POST /api/grupos/{id}/replicacion`, molde `PdcController`: `ReplicacionService`, `ReplicacionController` y cinco records. **Dos llamadas y no un alta compuesta**: si la réplica falla queda un grupo pelado, que es exactamente lo que el producto produce hoy al crear cualquier grupo y que `GrupoService.borrar` sí deja borrar. Falla en seguro sin tocar `GrupoService`. Cuatro modificaciones al código existente y sólo cuatro: `exigirSinDependientes` a `public`; `ActividadRepository.findConPoblacionPorGrupo`; `Plaza.agregarSubgrupo`; y el javadoc de la deuda. **Desviación demostrada, no opinada:** la consulta escrita por el arquitecto con el filtro en el `where` TRUNCA las colecciones fetchadas —`U` valdría siempre `{hermano}` y los 39 bloques saldrían replicados—; aplicada literalmente, 6 de 10 tests caen y `.reparto` sale vacío. Se movió el filtro a subconsulta y se usa `left join fetch` para que una plaza sin subgrupos no desaparezca de la colección en silencio.
  M3, CAMPAÑA DE 10 MUTACIONES, todas mordiendo su test por la vía prevista y todas restauradas por DIFF contra copia previa, nunca por el verde de la suite. **Y una autocorrección que es material de método: la primera corrida informó que M9 no ponía nada en rojo, y el superviviente no existía: el `grep` que filtraba la salida medía otra cosa.** Aislada, M9 tumba T9, T1 y T10. Es la cuarta instancia registrada de la forma de S117 (`EXIT=$?` tras un `echo`), S121 (el token inexistente) y S122 (el contador sobre el `<td>`) —el instrumento se apoya en algo que SE PARECE al conjunto medido y no lo es— y **la primera que ocurre DENTRO de la propia campaña de mutación**, es decir en el instrumento que existe para desconfiar de los demás.
  EL REMATE, Y ES UN DEFECTO INTRODUCIDO AL PASAR DEL CONTRATO AL GUION. El contrato exigía que la plaza de una asignación fuera «una vía donde el original podría estar»; el guion de implementación escribió sólo la primera mitad de la condición, y Claude Code implementó lo escrito —correctamente— y lo señaló. Resultado: el espejo del bloque A podía cablearse en una vía del bloque B. Se corrige DENTRO del Cambio y no se registra como deuda (M5: no es permiso para bajar el listón dentro de un cambio), con T11 y M10. De paso los dos métodos que alimentaban la validación se funden en `destinosDeReparto()`, cuyas claves SON el conjunto de espejos que el cuerpo debe nombrar, con lo que la lista de esperados y la de vías válidas ya no pueden desalinearse al tocar una sin la otra. **Y el refactor obligó a REVALIDAR M4, M6 y M7 sobre las anclas nuevas**, porque reescribir la función bajo un test no hereda su cobertura (M3): las tres siguen mordiendo.
  ESTADO INÉDITO, ASERTADO Y NO SUPUESTO: el alta crea el primer subgrupo con CERO plazas de toda la base (los 334 actuales están cableados). No lo rechaza nada —ni el dominio del solver, que exige ≥1 grupo y no ≥1 plaza, ni el mapper, ni las tres reglas de prevalidación— y entra como aserto explícito (T6, T10) en vez de como confianza.
  DEUDA. Nace UNA y se paga en la misma sesión: **`D-javadoc-plazas-caducado`** (técnica real, sede O-particiones), con TRES sedes —el javadoc de clase de `ActividadService`, el de `Actividad.actualizar`, ambos con un `{@link}` a `limpiarPlazas` que no existe, y la contradicción de `PlazaRequest` con el ctor de `Plaza` sobre la estabilidad del código de plaza—. **Se paga porque MORDIÓ EN ESTA SESIÓN**, no por limpieza de paso: esa contradicción llevó a escribir en el contrato que las decisiones nombraran plazas por código, y el contraste de M4 tuvo que corregirlo. Mismo argumento con que S137 pagó `D-catalogo-meta-enganosa`. **Deuda bloqueante del proyecto: 0.**
  LIMPIEZA (M1-bis): archivada S137 a `bitacora-sesiones.md` (promovida a `### Sesión 137`, insertada al final en orden ascendente, cuerpo íntegro); degradada S138 a «Última sesión registrada (previa):»; S139 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S136 a S137. R4/costura: **esta vez las suites SÍ se recorren**, porque por primera vez desde S132 se ha tocado `app/src`: **91 de Maven en solver, sin moverse, y 293 en app (282 + 11)**. `vitest` 419 NO se recorre y se declara por qué: `app/frontend/src` está intacto. El token nuevo nace con definición viva aquí y citante vivo en `gestion_proyecto.md` §4.
  **O-particiones (H2) sigue ABIERTO, con `C-alcance-particiones` y `C-replicación-alta` HECHOS.** De las tres condiciones del criterio: la **(1)** queda CUMPLIDA en servidor —incorporación automática en los 29 bloques replicados, cero decisiones—; la **(2)** a medias —el servidor acepta exactamente N asignaciones, pero «por pantalla» exige `C-alta-por-pantalla`—; la **(3)**, reversibilidad, entera por hacer y es `C-alta-reversible`. Nada verificado todavía contra la base de referencia: los 11 tests corren sobre fixture, y el gesto de prueba del criterio —alta real sobre el centro completo— no se ha ejecutado. **Aviso de operación:** en cuanto un grupo tenga subgrupos, `GrupoService.borrar` lo veta con 409, así que ninguna prueba del alta se hace sobre `app/educhronos-s137-centro-completo.db` sino sobre copia, o la base de referencia queda con un grupo imborrable dentro. O-ajuste-cierre (H1) sigue vivo, sin deuda que lo bloquee y con su criterio pendiente de reescribir, que es trabajo de su propio M0. Commits: `277a808` (producción + 10 tests), `fb2b71f` (remate: validación estrecha + T11) y `912bc63` (javadoc). Árbol limpio al cierre, con `554072c` como antepasado.

### Sesión 140 — O-particiones (H2): **EL ALTA REPLICADA YA SE PUEDE DESHACER, Y AL PROBARLA SOBRE EL CENTRO REAL SE DESCUBRE QUE LA CONDICIÓN (1) NO ESTABA CUMPLIDA.** Dos Cambios de hecho en uno: `C-alta-reversible` construido, y el defecto de S139 que el M4 destapó, reparado dentro del mismo Cambio. Primer ciclo alta→deshacer ejecutado sobre COPIA del centro completo, con dump idéntico. 302 tests en app (293 → 300 → 302). Ningún fichero de `solver/` ni de `app/frontend/` tocado.
  TIPO Y RITUAL: Desarrollo — M0 + M2 en una pasada de lectura + M3 con dos campañas de mutación + M4 sobre la base real + M1.
  M0. Cambio `C-alta-reversible`, CREADO en este M0 (precedente: S107, S115, S135, S136, S138, S139). Objetivo O-particiones, Hito H2. R-invalidación: ninguna. Se eligió sobre `C-alta-por-pantalla` por tres razones y la primera es operativa: hasta hoy el alta producía un grupo imborrable, así que cada iteración de la pantalla habría exigido restaurar una copia a mano. Higiene/Método descartada por R-deuda: **15 entradas contadas contra §4**, ninguna bloquea. Deuda bloqueante del proyecto: 0.
  **EL DISEÑO: SUB-RECURSO `DELETE /api/grupos/{id}/replicacion`, NO ENSANCHAR `GrupoService.borrar`.** El 409 del borrado de grupo protege un caso real —un grupo con un curso encima, en 24 subgrupos cableados, no debe desaparecer de un clic— y ensancharlo a cascada habría perdido esa protección para TODOS los grupos, no sólo para el recién replicado. El sub-recurso cierra además la simetría que S139 eligió a propósito: se sube con dos llamadas y se baja con dos.
  **EL DESHACER NO ES EL ESPEJO DEL ALTA, Y ES LA DECISIÓN DE DISEÑO DE LA SESIÓN.** `cablearReparto` eligió la vía con un `Map<String,Long>` que viene del cuerpo del POST y NO SE PERSISTE: la decisión no se puede recomputar. El deshacer LEE EL ESTADO —recorre buscando el espejo, no el original—, con lo que una sola travesía sirve igual para los 29 bloques replicados y para los 10 de reparto. Corolario medido después: limpia igual de bien lo que el alta cableó MAL, que es lo que permitió reparar el defecto de S139 sin dejar ninguna base atrapada.
  **EL DEFECTO VIVO DE S139, MEDIDO Y REPARADO. No era latente: eran 30 de las 219 actividades del centro.** `Actividad.plazas` es bolsa y `findConPoblacionPorGrupo` hace `left join fetch` de `p.subgrupos`; el `distinct` del HQL desduplica la RAÍZ, no la colección, así que `getPlazas()` repetía cada plaza una vez por subgrupo. Dos caras: el `GET` del plan reportaba 4 vías donde había 2, y —la grave— `clasificar` decidía «es bloque» con `getPlazas().size() > 1`, de modo que una actividad de UNA plaza compartida entre un ordinario y su PDC medía 2, entraba como bloque y `cablearReplicados` le metía el espejo. **En castellano: replicar 4ºE de 4ºA sentaba a 4ºE en la clase de Educación Física de 4ºA y su PDC**, en su aula y su tramo, y en silencio. Forma única: los cinco ordinarios con PDC (3ºA/3ºB/3ºC con 4 actividades cada uno, 4ºA/4ºD con 2). Lo cazó **G4, el aserto de producción** de este Cambio, que contaba tres retiradas donde había una. **Por qué S139 no pudo verlo: su fixture monta `ACT-UNICA` con UN solo subgrupo**, así que la forma «una plaza, dos grupos» —30/219 del catálogo real— no estaba reproducida y T10 pasaba por eso. Arreglo en `analizar`, deduplicando antes de clasificar; `clasificar` pasa a recibir plazas y reagruparlas, con lo que el recuento que decide es el de filas de `plaza`, que es lo que la regla nombra. No queda un solo `actividad.getPlazas()` fuera de `plazasDe`. `ActividadRepository` NO se toca: cambiar la consulta alteraría otras rutas y eso no está medido.
  **UN TEST ESTABA VERDE POR EL MOTIVO EQUIVOCADO, Y LO CAZÓ LA MUTACIÓN.** T20 capturaba un «antes» con `subgruposDePlaza(...)`, que recorre `findAll()` sin join: eso inicializa `Actividad.plazas` LIMPIA en el contexto de persistencia, y el `left join fetch` posterior no repuebla una colección ya inicializada. El POST nunca veía la bolsa inflada, así que T20 pasaba con la corrección y sin ella. Quitado el «antes» y escrito el estado esperado literal, cae con la mutación. El motivo queda en su javadoc: es una trampa que reaparece en cuanto alguien añada un «leer el estado previo» a un test de esta clase. **Es la razón de ser de M3 en su forma más limpia: sin la campaña se habrían entregado dos tests verdes de los cuales uno no medía nada.**
  M4 SOBRE EL CENTRO REAL, en copia (`app/educhronos-s140-m4.db`) y con el md5 de la referencia verificado al abrir y al cerrar. **El clasificador de S139 se contrasta por primera vez fuera de fixture: 6 replicados / 1 de reparto en 4º ESO, clavando a S138.** Y la corrección se demuestra necesaria de paso: `EFis-4ºA+4ºADi` y `TUT4-4ºA+4ºADi` ya NO aparecen en el plan; antes habrían salido como replicados y el recuento habría dado 8. Ciclo completo: 24 subgrupos creados, `4ºE-Completo` con CERO plazas —el estado inédito que S139 asertó—, `DELETE /replicacion` con 24 borrados y 30 descableados (la diferencia son nueve espejos en dos plazas y el `-Completo` en ninguna), y `DELETE /api/grupos/29` respondiendo **204** —no 200, que es lo que el encargo esperaba: `@ResponseStatus(NO_CONTENT)` en `GrupoController:90`—. **Dump antes/después IDÉNTICO, 1268 líneas contra 1268, cero filas sin deshacer.** La reversibilidad alcanza también a los ids: el esquema declara `id integer, primary key (id)` sin `AUTOINCREMENT`, así que no hay `sqlite_sequence` que excusar y `max(subgrupo.id)` vuelve a 334.
  **EL CRITERIO, RECONTADO.** La condición **(1)** pasa de «cumplida en servidor» (S139) a CUMPLIDA DE VERDAD, y el tramo intermedio queda escrito: estuvo declarada cumplida y rota durante una sesión. La **(3)** queda CUMPLIDA y verificada por comparación sobre el centro real. La **(2)** sigue pendiente de `C-alta-por-pantalla`, y su texto se PRECISA: S138 contó BLOQUES de reparto (1 en 4º ESO) y el M4 de S139 cambió la unidad de decisión a espejo-por-plaza, de modo que ese único bloque exige 4 asignaciones. Las dos cifras son correctas y miden cosas distintas; el texto se escribió antes del cambio de unidad. No cambia lo que hay que construir, pero sin la precisión la pantalla se declararía incumplida sin razón.
  DEUDA. **No nace ninguna con token nuevo.** `D-guion-exit-enmascarado` suma TRES instancias, las tres del arquitecto y las tres en guiones de lectura: (1) `CREATE TABLE` en mayúsculas contra un `schema.sql` en minúsculas, con `AWK_EXIT=0` y una sección falsamente vacía que, leída como esquema sin cascadas, habría mandado el diseño en dirección contraria; (2) un `awk` que enganchaba en la primera APARICIÓN del nombre —la llamada— y no en la declaración, volcando el final de otros tres métodos; (3) una función `anclar` que no distinguía «ancla que debe existir» de «ancla que debe faltar» y abortó por un `[0]` que era el resultado esperado. R-deuda aguanta. **`D-arranque-no-literal`, instancia nueva y de forma inédita: el PUNTERO caducó.** Su ficha afirma que el literal vive en la entrada de S130 del plan; esa entrada se archivó y hoy está en `bitacora-sesiones.md:8812`. La deuda cuyo diagnóstico es «lo que se describe y no se cita no se puede verificar después» tiene ahora el mismo problema un nivel más arriba. **El puntero se corrige en este cierre**, porque es una línea y porque la siguiente sesión que arranque la app tropieza igual; la deuda sigue viva (su arreglo es un `.sh` versionado). **Deuda bloqueante del proyecto: 0.**
  OBSERVACIÓN QUE NO ES DEUDA: la mitad **(b)** de G2 queda escrita SIN test discriminante, deliberadamente. No es rama muerta: es aserción sobre una invariante que hoy sostiene `subgruposDe` —filtra por «contiene este grupo», luego un subgrupo mono-grupo tiene forzosamente ese—, así que cualquier test sería verde con la línea y sin ella, que es peor que no tenerlo. Misma naturaleza que G4. **Si el arreglo de S139 hubiera tocado cómo se resuelven los subgrupos del grupo, (b) pasaría a ser alcanzable y entonces sí llevaría test**; no lo tocó. Queda anotado en el cuerpo de `dba98a0`. Y una lección de la propia campaña: la tabla de mutación decía «Quitar G2 → T14» mutando la LLAMADA entera, lo que ocultaba que una de las dos mitades no estaba discriminada; medirla por mitades es lo que lo destapó, y es la forma de S139 —el instrumento que mide algo que SE PARECE al conjunto medido— dentro de la propia campaña.
  MÉTODO. Tres normas del prompt de apertura quedan medidas contra la práctica y PIERDEN, y todas son material de la sesión de Higiene/Método: la norma de documentación derogada por M-doc en S122, viva en el prompt desde hace 18 sesiones; el mensaje de commit «de UNA sola línea», que contradice la convención de los cuatro commits anteriores del repo —`type(scope): asunto (Sxxx)` con cuerpo y dos trailers— y que se abandonó a favor de la del repo con el argumento de reutilización; y la tabla de Tipos condicionando M4 a «>1 módulo», ya registrada por S139. **El cuerpo del commit resultó ser la sede correcta para las tres cosas sucias del Cambio** (el defecto de S139 con sus cifras, el `flush()` no discriminado y G2(b) sin test): un commit que sólo cuenta lo limpio miente por omisión.
  EL `flush()` NO ESTÁ DISCRIMINADO Y SE CONSERVA. Quitarlo no tumba nada: Hibernate ya emite las actualizaciones de colección antes que los borrados de entidad. Se deja puesto —depender de ese orden implícito es lo que costó la lección de S73, y `PdcService.borrar` lo escribe igual— pero el comentario dice LO MEDIDO en vez de afirmar que es obligatorio, que es lo que el arquitecto había escrito sin medir.
  LIMPIEZA (M1-bis): archivada S138 a `bitacora-sesiones.md` (promovida a `### Sesión 138`, insertada al final en orden ascendente, cuerpo íntegro); degradada S139 a «Última sesión registrada (previa):»; S140 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S137 a S138. R4/costura: **suites recorridas, porque se ha tocado `app/src`: 91 en solver (sin moverse) y 302 en app**, con `surefire-reports` borrado antes y confirmadas por las dos vías. `vitest` 419 NO se recorre: `app/frontend/src` sigue intacto desde S132. Ningún token nuevo que censar. Commits: `dba98a0` (el deshacer) y `faa7a82` (la reparación del defecto de S139, enmendado para bajar cuatro citas de `S141` a `S140` y para añadir cuerpo y trailers; etiqueta `s140-fix-pre-amend` conservada sobre el original).
  **O-particiones (H2) sigue ABIERTO, con `C-alcance-particiones`, `C-replicación-alta` y `C-alta-reversible` HECHOS, y le queda UN Cambio: `C-alta-por-pantalla`.** Las condiciones (1) y (3) del criterio están CUMPLIDAS y verificadas sobre el centro real; la (2) es enteramente lo que falta. O-ajuste-cierre (H1) sigue vivo, sin deuda que lo bloquee y con su criterio pendiente de reescribir, que es trabajo de su propio M0. Árbol limpio al cierre, sin push, con `72936e8` como antepasado en `origin/main`.

### Sesión 141 — O-particiones (H2): **EL ALTA REPLICADA LLEGA A LA PANTALLA Y EL CRITERIO QUEDA CUMPLIDO SOBRE EL CENTRO REAL. O-particiones CIERRA y con él CIERRA H2, el primer hito terminado del proyecto.** Un grupo nuevo de 4º ESO entra en el curso desde la interfaz con 4 decisiones y cero edición subgrupo a subgrupo, y se deshace sin dejar rastro. Ciclo de TRES dumps —antes != pico, pico != después, antes == después— que es la primera verificación CONCLUYENTE de reversibilidad del proyecto. vitest 419 -> 441 (primera vez que `app/frontend/src` se toca desde S132); app 302 -> 303; solver 91 sin moverse.
  TIPO Y RITUAL: Desarrollo — M0 + M2 + M4 de contraste + implementación + M3 con dos campañas + M4 sobre el centro real + M1.
  M0. Cambio `C-alta-por-pantalla`, CREADO en este M0 (precedente: S107, S115, S135, S136, S138, S139, S140; la ficha de O-particiones no tenía bullet «Cambios que agrupa», igual que le pasó a O-demo en S115). Objetivo O-particiones, Hito H2. R-invalidación: ninguna —O-navegación (S127) y O-diseño (S133) están cerrados y H3/H4 no tocan esta superficie—. Higiene/Método descartada por R-deuda: **15 entradas recontadas contra §4**, ninguna bloquea. Deuda bloqueante del proyecto: 0.
  **EL ALCANCE INCLUYE EL DESHACER, Y LO DECIDE UNA MEDICIÓN, NO LA AMBICIÓN.** El M0 lo había dejado fuera por R-terminado —la condición (3) estaba CUMPLIDA desde S140—. El M2 lo revierte: `GrupoService.borrar` veta con 409 un grupo con subgrupos, la replicación mete al grupo nuevo en 24, y el frontend NO TENÍA NADA de replicación (`grep -rn -i replicacion app/frontend/src` -> exit=1). Entregar el alta sin el deshacer habría fabricado, desde la pantalla y de un clic, un estado del que el usuario no sale por producto: la enfermedad exacta de `D-horario-irreversible`. Queda FUERA, en cambio, un «deshacer» permanente en la lista de grupos: no existe lectura que responda «¿este grupo tiene espejos?» —el GET exige `?hermano={código}`— así que exigiría endpoint nuevo.
  **EL M4 DE CONTRASTE PAGA DOS VECES, Y LA SEGUNDA CAMBIA EL CAMBIO.** (1) `ViaDTO` no lleva profesor ni aula, y `asignatura` es el CÓDIGO, no el nombre. Se decide NO ensanchar: el dato está a mano (la sesión está abierta, `planificar` es `@Transactional(readOnly=true)`) pero el precio es un N+1 más gordo o ensanchar `findConPoblacionPorGrupo`, que es LITERALMENTE el gesto que produjo el defecto de S140. Nace `D-via-sin-quien-ni-donde`. (2) **El plan no ataba los espejos a su bloque**: `destinosDeReparto()` recorre `plaza.getSubgrupos()`, dato que no viajaba, y `validarAsignaciones` exige EXACTAMENTE ese `keySet()`, así que mandar `subgruposACrear` entero tampoco valía. Sin ese enlace, «un selector por espejo» no era implementable y caían 6 de los 19 casos. **La salida elegida no fue ensanchar `ViaDTO` con lo que faltaba, sino PUBLICAR el mapa que el servidor ya construía**: si el plan publica el mismo conjunto contra el que el POST valida, el cliente NO PUEDE componer un cuerpo inválido. Sin tocar repositorios, sin consulta nueva.
  **Y DE PASO SE CORRIGE UN ERROR DEL PROPIO CONTRATO:** el arquitecto había escrito «opciones = las vías del bloque», y es falso —`Destino.plazas()` acumula las vías de TODOS los bloques en que aparece un espejo, y el espejo recibe UNA sola plaza—. La unidad de decisión es el ESPEJO, que es lo que S139 fijó al hacer I2 inviolable por construcción; el contrato la había vuelto a poner en el bloque.
  **LA COSTURA, MEDIDA CON CONTROL EN VEZ DE ATADA CON JAVADOC.** La primera entrega dejó `espejosDe` como SEGUNDA travesía de la misma regla, atada al mapa por comentario. S139 había fundido esos dos caminos a propósito, con el argumento de que así «no pueden desalinearse al tocar una sin la otra». Se funde: `espejosDe` pasa a ser el bucle interno de `destinosDeReparto`. **La fusión no es preferencia de estilo y se demuestra:** mutando `espejosDe`, el montaje separado caza 1 test y el fundido 12. No se escribe test de igualdad entre el conjunto publicado y el `keySet()`: fundidos son la misma travesía, así que sería verde con la línea y sin ella —mismo criterio con que S140 dejó G2(b) sin test—.
  **M3, DOS CAMPAÑAS, Y UNA TRAMPA QUE `metodo.md` NO CUBRÍA.** Trece mutantes en el diálogo y uno en Java, cada uno cayendo con su caso. La primera pasada NO verificó las restauraciones por diff —se restauró con `cp` y se difó una sola vez al final— y al rehacerla apareció la forma nueva: el arnés restauraba con `shutil.copy2`, que PRESERVA la mtime, el fuente limpio quedaba más viejo que el `.class` mutado, Maven no recompilaba y la suite corría contra el bytecode mutado con el fuente correcto delante. Doce fallos atribuidos a la fusión que eran basura. **En Java el diff del FUENTE no basta: hay que invalidar el artefacto.** Es la trampa de S110 movida del fuente al `.class`, y entra como precisión a M3.
  **DOS SUPERVIVIENTES, Y NINGUNO LO HABÍA PEDIDO LA TABLA.** (a) El caso (20) NO EXISTÍA: había veinte `it` porque el (14) se partió en dos, no porque hubiera un vigésimo requisito; la unión de bloques estaba implementada y documentada SIN caso, y `.slice(0,1)` sobrevivía en verde. (b) Quitar entero el filtro de bloques —ofrecer a cada espejo las vías de TODOS los bloques de reparto— tampoco lo cazaba nadie, porque en los dos fixtures todo espejo estaba en todos sus bloques. Es la dimensión CONTRARIA a la del (20): que la unión no se pase, no que se quede corta. Nacen (20) y (21), con un espejo mono-bloque añadido al fixture. El caso (10) SÍ discrimina —mandar el código del propio grupo como hermano lo pone rojo y sólo a él— pero su otra mitad, espejo contra original, es INDISCRIMINABLE: el código del original no viaja en el plan y ningún fixture puede provocar esa confusión. Queda escrito en su comentario en vez de fingir cobertura.
  M4 SOBRE EL CENTRO REAL, en copia (`app/educhronos-s141-m4.db`), con el md5 de la referencia verificado al abrir, al construir y al cerrar. **El gesto son CUATRO pasos y no dos: crear 4ºE -> replicar desde 4ºA -> deshacer -> borrar 4ºE.** El sub-recurso cubre el segundo y el tercero; el «Borrar» de la lista hace el cuarto, y sólo funciona porque el deshacer dejó antes el grupo pelado. **El pico, medido y no supuesto: 79 líneas añadidas y cero quitadas** —24 `subgrupo` (ids 335-358 correlativos), 24 `subgrupo_grupo`, 30 `plaza_subgrupo`, 1 `grupo_administrativo`—. 23 espejos cableados (16 en una plaza, 7 en dos) y **`4ºE-Completo` en CERO plazas**, que es lo que T20 congela sobre fixture y lo que S139 asertó como estado inédito: primera verificación sobre datos reales. **I2 intacto sobre el centro real:** ningún espejo en dos plazas de la misma actividad. Clasificación real 6 replicados / 1 de reparto, clavando a S138 y a S140. El deshacer es exactamente inverso por tabla y no añade una sola línea. El hermano fue **4ºA**, dato que la BASE NO PUEDE DISTINGUIR —4ºA y 4ºB casan los 24 sufijos y aparecen en las 30 plazas— y cuyo único testigo es la pantalla.
  **EL CRITERIO, CERRADO.** (1) CUMPLIDA y ahora también desde la pantalla. (2) CUMPLIDA: 1 bloque, 4 decisiones, cero subgrupos tocados a mano. (3) CUMPLIDA con el montaje de TRES dumps, que es el que de verdad concluye: el de S140 —antes/después— no distinguía «ida y vuelta» de «nada», porque sin `AUTOINCREMENT` en el esquema no hay marca de agua que delate el paso. **O-particiones TERMINADO. H2 TERMINADO.**
  DEUDA. Nacen DOS, las dos sin sede porque colgarlas de un objetivo que cierra hoy equivale a decidir que no se pagan nunca (criterio de `D-instrumento-criterio4-ciego-al-ancho`). **`D-espejo-vía-sin-coincidencia`**: el desplegable ofrece las CINCO vías del bloque a cada espejo sin relacionar su nombre con la asignatura de la vía, ni sugiriendo ni advirtiendo; el arquitecto, conduciendo el M4 por primera vez, contestó bajando por la lista y mandó `4ºE-TEC` a una vía de FOPP, aceptado en silencio por pantalla y servidor. **`D-via-sin-quien-ni-donde`**, con su presión ya medida a la baja: las dos vías de EXPRE SÍ se distinguieron por sus grupos actuales, así que la carencia no mordió en 4º ESO. Y TRES fichas pierden su sede candidata al cerrar el objetivo: `D-avisos-como-bloque-fijo`, `D-plazas-ocultas-solo-al-arrastrar` y `D-itinerario-como-grupo`.
  MÉTODO. **`D-guion-busca-token-esperado` suma CINCO instancias, las cinco del arquitecto y todas de la misma forma: un recuento o una derivación AFIRMADOS por razonamiento teniendo el fichero a un turno de distancia.** Dos `grep` de un solo salto que dejaron fuera justo el record que la sección existía para volcar (`PlazaDescableadaDTO` en M2, `ViaDTO` en M4); «opciones = las vías del bloque»; «seis guardas inalcanzables y queda una» cuando son cuatro y quedan tres —las dos de `derivarEspejos` son condiciones del catálogo y ningún filtro de pantalla las excluye—; y «S140 midió 1268 líneas» contra las 2505 que salieron, dos volcados de alcance distinto arrastrados como si fueran el mismo instrumento. **Y `D-guion-exit-enmascarado` suma una, de Claude Code y en la sesión que llevaba citándola desde el M2:** `mvn ... | sed > fichero; echo $?` lee el estado de `sed`, y salió exit=0 sobre un BUILD FAILURE. R-deuda aguanta; ninguna bloqueaba nada.
  DIVERGENCIAS DEL ENCARGO, TODAS ACEPTADAS Y TODAS MEJORES QUE LO ESCRITO. «Plan vacío» se implementa como «no haría nada» y no como «cero bloques»: un plan con espejos y sin bloques SÍ hace algo —crea el `-Completo` sin cablear, que es el comportamiento intencionado—, y bloquear por «cero bloques» lo habría matado. Los tipos del modelo pierden el sufijo `DTO`, como los otros trece de `models/`. `HttpTestingController` y no doble de servicio, que es lo único con lo que los casos pueden afirmar sobre rutas y AUSENCIAS. Señales con `(change)` y no Reactive Forms (precedente `horario-view.html:13`), porque la validez de un `FormGroup` no es una señal en una app zoneless. `<optgroup>` por actividad, porque con dos bloques dos vías distintas serían las dos «vía 1». Y `grupo-lista.spec.ts` hubo que tocarlo, forzado: su caso (7) fija los rótulos por igualdad de lista completa.
  LIMPIEZA (M1-bis): archivada S139 a `bitacora-sesiones.md` (promovida a `### Sesión 139`, insertada al final en orden ascendente, cuerpo íntegro); degradada S140 a «Última sesión registrada (previa):»; S141 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S138 a S139. R4/costura: suites recorridas por las tres vías, **441 de vitest, 303 en app y 91 en solver**, con `surefire-reports` y `target/classes` borrados antes. `solver/src/main` NO tocado, luego `referencia-codigo-solver.md` no se regenera. **El censo R4 subió de 26 a 29 sospechosos por el archivado, y esta misma frase lo devolvió a 26: la anotación se desmintió a sí misma y ahí está el hallazgo.** El censo mira sólo el corpus vivo —plan, gestión y método—, así que al salir la entrada de S139 tres tokens se quedaron con una sola cita: `C-alcance-particiones`, `D-asignatura-sin-nivel` y `D-javadoc-plazas-caducado`. Nombrarlos aquí para documentarlo les dio la segunda, porque **el instrumento no distingue una definición de una cita de trabajo de una MENCIÓN en una nota de cierre**. El 29 fue real y ya no es reproducible; lo que un lector medirá hoy es 26, y el motivo queda escrito en vez de dejar un número que el propio verificador contradice (que es la familia de `D-declarado-sin-artefacto`). Consecuencia que sobrevive a la frase y que es la que importa: esta mención queda como citante vivo PERMANENTE de los tres, así que si alguno pierde su cita real, R4 no avisará. Nace `D-censo-r4-cuenta-menciones`.
  **O-particiones (H2) ✔ TERMINADO. H2 ✔ TERMINADO, el primer hito del proyecto.** El siguiente por §5 es el cierre de H1: **O-ajuste-cierre sigue vivo, sin deuda que lo bloquee, y su «Terminado cuando» sigue siendo INVÁLIDO** —reescribirlo sobre los huecos que destape un guion de aceptación de H1 es trabajo de su propio M0, con el precedente de forma y coste de S138—. Deuda bloqueante del proyecto: 0.

### Sesión 142 — O-ajuste-cierre (H1): **EL CRITERIO SE REESCRIBE SOBRE MEDICIÓN, Y EL OBJETIVO NO SE REDUCE: SE AGRANDA. «Ajustar a mano» no existe —una fila de `sesion` es inmutable e imborrable por la API—, así que el criterio 1 de Fase 8 no estaba incumplido: era inmedible tal como estaba escrito.** El criterio 4 queda CUMPLIDO y verificado por mutación sobre el centro real contra 96 % de ruido de fondo. Cuatro generaciones, 40 minutos de solver, ni un fichero de código tocado.
  TIPO Y RITUAL: MEDICIÓN — M0 + M2 en pasadas de solo lectura + M4 conducido por el arquitecto en navegador + M1. Sin M3 (no se muta lógica de producto). Cuarto caso del tipo tras S136, S138 y S139; sigue sin fila en la tabla de Tipos de `metodo.md`.
  M0. Cambio `C-alcance-ajuste`, CREADO en este M0 (precedente: S107, S115, S135, S136, S138, S139, S140, S141). Objetivo **O-ajuste-cierre**, Hito **H1**. R-invalidación: ninguna —O-navegación (S127) y O-diseño (S133) cerrados; H3 y H4 no tocan esta superficie—. **Primer M0 del proyecto que elige entre FRENTES y no entre Cambios**, porque H2 cerró en S141 y no hay objetivo abierto que continuar. **H3 descartado con argumento:** no tiene ningún objetivo escrito en §3, así que M0 no puede nombrar el término leyéndolo, y §5 fija el orden H2 → cierre de H1 → H3 sin que haya hecho nuevo que lo invierta —al contrario: cerrar H2 consumió la razón por la que §2 daba a H1 valor BAJO—. **Higiene/Método descartada, y el argumento de siempre ya no vale:** sin objetivo activo, «ninguna deuda bloquea el objetivo activo» es una frase vacía; la razón real es R-apertura, que no puede nombrar los tres términos.
  **EL RECUENTO DE HIGIENE NO REPRODUCE EL NÚMERO HEREDADO, Y ESO ES EL PRIMER HALLAZGO.** Bajo criterio escrito —sede «sesión de Higiene/Método» o «el script de R4 pendiente desde S101»— salen **15 entradas hoy**: doce explícitas más `D-guion-exit-enmascarado`, `D-arranque-no-literal` y `D-censo-r4-cuenta-menciones`. Pero la tercera **nace en S141**, así que antes de S141 este mismo criterio daba 14 y S140 y S141 contaron 15. O aquel recuento incluía las tres «Transversal, sin objetivo asignado» que este excluye, o no era reproducible. No se supone cuál: se deja escrito el criterio para que el siguiente recuento sea comparable, que es lo que ninguno de los anteriores dejó.
  **COSTURA MEDIDA EN EL M0: las fichas de las dos deudas de guion no registran sus propias instancias.** `D-guion-busca-token-esperado` decía «TRES instancias» (S135) cuando el plan contaba ocho; `D-guion-exit-enmascarado` se quedaba en cinco casos (S121) sin las tres de S140 ni la de S141. El recuento vivía sólo en el plan y en la bitácora, y §4 es donde un lector iría a buscarlo. Es la familia de la propia deuda: un instrumento que no cuenta lo que dice contar. Ambas fichas corregidas en este cierre (R5).
  **EL BANCO.** Copia `app/educhronos-s142-m2.db` desde la referencia `educhronos-s137-centro-completo.db`, con md5 `64d671fe…01fc7` verificado al abrir, durante y al cerrar; **intacta**. El literal de arranque se LEYÓ de la bitácora en vez de reconstruirse por séptima vez, y el rastreo destapó que hay **dos versiones vivas que no dicen lo mismo**: `plan:2704`, dentro de la ficha de `D-arranque-no-literal`, era la de UN paso y conservaba el hueco `<ruta absoluta>` sin rellenar —exactamente lo que la deuda denuncia—; la buena es la de TRES pasos de `bitacora:8811` (S130), ratificada en S133. **El puntero se corrige aquí y la deuda sigue viva**, mismo criterio con que S140 corrigió su puntero caducado.
  **CUATRO GENERACIONES, 819 SESIONES LAS CUATRO VECES.** H1 sin pin: `FEASIBLE`, objetivo 215,0, 602 s. H2 con el pin en el tramo 10: 225,0. H3: **no lo lanzó nadie a propósito** —el arquitecto pulsó «Generar horario» en la vista y no había forma de parar—, cuerpo `{}` y nombre por defecto, 222,0; resultó ser una repetición gratuita del caso de control. H4 con el pin mutado al tramo 34: 217,0, primera corrida con cota inferior no nula (1,0). El oráculo de 819 se clava las cuatro veces, tercera y cuarta confirmación de la cifra derivada del catálogo en S135.
  **EL CRITERIO 4, CUMPLIDO, Y EL MONTAJE ES LO QUE LO PRUEBA.** La primera corrida con pin NO discriminaba y así se registró: se pinchó donde el solver ya había puesto la instancia, y el índice 3 se quedó quieto SIN pin, de modo que «quedarse» era compatible con «el pin sujeta», con «habría caído ahí igual» y con «el pin se perdió en silencio» —hipótesis nada teórica: el javadoc de `BloqueoMapper` advierte de que el cruce por identidad de objeto pierde el pin sin excepción y sin traza—. **Lo decide la MUTACIÓN:** pin movido al tramo 34, y la instancia y sus TRES plazas aterrizan en el 34, con el 96 % de las 819 sesiones moviéndose entre corridas por puro ruido (`D-generacion-no-reproducible`). Tres corridas de control en el 10, una mutada en el 34. **NOTA DE ALCANCE:** verificado el pin de TRAMO; el de AULA no se ejercita —P3 ya estaba en A17 en las cuatro corridas— y sólo alcanzaría al 13 % de las sesiones (109 de 819; 279 plazas de aula fija contra 37 variables). No se ejercita a propósito: el criterio dice «una sesión bloqueada no se mueve», y la sesión es el cuándo (R-terminado).
  **EL CRITERIO 1 NO ESTABA INCUMPLIDO: ERA INMEDIBLE, Y ES EL HALLAZGO DE LA SESIÓN.** Una fila de `sesion`, escrita, es **inmutable e imborrable por la API**, confirmado por tres vías convergentes: no existe `SesionController` entre los trece controladores; `HorarioController` expone POST y dos GET, sin PUT, PATCH ni DELETE; y la única escritura sobre `sesionRepository` en todo `src/main` es el `saveAll` de `GeneradorHorarioService:277`, dentro de `guardar()`. En pantalla se ve lo mismo: **soltar no reubica, soltar PINCHA** —`horario-view.ts:284` manda `POST /api/bloqueos` con `aulas: []`— y la rejilla no se mueve. El horario sólo cambia regenerándolo entero: diez minutos y el 96 % de ruido. **Tres sedes describían una capacidad que el backend no tiene**: el paso 4 del estado final en §1, la fila de H1 en §2 y el entregable de Fase 8, las tres con «drag & drop para reubicar». Las dos primeras corregidas en este cierre.
  **Y HAY UNA RESTRICCIÓN DE DISEÑO QUE EL CRITERIO NUEVO RESPETA EN VEZ DE ATROPELLAR.** El javadoc de `slotsOcupados` es tajante: la marca `.ocupado` afirma sólo «en este tramo ya hay clase EN LA VISTA ACTUAL», es ciega por construcción a los recursos que la vista no muestra, y **no debe crecer hacia una verificación** porque sería un cuarto espejo de las restricciones. El argumento es bueno. La reconciliación se escribe en el criterio: el veredicto lo emite el SERVIDOR al intentar el movimiento y la rejilla lo muestra sin calcularlo.
  **EL CRITERIO 2 SE CUMPLE POR UNA VÍA DISTINTA DE LA QUE EL CRITERIO DESCRIBE.** No existe selección de celda —en toda la rejilla hay un solo `(click)`, el del despinado, y su comentario dice que el `stopPropagation` es defensivo precisamente para que nadie confunda despinar con seleccionar—. No hace falta: la rejilla recibe las violaciones como input y las marca TODAS a la vez por clase CSS, con el detalle en `title`. La información está y es alcanzable. Lo que falla es CÓMO: sólo con el ratón encima, sin foco de teclado. Eso es accesibilidad y no hueco funcional; sede O-diseño, fuera del criterio.
  **EL CRITERIO 3 FALLA POR EL LADO CONTRARIO AL QUE ANTICIPABA.** No hubo INFEASIBLE opaco: hubo **«Catálogo sano: sin hallazgos de pre-validación»** —`GET /api/prevalidacion` devuelve `[]`— sobre un centro con los 28 grupos a holgura cero desde S135, que a continuación consume los 600 s enteros y se queda en `FEASIBLE` con cota inferior 0. `PrevalidacionService` comprueba TRES reglas (`PROFESOR_SOBRECARGADO`, `REPETICIONES_EXCEDEN_DIAS`, `GRUPO_SOBRECARGADO`, :64-70) y **S8 no está entre ellas, siendo la ÚNICA invariante verificable sin solución** —`VerificadorSolucion:43,56` lo dice: «S8 es propiedad del catálogo, no del horario»—. Afirmar «sano» es afirmar más de lo que se ha mirado. Enlaza con `D-prevalidacion-ciega-a-holgura-cero`, y su presión está medida: S136 midió que los 7 tutores de Bachillerato son falsos y PORTANTES, sosteniendo S8 en seis actividades de bloque.
  **EL CUARTO HUECO NO VENÍA EN EL GUION: LO SUFRIÓ EL ARQUITECTO.** Un clic en «Generar horario» lanzó 600 s irreversibles sin confirmación y sin forma de parar; ese es el horario 3. **`ConfirmarGeneracion` EXISTE**, está montado desde `(click)="generar()"` y aseverado por su propio spec, y su texto dice la verdad —verificado: `GeneradorHorarioService:195-197` prevalida y aborta sin llegar al solver, y el 422 sale por `ResponseStatusException`, la vía que `FalloGeneracionDTO` documenta como muda—. Pero `generar()` (`horario-view.ts:342`) sólo lo abre si hay avisos de severidad `ERROR`, y con lista vacía va directo. **El diálogo está construido, aseverado e inalcanzable justo donde más falta hace.** Es la mitad de UX del mismo hueco que C3, no un token aparte, y es la condición 4 del criterio nuevo.
  **LO QUE ESTÁ BIEN, QUE TAMBIÉN SE CUENTA.** Sobre las 819 sesiones: cero profesores en dos sitios a la vez y cero aulas con dos sesiones en el mismo tramo. Las duras se respetan, tercera medición sobre este centro tras S119 y S135. Y el estado de espera de la generación aparece en pantalla, que es lo que S118 puso y `D-generacion-sin-indicador` describía.
  **INSTRUMENTOS PROPIOS QUE FALLARON, LOS TRES DEL ASISTENTE.** (1) Dos `grep` apuntaban a `app/frontend/src/app/components/horario`, **que no existe**, con el `2>/dev/null` tapando el exit 2: midieron cero pareciendo medir, y de ahí se habría concluido que no hay atribución ni arrastre. (2) Un `having n>=1` que lo cumple toda fila y contaba filas del join en vez de sesiones: los supuestos choques eran el abanico subgrupos×grupos. (3) Un chequeo de «28 grupos» para discriminar la base servida, cuando la referencia también tiene 28 —habría dado verde contra la base equivocada—; Claude Code lo sustituyó por el fd abierto en `/proc/<pid>/fd`, que sí discrimina. Las tres son `D-guion-busca-token-esperado`, y las tres las cazó Claude Code. Se registran porque un método que sólo anota los aciertos no mide nada (S138). En el M1, Claude Code detectó además el «~90 %» de H1 y las cinco filas de §4 rotas por pipes sin escapar que quedaban tras arreglar la que se le nombró.
  **`D-post-horario-sin-sesiones`: TERCERA Y CUARTA CONFIRMACIÓN, Y POR FIN LA CAUSA.** La ficha existía desde S114 y describía el síntoma sin explicarlo. Ahora está medido: `guardar()` (`GeneradorHorarioService:256`) hace `save` + `saveAll` y **nunca mantiene el lado inverso** de una `@OneToMany(mappedBy, LAZY)` creada con `new`; y `open-in-view` no está declarado en ningún `application.properties`, así que vale su defecto `true` y el `findById` de `proyectar()` devuelve la instancia cacheada en primer nivel. **El rastro que lo prueba:** `fechaGeneracion` viaja en nanos por el POST y en milis por el GET, truncada por SQLite. Corolario que corrige el repo: el comentario de `GenerarHorarioEndpointTest:58-66` afirma que la colección vacía es «artefacto del harness, no de producción» — **es falso**, con OSIV producción se comporta igual, y el `@DataJpaTest` reproducía lo real en vez de enmascararlo. Y el arreglo **ya está escrito en el repo**: `BloqueoService.aulasDe()` reconsulta el repositorio en vez de fiarse de la colección inversa, y por eso su POST sí devuelve el cuerpo completo. Ficha ampliada; NO se paga (R-deuda, y tocar OSIV es transversal). Sigue latente sólo porque `lanzarGeneracion()` descarta la proyección del POST y recarga por GET fresco, decisión de S93 tomada por otra razón.
  DEUDA. Nacen TRES tokens. **`D-parametrizacion-muerta-en-ui`**: `horario.service.ts:27` manda `{}` fijo, así que `nombre`, `maxSegundos`, `semilla` y `via` existen en la API —D29 dada por cerrada en su javadoc— y son inalcanzables desde la aplicación, porque el diálogo sólo devuelve `true/false`. Familia de `D-declarado-sin-artefacto`. Sin sede. **`D-bloqueo-id-no-estable`**: `guardar()` borra y reinserta, luego el id del bloqueo no es estable por contrato; que hoy volviera a salir 1 es reutilización de rowid de SQLite, no garantía. Sin sede. **`D-cast-sin-comprobacion-drop`**: `evento.item.data as InstanciaCelda` es un cast sin comprobar, válido sólo mientras el `cdkDropListGroup` conecte únicamente celdas de esta rejilla; riesgo ya declarado en el propio código. Sede O-ajuste-cierre, porque la condición 1 del criterio nuevo toca ese punto exacto. **Deuda bloqueante del proyecto: 0.**
  OBSERVACIONES QUE NO SON DEUDA. El dominio dice `bloqueo` y la UI dice `pin` para lo mismo: cualquier ficha con un solo término describe media superficie. El bundle inicial sigue en 554,18 kB sobre 550 (`D-bundle-presupuesto`, preexistente, O-diseño). Y `D-horario-irreversible` queda cuantificada: la copia acaba con **4 horarios y 3276 filas de `sesion`**, sin `DELETE /api/horarios/{id}`; la cascada existe en el esquema y nadie puede dispararla desde la API.
  SUITES: 91 (solver) + 303 (app) de Maven y 441 de vitest, **HEREDADAS de S141 y NO recorridas en S142**, que no tocó una línea de código. `solver/src/main` no tocado: `referencia-codigo-solver.md` no se regenera.
  LIMPIEZA (M1-bis): archivada S140 a `bitacora-sesiones.md` (promovida a `### Sesión 140`, insertada al final en orden ascendente, cuerpo íntegro); degradada S141 a «Última sesión registrada (previa):»; S142 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S139 a S140. R4/costura: sin código en el árbol, así que las suites de S141 —91 + 303 de Maven y 441 de vitest— siguen midiendo este árbol y NO se recorren. **Censo R4 de tokens nuevos:** los tres (`D-parametrizacion-muerta-en-ui`, `D-bloqueo-id-no-estable`, `D-cast-sin-comprobacion-drop`) nacen con DOS sedes cada uno —definición viva en §4 de `gestion_proyecto.md` y cita de trabajo en esta entrada—, que es lo que evita engordar `D-deuda-sin-sede-en-el-plan`. **HIGIENE DE TABLAS, de paso y por R5:** seis filas de §4 estaban rotas por pipes sin escapar y se renderizaban mal —`D-guion-exit-enmascarado` (15 columnas), `D-motivo-rechazo-sin-registro` (10), `D-error-generacion-pin` (6), `D-generacion-sin-indicador` (6), `D-gh6-tutor-contradictorio` y `D-configuracion-monolitica` (4 en tablas de 3)—; escapadas SIN alterar una letra del texto histórico, aseverado comparando ambas versiones tras normalizar los escapes. Todas preexistentes en HEAD; las cinco últimas las encontró Claude Code al verificar la primera.
  **EL CRITERIO, REESCRITO — y no reduce el objetivo: lo AGRANDA.** S138 midió y O-particiones se redujo a la mitad; aquí pasa lo contrario, porque la condición 1 es superficie nueva (endpoint, validación, persistencia y gesto). Se deja en este objetivo y no en uno aparte porque un objetivo llamado «el ajuste manual está completo» que excluyera recolocar una clase tendría el nombre mintiendo. Queda dicho para la métrica de §7: si al descomponerlo salen más de ocho sesiones, es que esconde dos objetivos.
  **O-ajuste-cierre (H1) sigue ABIERTO, ya con criterio VÁLIDO y una de sus cinco condiciones CUMPLIDA.** Sin deuda que lo bloquee. Deuda bloqueante del proyecto: 0.

### Sesión 143 — O-ajuste-cierre (H1), C-mover-sesion-backend: **EL ENDPOINT DE RECOLOCACIÓN EXISTE, ESTÁ MEDIDO Y ES CORRECTO, Y SOBRE EL CENTRO REAL NO PUEDE PRODUCIR NI UN SOLO MOVIMIENTO VÁLIDO.** Los 28 grupos están a 30/30 tramos y no hay plazas sin subgrupo: cualquier instancia, en cualquiera de los 29 destinos, choca con lo que su grupo ya tiene ahí. 59 PUT sobre el centro completo, 58 rechazos y una idempotencia; el fichero de la base queda byte a byte idéntico al de partida. app 303 -> 317.
  M0. Cambio C-mover-sesion-backend, creado en este M0 al descomponer el criterio reescrito en S142. La descomposición de las condiciones 1 a 4 —el primer trabajo que la ficha encargaba al M0— sale en TRES Cambios y no en cuatro: (1) y (2) son el mismo endpoint, su camino verde y su camino de rechazo, y se parten en backend y gesto por dependencia técnica real; (3) es propio. La condición (4) NO es Cambio: se arregla en `generar()` (`horario-view.ts:342`) abriendo siempre el diálogo, y cae en el camino del Cambio del gesto. **Y (3) y (4) NO son la misma familia a efectos de arreglo, contra lo que el traspaso de S142 daba por hecho:** S136 midió que los 7 tutores de Bachillerato son falsos pero PORTANTES, así que S8 pasa; meter S8 en la prevalidación seguiría devolviendo lista vacía y el diálogo seguiría inalcanzable. R-invalidación: ninguna. Higiene/Método pierde por R-deuda —vuelve a haber objetivo activo, y ninguna de sus 15 entradas bloquea las condiciones 1 a 4—.
  M2. TRES respuestas medidas. (1) La unidad que se mueve es la INSTANCIA, no la fila: `sesion` NO tiene `actividad_id` —su DDL es `(indice, aula_id, horario_id, plaza_id, tramo_inicio_id)`— y a la actividad se llega por `plaza`. 632 instancias por horario; 551 de 1 fila, y luego 27, 29, 4, 15 y 6 de 2 a 6 filas. **Filas por instancia == aulas distintas en las seis categorías:** cada plaza de un desdoble lleva su aula, así que mover arrastra hasta 6 aulas y el choque de aula es plural por construcción. 632/632 ocupan un solo tramo, PERO la columna se llama `tramo_inicio_id` y `PatronTemporal` con la lista blanca de D13 existen: el 632/632 mide ESTE centro, no el modelo. (2) El verificador se reutiliza como pieza de dominio —`VerificadorSolucion` es sin estado y `SolucionMapper` reconstruye desde las filas persistidas— pero `DiagnosticoService.diagnosticar(id)` NO, porque su firma sólo sabe leer lo persistido y no admite una solución hipotética. **`ReglaDura` tiene OCHO valores, no tres** (`INSTANCIA_SIN_COLOCAR`, `BLOQUE_IMPOSIBLE`, los cuatro solapes, `DISTRIBUCION_MISMO_DIA`, `TUTORIA_SIN_TUTOR`), de modo que el verificador cubre el inicio de bloque y la distribución y es puerta suficiente para las duras. (3) El molde de error es el MINORITARIO: `FalloGeneracionDTO`, no `ResponseStatusException`, que `HorarioController:78-85` ya documenta que deja el cuerpo mudo (`D-F8.6-ii-a`).
  MEDICIÓN DEL CICLO, previa al diseño y con arnés desechable. Ciclo completo de un movimiento validado sobre 819 sesiones, 5 pasadas con `em.clear()` entre ellas: mediana 250 ms, mínimo 170 ms. El reparto es lo que decide: `cargarProblema` se lleva el 79 % y `verificar` cuesta 8 ms. **La pregunta abierta —verificar entero o filtrar por celdas— queda contestada y era un no-problema:** dos verificaciones son ~16 ms, el 6 % del ciclo, así que filtrar no compra nada y además exigiría re-derivar la semántica de las restricciones fuera del verificador, que es el cuarto espejo que el javadoc de `slotsOcupados` rechaza por escrito. Umbrales declarados ANTES de medir para no ajustarlos al resultado: <300 ms el contrato sigue, 300 ms-2 s sigue con estado de espera obligatorio en el gesto, >2 s se reabre. Violaciones duras en reposo: 0, confirmando S142. Dato secundario no pedido que ordena el diseño: sin `em.clear()` la carga baja de 197 a 90 ms, luego **una petición paga UN solo `cargarProblema`** y el diff por diferencia no lo paga dos veces.
  CONTRATO. `PUT /api/horarios/{horarioId}/instancias` con `{actividadCodigo, indice, dia, orden}` en el CUERPO y no en la ruta: `actividadCodigo` lleva tildes, `+` y `º` (`EFís-4ºA+4ºADi`) y ya hay deuda viva sobre esos códigos (`D-codigo-actividad-tilde`); meterlos en un segmento es fabricar un problema de codificación por estética REST. Causas enumeradas: 400 TRAMO_INEXISTENTE, 404 HORARIO_INEXISTENTE, 404 INSTANCIA_INEXISTENTE, 409 VIOLA_REGLA_DURA con las violaciones NUEVAS, 409 INSTANCIA_PINADA. El aula no cambia al mover. Instancia pinada se rechaza sin tocar el pin: el pin es del CENTRO y dice dónde debe ir en la próxima generación, así que permitir el movimiento haría que la siguiente generación deshiciese el ajuste sin explicación.
  DECISIONES QUE LA IMPLEMENTACIÓN CORRIGIÓ, las cuatro medidas y no supuestas. (a) **Controlador propio y no un método más en `HorarioController`:** ampliar su constructor rompió la compilación de 5 specs ajenas que lo construyen a mano (`DiagnosticoRoundTripTest`, `GenerarHorarioEndpointTest`, `PrevalidacionEndpointTest`, `HorarioControllerHttpTest`, `MapeoFalloEndpointTest`). `MovimientoInstanciaController` comparte prefijo `/api/horarios` sin colisión y deja cero specs ajenas tocadas. (b) **El orden de las violaciones NO es estable entre pasadas** —`verificarNoSolapes` agrupa por `HashMap` y el orden de inserción depende de esa iteración—, así que la diferencia se hace por MULTICONJUNTO y nunca por posición. `Violacion` y `CeldaRef` sí son records con `equals` por valor en toda su profundidad, comprobado antes de apoyarse en ello. (c) Las reglas «pin -> 409» e «idempotencia -> 200» chocan cuando se pide su tramo actual a una instancia pinada; desempata el pin, y desde la rejilla el caso no ocurre porque `alSoltar` ya descarta el mismo slot. (d) `Sesion` no tenía ningún mutador: se le añade `moverA(TramoSemanal)` y sólo ese. **`D-post-horario-sin-sesiones` mordió en vivo durante el desarrollo:** la primera corrida dio 404 en todos los movimientos e `INSTANCIA_SIN_COLOCAR` en el diagnóstico porque `horario.getSesiones()` venía vacía. El servicio no usa la colección inversa en ningún punto: reconsulta para localizar, para reconstruir y para responder.
  COBERTURA. 14 casos, en dos commits. Los 11 del primero se verificaron con tres mutantes; los 3 del segundo cubren dos huecos que el encargo inicial dejaba abiertos y que ningún caso anterior veía: que un rechazo NO escriba, y que el aula se conserve. **El aserto del recuento total de `sesion` es lo único nuevo de verdad en los dos primeros** —lo dice el informe de Claude Code sin que nadie se lo preguntara—: la cobertura previa ya aseveraba «la fila sigue en su tramo original», que seguiría siendo cierta ante un mover implementado como borrar+reinsertar fallando entre medias. Dos precisiones del ejecutor mejores que el encargo: hicieron falta DOS mutantes de escritura prematura, uno por cada rama de salida, porque el pin se comprueba antes del veredicto y una escritura situada detrás de él nunca llega a ejecutarse en esa rama; y el aula se compara por pares `plaza->aula` ordenados y no por multiconjunto de aulas, que habría dejado pasar una permutación entre las 6 plazas.
  M4 SOBRE EL CENTRO REAL, por HTTP y SIN frontend: el arranque omite a propósito `mvn -pl app package -DskipTests` —el paso que construye la UI— para que no exista clic capaz de disparar una generación de 600 s, que es lo que ocurrió en S142. Base servida verificada por `/proc/<pid>/fd` y no por conteos, porque contar 28 grupos NO discrimina. Instancia elegida: `Bloque-AFAVS_CeH_DT_RefLe-4ºA+4ºB+4ºC+4ºD` índice 1, 6 filas, 6 aulas, JUEVES-1, ocupando 4 grupos. **Barrido de los 29 destinos: 29 de 29 rechazados**, cero 200, cero 400, cero 404. SOLAPE_AULA 143, SOLAPE_GRUPO 116, SOLAPE_PROFESOR 91, SOLAPE_SUBGRUPO 16. Los cuatro grupos salen en los 29 destinos (29x4 = 116): **la ocupación medida y el veredicto del endpoint coinciden sin holgura.** Idempotencia: 200 y 3276 filas antes y después. Rechazo sin escritura: las 6 filas en su tramo y sus 6 parejas `plaza->aula` intactas. Pin: 409 INSTANCIA_PINADA con `sesion_bloqueada` y `aula_bloqueada` idénticas byte a byte. **La prueba más dura no la da ningún test: tras 59 PUT, un arranque de Spring con `schema.sql` y un GET de proyección, el fichero de la base es byte a byte el mismo con el que empezó** (`5f02ef43b80887d5f33959a20801b1be`, idéntico al banco de S142).
  HALLAZGO, y vale más que el verde que no salió. **Sobre este centro no existe ningún movimiento válido, y no es un accidente del horario sino estructural:** 28 grupos a 30/30 tramos y cero plazas sin subgrupo, así que cualquier instancia movida a cualquier otro tramo choca con lo que su grupo ya tiene ahí. Medido en DOS tamaños —la de 6 filas y `AMO-1FPB` índice 1, de una sola fila—, 29/29 en ambos. No se fabricó hueco para producir un verde. Consecuencia sobre el producto, no sobre el código: **lo que este centro necesita para ajustar a mano no es mover, es INTERCAMBIAR dos instancias**, y un intercambio no se construye encadenando dos movimientos porque el estado intermedio es siempre inválido y lo rechazaría el propio veredicto. Nada del trabajo se pierde —la validación por diferencia, la transacción única, el servicio, los DTO y el molde de error son exactamente lo que un intercambio reutiliza—, pero hay que decirlo claro: el endpoint está terminado y correcto, y por sí solo no le sirve a este centro.
  CRITERIO. Condición (1) **NO cumplida y su enunciado es INMEDIBLE sobre el centro real**, por la misma razón de fondo que S142 desmontó en el criterio anterior: el verde sólo existe contra fixtures sintéticas. La mitad servidora está construida, cubierta y medida. La condición (2) tiene su mitad de SERVIDOR demostrada sobre datos reales —el veredicto lo emite el backend y nombra el recurso, 350 violaciones nombradas en el barrido— y su mitad de INTERFAZ pendiente. (3) y (4) sin tocar. (5) sigue cumplida. **Queda planteado para el M0 de S144, y no se ejecuta aquí:** reescribir la condición (1) para que su verificación sea el intercambio, y decidir si este objetivo son dos —el motor de ajuste y el cierre de H1 no son la misma cosa, y con el intercambio, su gesto y la prevalidación S8 quedan cuatro Cambios más el hecho, aún por debajo del ocho de §7 pero con naturaleza distinta—.
  DEUDA. Nace `D-proyeccion-instancia-espejo` (sede O-ajuste-cierre), con ficha en §4 de `gestion_proyecto.md` y esta cita de trabajo, que son las DOS sedes que exige no engordar `D-deuda-sin-sede-en-el-plan`. `D-post-horario-sin-sesiones` sube de grado: QUINTA confirmación y la primera que cuesta trabajo en vez de describir un cuerpo raro. UNA instancia de `D-guion-busca-token-esperado`, del arquitecto: escribió la consulta del tamaño de instancia contra `sesion.actividad_id`, columna inexistente, teniendo el DDL a un turno de distancia; la cazó Claude Code al ejecutar. UNA instancia de `D-guion-exit-enmascarado`, de Claude Code y DECLARADA POR ÉL antes de que nadie la viese: un `[ok] ni la referencia ni el banco están abiertos` que salió del fallo de un `ls` sobre un pid inexistente y no de una comprobación real. **Recuento de Higiene/Método: 15 en el M0 y 16 AL CIERRE**, y el propio movimiento es el dato: el 15 se contó contra §4 con el criterio escrito en S142 —doce con sede explícita más `D-guion-exit-enmascarado`, `D-arranque-no-literal` y `D-censo-r4-cuenta-menciones`— y fue la PRIMERA vez que el número reprodujo, que era para lo que S142 dejó el criterio escrito. La decimosexta la pare esta misma sesión al cerrar: `D-censo-r4-ciego-a-la-extincion`. Dejar escrito 15 sabiendo que hay 16 sería la enfermedad que el criterio venía a curar. Nace también `D-censo-r4-ciego-a-la-extincion`, encontrada por Claude Code en el R4 de este cierre: al archivar S141, `C-alcance-particiones` cayó a CERO apariciones en el corpus vivo y salió del censo, mientras cuatro tokens que sólo bajaron a una entraron. El instrumento vigila al huérfano a medias y calla ante el total. Deuda bloqueante del proyecto: 0.
  SUITES. solver 91 -> 91 (no se tocó `solver/`), app 303 -> 317 (+14), vitest 441 SIN CORRER y afirmado por AUSENCIA de cambios: `git status -- app/frontend` da cero líneas. La línea base se midió corriendo la suite antes de escribir, no heredando la cifra. `solver/src/main` no tocado, luego `referencia-codigo-solver.md` no se regenera.
  LIMPIEZA (M1-bis): archivada S141 a `bitacora-sesiones.md` (promovida a `### Sesión 141`, insertada al final en orden ascendente, cuerpo íntegro); degradada S142 a «Última sesión registrada (previa):»; S143 queda como única cabecera H3 viva. Los dos censos de la bitácora pasan de S140 a S141. R4/costura: **las suites SÍ se recorrieron en la sesión** —91 en solver sin moverse y 317 en app, con `target/classes` borrado antes—, y el M1 no toca código, así que siguen midiendo este árbol; `vitest` 441 NO se recorre porque `app/frontend/src` está intacto. Commits del Cambio: `8729e62` (endpoint y 11 casos) y `c5df06a` (los 3 casos de no-escritura y aula). El token nuevo `D-proyeccion-instancia-espejo` nace con definición viva en §4 de `gestion_proyecto.md` y cita de trabajo en esta entrada. PENDIENTE PARA EL M0 DE S144, anotado y no ejecutado: la fila de `D-post-horario-sin-sesiones` vive en «Mejora futura, cuelga y espera», y S143 la falsifica —costó una corrida entera y obligó a reconsultar en tres puntos—. Cambiar de tabla es cambiar su clasificación, que es decisión estratégica y no de cierre.

### Sesión 144 — O-ajuste-cierre (H1), C-intercambiar-instancias: **EL AJUSTE MANUAL PERSISTE SOBRE EL CENTRO REAL, Y LO CERTIFICA UN ENDPOINT QUE NO ES EL QUE HIZO EL CAMBIO.** El M0 reescribió la condición (1) sobre el INTERCAMBIO y puso una puerta de medición delante en vez de fiarse del argumento: 10 678 pares candidatos barridos sobre el centro real, **1 774 válidos** y 979 no degenerados, antes de escribir una línea de producto. El intercambio salió 200, `GET /diagnostico` devolvió **cero violaciones duras** después, y la vuelta dejó el fichero con **2 bytes de diferencia**, los dos de la cabecera SQLite. app 317 -> 334.
  M0. Cambio `C-intercambiar-instancias`, creado en este M0. Objetivo **O-ajuste-cierre**, Hito **H1**. R-invalidación: ninguna —O-navegación (S127) y O-diseño (S133) cerrados, H3 y H4 no tocan esta superficie, y §5 mantiene el orden sin hecho nuevo que lo invierta—. **Las DOS preguntas estratégicas de S143, resueltas y no esquivadas.** (1) La condición (1) SE REESCRIBE sobre el intercambio, por dos argumentos: el de medición —el listón de S142, un verde que sólo existe contra fixtures sintéticas es inmedible— y el de producto, que es el que manda: en un centro a 30/30 con cero plazas sin subgrupo, ajustar a mano sólo puede ser intercambiar. (2) **El objetivo NO se parte:** §7 pide más de ocho sesiones para sospechar y van cuatro Cambios; el corte «motor» / «cierre» dejaría partidas las condiciones 1 y 2; y el argumento de S142 sobre el nombre que mentiría sigue en pie. Disparador escrito para reabrirlo. **Y la reescritura NO se ejecutó sin puerta:** nadie había medido que exista un intercambio válido sobre este centro, y escribir el criterio sin comprobarlo habría repetido el error que S143 destapó. **Recuento de Higiene/Método: 16**, contado contra §4 con el criterio de S142 —13 con sede «sesión de Higiene/Método» y 3 con sede «el script de R4 pendiente desde S101»—, descartando dos falsos positivos que el grep sí trae (`D-tokens-inexistentes`, sede «sin objetivo asignado», y `D-javadoc-plazas-caducado`, sede cerrada). **Segunda vez consecutiva que el número reproduce**, que es para lo que S142 dejó el criterio escrito. Ninguna bloquea: Higiene/Método pierde por R-deuda.
  **LA PUERTA DE MEDICIÓN, con sus umbrales declarados ANTES de correr.** Arnés desechable de solo lectura sobre copia (`educhronos-s144-m2.db`), `@DynamicPropertySource` + `spring.sql.init.mode=never`, un único método `@Transactional(readOnly = true)` —la inversión de `idxTramo` va por identidad de objeto de `TramoSemanal`, así que catálogo, solución y verificaciones tienen que vivir en el mismo contexto—. Válido == diferencia por MULTICONJUNTO vacía contra el veredicto en reposo, nunca `esValida()`. Escrito antes de medir qué se concluiría de cada resultado, **incluido que cero válidos obligaba a una tercera reescritura y a volver al M0**. Horario 1, 632 instancias colocadas, 0 violaciones en reposo (cuarta medición tras S119, S135 y S142), una excluida por pin. **10 678 pares que comparten grupo de 198 765 posibles; 1 774 válidos (16,6 %)**, 2,13 ms/par y 22,7 s el barrido —la calibración predijo 9 ms/par por calentamiento de la JIT en el primer par, así que el presupuesto se sobrestimó y no al revés—. El nivel 2 no llegó a ejecutarse. **Por qué salen tantos, y es el argumento de fondo del Cambio:** el intercambio es CONSERVATIVO para todo recurso que ambas instancias comparten, así que exigir grupo común neutraliza de entrada `SOLAPE_GRUPO` y `SOLAPE_SUBGRUPO`, que son los que rechazaron los 29 destinos de S143.
  **EL DATO QUE VALE MÁS QUE EL VERDE: CERO PARES BLOQUE CONTRA BLOQUE.** De los 1 774, 795 son entre instancias de la MISMA actividad —degenerados: mismas plazas, mismos profesores, misma aula; no mueven ningún recurso de sitio, y aun así el endpoint debe ACEPTARLOS, porque rechazarlos exigiría una regla de dominio que nadie ha escrito— y 979 entre actividades distintas. De esos 979, **sólo 5 (0,5 %) involucran una instancia multiplaza, y NINGUNO tiene las dos**. El estrechamiento lo produce el aula: una instancia multifila se lleva 2 aulas intactas al destino, y que las dos queden libres allí es raro; que ocurra en ambos sentidos no pasa ni una vez. Los cinco tienen la misma forma —un bloque de 2 plazas contra una actividad de una plaza del mismo grupo— y tres cambian de día. **Es una propiedad medida de este centro, no un límite del arnés**, y queda escrito el censo completo para quien monte el gesto. **CORREGIDO en S145, por medición y no por opinión: NO quedó escrito.** De los cinco pares solo se enumeró el que S144 usó; el arnés que los produjo se borró antes del cierre y los otros cuatro no están en ningún fichero del repositorio. Nueva instancia de `D-declarado-sin-artefacto`. El Cambio del gesto salió adelante igual, porque el par documentado cumplía los tres criterios que se le pedían.
  CONTRATO. `PUT /api/horarios/{horarioId}/instancias/intercambio` con las DOS instancias NOMBRADAS en el cuerpo, no una instancia y un tramo. Reutilizar el endpoint de movimiento e intercambiar cuando el destino está ocupado queda DESCARTADO con argumento: la misma petición produciría efectos distintos según el estado del servidor sin que el cliente sepa cuál ocurrió, y localizar la segunda instancia por su tramo sólo funciona si en cada celda hay exactamente una, que es propiedad de ESTE centro. Causas: 404 `HORARIO_INEXISTENTE`, 404 `INSTANCIA_INEXISTENTE` diciendo CUÁL de las dos, 409 `INSTANCIA_PINADA`, 409 `VIOLA_REGLA_DURA`, 400 `INSTANCIAS_IGUALES`. **`TRAMO_INEXISTENTE` desaparece**: no hay tramo en el cuerpo, `resolverTramo` no entra, y la rama homónima de `conInstanciaEn` queda inalcanzable desde aquí, anotado en su javadoc. Es superficie que se RETIRA, no que se hereda. Desempates heredados de S143 y no reinventados: mismo tramo -> 200 no-op; mismo tramo Y pinada -> manda el pin, 409. El aula no cambia.
  **REUTILIZACIÓN, medida y no declarada.** Cero clases nuevas de control: `MovimientoInstanciaController` tenía ctor de un solo argumento y el intercambio usa el mismo servicio, así que el motivo que justificó separar de `HorarioController` en S143 —romper cinco specs ajenas— no aplica y se verificó antes de escribir. `conInstanciaEn` ENCADENA limpio (toma las asignaciones de la solución que recibe, así que la segunda llamada ve la primera mutación y propaga `aulasElegidas` intacto); lo único que no encajaba era la firma, que pedía un `MoverInstanciaRequest`, y se GENERALIZÓ a `(actividadCodigo, indice)` en vez de copiarse, igual que `releerInstancia`. `soloNuevas` y `cargarProblema` tal cual, una sola carga por petición. Decisión del ejecutor que se ratifica y queda como contrato: el cuerpo del 200 son DOS listas, `{primera, segunda}`, y no una concatenada, para que el gesto pueda pintar cada lado sin reagrupar por `actividadCodigo`.
  COBERTURA Y MUTACIÓN. 17 casos, conteo medido corriendo la suite antes de escribir y no heredado —la corrida bruta daba 318 porque el arnés se contaba a sí mismo, y se cazó: es `D-guion-busca-token-esperado` NO ocurriendo, y un método que sólo anota los aciertos no mide nada (S138)—. Siete mutantes, y **uno SOBREVIVIÓ: `m3`, comparar las violaciones por posición en vez de por multiconjunto, pasaba los 16 casos nuevos Y los 14 de S143.** El diagnóstico del ejecutor es correcto y estructural: `VerificadorSolucion` recorre las familias en orden fijo, así que la inestabilidad que motiva la resta por conteo sólo vive DENTRO de una familia, y ningún caso tenía dos violaciones de la misma familia con una nueva en medio. **Hueco abierto en S143, no introducido aquí, y se paga:** `soloNuevas` decide qué se le nombra al usuario, que es la condición (2) del criterio activo. Caso 17, con dos `SOLAPE_PROFESOR` preexistentes y uno nuevo de la misma familia; **discrimina al máximo, 3 en vez de 1**, porque la nueva entra por delante en la iteración del `HashMap` y desfasa a las dos que no se movieron. Un solo caso, en el spec nuevo, con javadoc explicando que cubre el método compartido; NO se duplicó en el de S143. `MovimientoInstanciaEndpointTest` sigue en 14, intacto.
  **M4 SOBRE EL CENTRO REAL, y es el PRIMERO DEL OBJETIVO QUE ESCRIBE.** Copia nueva `educhronos-s144-m4.db`; el literal de arranque se CITÓ de la entrada de S130 en la bitácora en vez de reconstruirse por octava vez (`D-arranque-no-literal` sigue viva), omitiendo a propósito el `package` por la razón de S143. Base servida verificada por `/proc/1349/fd` y no por conteos. **Orden deliberado: primero los seis rechazos, después el verde**, porque un 200 destruye el estado que se fotografió. Los seis salen con su cuerpo, los 404 dicen cuál de las dos instancias falta por ambos lados, y **tras los seis el md5 es idéntico al inicial: ni el `change_counter` se movió**, ni siquiera en los dos que llegaron hasta el veredicto. El caso 15, verificado por mutación en fixtures, queda medido sobre el centro real. **El 409 de regla dura devolvió 8 violaciones nuevas**, cuatro por destino, cada una con `recursoCodigo`, `tramoCodigo` y las celdas culpables: es la mitad de SERVIDOR de la condición (2) sobre datos reales, con más filo que las 350 de S143.
  **EL VERDE, Y SU PRUEBA INDEPENDIENTE.** `(Bloque-ALCT_Fr2-1ºC, 1)` J4, 2 filas y 2 aulas, contra `(Geo-1ºC, 2)` X4, 1 fila; multiplaza contra monoplaza y con cambio de día. 200, filas permutadas, pares plaza->aula idénticos a la foto, 3276 filas sin variar, proyección correcta, y **`GET /api/horarios/1/diagnostico` devolviendo CERO violaciones duras después del ajuste**. Ese cero lo emite un endpoint que no es el que hizo el cambio, que es exactamente lo que se le pedía: el veredicto propio no vale como su propia prueba. **Matiz que la foto reveló y que no se esconde:** las dos instancias comparten el aula A3, así que cada una la libera para la otra. No invalida nada —es un intercambio real y válido— pero explica por qué los multiplaza son cinco: el que existe aquí es el que se autoresuelve el aula, que es la otra cara del cero bloque-contra-bloque. La VUELTA salió 200 y devolvió ambas a sus tramos. **El md5 NO vuelve al inicial —73dcabd2…— y eso estaba predicho:** el `.dump` de 5787 líneas coincide, y `cmp -l` localiza exactamente 2 bytes distintos, en los offsets 28 y 96, ambos 91 -> 93. Son el `change_counter` y el `version-valid-for`, las dos escrituras de la sesión. Confirma el precedente de `D-pin-ocupada-no-persiste` con el byte exacto, y es mejor evidencia que un md5 que coincide: dice QUÉ cambió, no sólo que nada relevante cambió.
  **HALLAZGO PARA EL CAMBIO SIGUIENTE, encontrado ahora y no montando el gesto:** `DISTRIBUCION_MISMO_DIA` es regla dura y **no trae recurso** —el conflicto es de la actividad consigo misma—, así que la condición (2), leída al pie de la letra, tiene una rama que nadie puede cumplir. No se arregla aquí: es superficie del gesto. Queda anotado en la ficha con sus dos salidas, pintar el caso sin recurso o precisar la redacción.
  CRITERIO. Condición (1): **mitad SERVIDORA CUMPLIDA y demostrada sobre el centro real**; mitad de INTERFAZ pendiente, en el Cambio del gesto. Condición (2): mitad de servidor reconfirmada con más detalle; mitad de interfaz pendiente, con la precisión de `DISTRIBUCION_MISMO_DIA` que resolver. (3) y (4) sin tocar. (5) sigue cumplida. **Quedan DOS Cambios: el gesto en la rejilla —que absorbe la mitad de interfaz de (1) y (2) y la condición (4)— y la prevalidación S8 de la (3).**
  DEUDA. **`D-post-horario-sin-sesiones` CAMBIA DE TABLA**, decidido en el M0 y ejecutado en el cierre: de «Mejora futura» a deuda técnica real colgada de O-ajuste-cierre, no bloqueante. La definición de §4 decide sola —mejora futura es «algo que falta pero no está mal», y aquí hay algo mal hecho, medido y con coste cobrado—. SEXTA confirmación, y CUARTA copia a mano del arreglo que ya vive en `BloqueoService.aulasDe()`. Sigue sin pagarse: tocar `open-in-view` es transversal. `D-proyeccion-instancia-espejo` engorda: dos llamadas por respuesta, compensadas con el caso 16. No nace ningún token nuevo. **UNA instancia de `D-guion-busca-token-esperado`, del ASISTENTE y cazada por Claude Code al ejecutar:** el ancla de sustitución del guion de cierre se escribió `intercambio, y si el objetivo son dos.`, copiada del texto RENDERIZADO, cuando en el fichero los `**` de cierre de negrita van pegados —`intercambio**, y si el objetivo son dos.`—: cero ocurrencias. El ejecutor NO abortó a ciegas ni reconstruyó de memoria: localizó el ancla real, comprobó que era única y que delimitaba exactamente el bloque de la condición 1, y lo dijo. Es la causa raíz de S129 —patrones escritos de memoria teniendo el fichero a un turno— aparecida en un guion de ESCRITURA y no de lectura, que es la variante cara. Se registra porque un método que sólo anota los aciertos no mide nada (S138). **Deuda bloqueante del proyecto: 0.**
  SUITES. solver 91 -> 91 (`solver/` no tocado, así que `referencia-codigo-solver.md` no se regenera), app 317 -> 334 (+17, 43 -> 44 clases), vitest 441 SIN CORRER y afirmado por AUSENCIA de cambios: `git status -- app/frontend` da cero líneas. Línea base medida corriendo, no heredada. `target/classes` borrado antes de cada mutante, con restauración por `cp` sin `-p`.
  BANCOS. Referencia `educhronos-s137-centro-completo.db` en `64d671fe…`, verificada al abrir y al cerrar: **intacta**. `s142-m2`, `s143-m4` y la nueva `s144-m2` las tres en `5f02ef43…`, intactas. `s144-m4` sale en `73dcabd2…` con el contenido idéntico y los 2 bytes de cabecera. Ninguna generación lanzada, ningún presupuesto de 600 s gastado. `ArnesIntercambioS144Test.java` borrado antes del cierre y fuera del conteo.

### Sesión 145 — O-ajuste-cierre (H1), C-gesto-rejilla: **ARRASTRAR YA REUBICA, Y EL PIN DEJÓ DE COLGAR DEL ARRASTRE PARA TENER GESTO PROPIO.** Tres condiciones del criterio pasan a CUMPLIDAS —(1) entera, (2) entera y con su precisión resuelta, y (4)—, y la (5) vuelve a ser alcanzable tras una regresión que el propio Cambio introdujo y reparó en la misma sesión. El M4 sobre el centro real ejecutó cinco pasadas más una complementaria, y el verde del intercambio se demostró DOS veces: primero sobre un par degenerado y después sobre uno multiplaza entre actividades distintas, que es donde reagrupar mal sí se nota. vitest 441 -> 477.
  M0. Cambio `C-gesto-rejilla`, creado en este M0. Objetivo **O-ajuste-cierre**, Hito **H1**. R-invalidación: **ninguna** — O-navegación cerró en S127 y O-diseño en S133, y H3 y H4 no tocan esta superficie. Recuento de Higiene/Método contado contra §4 con el criterio de S142: **16** —13 con sede «sesión de Higiene/Método» y 3 con sede «el script de R4 pendiente desde S101», descartando `D-tokens-inexistentes`—, **TERCERA reproducción consecutiva** del mismo número; pierde por R-deuda, como las dos anteriores. **EL ORDEN SE DECIDIÓ CON ARGUMENTO, no por inercia:** de los dos Cambios que quedaban se eligió el gesto y no la prevalidación S8, porque sobre ESTE centro S8 no puede producir un verde observable —S136 midió que los 7 tutores de Bachillerato son falsos pero PORTANTES, así que S8 pasa y la lista seguiría vacía—, y hacerla primero habría repetido el defecto que S142 desmontó y S143 confirmó.
  LÍNEA BASE MEDIDA Y NO HEREDADA. `vitest` se corrió antes de escribir una línea: **441 en 50 ficheros, verde**, tras cinco sesiones sin correrse desde S141. app 334 y solver 91, también medidos. El contrato del backend se verificó contra el fuente Java —`MovimientoInstanciaController` y los seis DTO— ANTES de tipar nada en el cliente, con orden de parar si algo no casaba; casó.
  CUATRO DECISIONES DE DISEÑO, cada una con su razón. (a) **Pintado NO optimista**: la rejilla no se mueve hasta el 200, misma disciplina que el despinado (D-F8.6-ii-5); un movimiento optimista produce el MISMO estado final, así que sólo la fase «antes» discrimina y por eso todos los tests la aseveran. (b) **La regla 0/1/≥2 ocupantes**, con la rama ≥2 declarada LIMITACIÓN CONOCIDA y no adivinanza: con varias candidatas el arrastre no expresa con cuál intercambiar, y elegir por el orden de la celda sería inventar una intención que el usuario no manifestó. (c) **`Violacion` reutilizada tal cual** de `models/diagnostico.model.ts` y no duplicada: el backend reutiliza literalmente `ViolacionDTO`/`CeldaRefDTO` del diagnóstico para el cuerpo del rechazo, y un segundo espejo de la misma forma se desincronizaría en silencio. (d) **Función hermana `mensajeAjuste()` en vez de ensanchar `mensaje()`**, que es la excepción de firma del proyecto —degradado fijo de pin, sin parámetro—: ensancharla la obligaría a decir dos cosas distintas según quién la llame. **Y la condición (4) se implementó PRIMERO**, antes del primer `package`, porque es a la vez trabajo del Cambio y red de seguridad del M4: con «Generar horario» vivo y 600 s irreversibles, el diálogo es lo que impide un clic accidental.
  **LA OPCIÓN B DEL CANDADO, Y POR QUÉ NO LA A.** El candado se renderiza SIEMPRE pero permanece oculto por `opacity` y aparece en `:hover` de la instancia y en `:focus-visible` del botón. Se descartó la opción A —glifo permanente visible— porque un icono fijo sobre ~791 instancias degrada un objetivo CERRADO (O-diseño, S133) y repite exactamente la ambigüedad que `D-insignia-sin-leyenda` costó quitar en S128. No se usó `display:none` ni `visibility:hidden`: los dos sacan el botón del árbol de accesibilidad y lo vuelven infocalizable, que dejaría el gesto sólo para quien use ratón.
  **LA REGRESIÓN Y SU REPARACIÓN, sin suavizarla.** Al pasar el arrastre de pinar a ajustar, `BloqueoService.guardar` quedó SIN LLAMADOR y el producto perdió la capacidad de crear un pin, que §1 exige en el paso 4 del guion de aceptación. No lo detectó ningún test —los casos (21)-(26) medían el alta desde la suelta y se sustituyeron en bloque porque afirmaban un comportamiento retirado—: lo detectó el ejecutor al informar, y el arquitecto ordenó repararlo. Se reparó en la MISMA sesión con gesto propio, y se verificó en navegador que el aviso «N pines sin aplicar» vuelve a poder CRECER, que era el síntoma exacto de la capacidad perdida. Reparar lo que el propio Cambio rompe es terminarlo, no ampliarlo.
  COBERTURA Y MUTACIÓN. **Doce mutantes, todos con víctima nombrada y ninguno superviviente.** Primera tanda, siete: invertir la rama 0/1 (10 víctimas, reales (61) y (62)); quitar el fallback de recurso ausente (4, en DOS ficheros, y (57) sobrevive como debe); aplicar `r.primera` a los dos lados (1); subir la guarda ≥2 a ≥3 (1); volver al diálogo condicional (13, reales (27)(27b)(27c)); quitar el corte destino==origen (1); emitir `ocupantes: []` siempre (1). Segunda tanda, cuatro: invertir `alAlternarPin` (3); quitar el `(pointerdown)` (1, y ES la que prueba que el caso 5 mide algo); no añadir al índice tras el 200 (2); `filaDeClave` con `split('|')` (1). Y M12 sobre el caso (78): quitar la limpieza de `errorPin` (1).
  **LAS DOS CORRECCIONES DEL EJECUTOR AL ENCARGO DEL ARQUITECTO**, que se registran porque un método que sólo anota los aciertos no mide nada (S138). (i) El `stopPropagation()` del CLICK **no protege del arrastre**: el `cdkDrag` arranca en `pointerdown`, y lo que impide que un clic limpio arrastre es el `dragStartThreshold` de 5 px del CDK. El caso 4 tal como lo pidió el arquitecto —«quita la protección de propagación y comprueba que cae»— habría salido VERDE sin probar nada. Es una instancia de `D-guion-busca-token-esperado`, del ASISTENTE, y con el fichero a un turno de distancia. Se avisó ANTES de escribir, el arquitecto decidió añadir el `(pointerdown)` protector, y con él la mutación sí tiene víctima. (ii) `filaDeClave`, que el encargo no pedía: el contenedor tiene que resolver clave -> tramo, y hacerlo inline habría dejado sin test la dimensión del separador; resuelve contra la proyección **VIGENTE** y no la del montaje, porque tras un ajuste pinar con el tramo viejo clavaría la instancia donde ya no está.
  **M4 SOBRE EL CENTRO REAL: CINCO PASADAS EN ORDEN, LO QUE NO ESCRIBE PRIMERO.** Copia nueva `educhronos-s145-m4.db` desde `educhronos-s144-m2.db`, base servida verificada por `/proc/<pid>/fd` y no por conteos. (3.1) **Condición (4)**: el diálogo se abrió con la lista de avisos VACÍA diciendo el coste —diez minutos, sustituye el trabajo en curso, no se deshace— y con el botón principal sin el «de todos modos» que sin hallazgos no se referiría a nada; cancelado con Escape y con backdrop, **cero peticiones** y las filas de `horario` en 4. (3.2) **Rechazo**: un arrastre real devolvió 409 y la UI pintó `SOLAPE_AULA — Gimnasio en M1`, `SOLAPE_PROFESOR — LEN7 en L1` y **`DISTRIBUCION_MISMO_DIA — LCL-1B-A #2, LCL-1B-A #3`**; el caso sin recurso cayó SOLO, no fabricado. (3.3) **El verde**, con la red abierta desde antes de soltar. (3.4) **El pin**: `POST /api/bloqueos` 200, aviso de 1 a 2, fila nueva en `sesion_bloqueada`, y el despinado devolvió las dos a 1. (3.5) **Vista por profesor**: 18 de 30 celdas vacías para BYG1, soltar en una manda `mover` y devuelve 409 nombrando `Ing-1ºA #4`, una clase que el usuario NO VE en esa vista. La predicción del arquitecto se confirmó, no se falsó.
  **LA PASADA COMPLEMENTARIA, Y POR QUÉ HIZO FALTA.** El primer verde cayó sobre `LCL-1B-A` #3 contra #4: misma actividad, mismas plazas, mismo profesor, misma aula. Es uno de los 795 pares DEGENERADOS que S144 censó aparte, y es **el único caso donde reagrupar mal no se nota**, justo lo que el contrato de las dos listas existe para impedir. **Lo detectó el arquitecto al leer el informe; no lo detectó ningún test.** Se repitió sobre par NO degenerado y MULTIPLAZA: `Bloque-ALCT_Fr2-1ºC` #1 —dos filas, aulas A17 y A3, J4— contra `Geo-1ºC` #2 —una fila, A3, X4—. Tres evidencias que el degenerado NO podía dar: la respuesta llegó como objeto con `primera` y `segunda` de **longitudes distintas, 2 y 1**, y `actividadCodigo` distinto en cada lado; los totales BLANDOS se movieron —ventanas 206 -> 205, consecutivas 9 -> 10— cuando el degenerado los dejó clavados en 206/9; y `cmp -l` localizó **TRES** bytes de `sesion.tramo_inicio_id` y no dos, que es la firma de la multiplaza. `XMLHttpRequest` instrumentado antes de soltar registró **una sola petición, el PUT, y ningún GET**: la rejilla pintó cada lado sin recargar la proyección.
  PRUEBA INDEPENDIENTE, las dos veces. `GET /api/horarios/1/diagnostico` devolvió **cero violaciones duras** después de cada intercambio, y `GET /api/horarios/1/proyeccion` —endpoint distinto del que escribió— las instancias en su tramo nuevo con sus aulas conservadas. El veredicto del propio endpoint no vale como su propia prueba.
  MD5 POR HITO, con los bytes localizados por `cmp -l` y su campo. Partida `5f02ef43…`; **arranque, dos cancelaciones del diálogo y dos 409 no la movieron** —ni el `change_counter`—. Tras el intercambio degenerado `a25708b3…`; tras pinar `e96ed7c0…`; tras despinar `3965f320…`; final, tras el intercambio multiplaza, **`95050aaa8f6b7e97ca8e3273f011221a`**. Los DIEZ bytes acumulados frente a `s144-m2` quedan clasificados uno a uno: **28** y **96**, los bytes bajos del `change_counter` y del `version-valid-for` (91 -> 95, las cuatro escrituras); **232878** y **232890** (26 -> 19), los `sesion.tramo_inicio_id` de las dos filas del bloque; **234923** (19 -> 26), el de Geo; **240712** y **240727** (15 <-> 8), los del intercambio degenerado; y **135156, 135158 y 135159**, estructura interna de la página 33 tras el INSERT+DELETE del pin. Estos tres NO se diagnosticaron leyendo bytes: los zanja el `.dump`, **5787 líneas antes y después, exactamente cinco filas de `sesion` distintas y ninguna diferencia fuera de esa tabla**, con `sesion_bloqueada` idéntica al origen.
  CRITERIO. Condiciones **(1) CUMPLIDA ENTERA** —la mitad de interfaz, demostrada en navegador y sobre par no degenerado—, **(2) CUMPLIDA ENTERA** con su precisión resuelta, **(4) CUMPLIDA** y **(5)** de nuevo ALCANZABLE tras la reparación. Queda **(3)**, la prevalidación S8, que es también el ÚNICO Cambio abierto del objetivo. Cinco Cambios en total, por debajo del ocho de §7, y ninguna sexta condición: **el disparador de replanteo NO se dispara**.
  DEUDA. Cuatro altas: `D-candados-en-tabulacion` y `D-spa-sin-fallback-de-rutas` en «Deuda técnica real» (sedes O-ajuste-cierre y H4), y `D-ajuste-ciego-fuera-de-vista-grupo` y `D-lado-instancia-en-prosa` en «Mejora futura», porque la definición de §4 decide sola y ninguna de las dos describe algo mal hecho. **`D-pin-ocupada-no-persiste` CERRADA POR EXTINCIÓN DEL CAMINO, y sin diagnóstico: se dice así y no se finge otra cosa** — la discrepancia de S127 nunca se explicó; lo que ha ocurrido es que el gesto que la producía ya no existe. **DOS instancias de `D-guion-busca-token-esperado`, y las dos de ESCRITURA.** La primera, del asistente, en el guion del Cambio (el `stopPropagation` que no protegía del arrastre, ver arriba). **La segunda es DEL EJECUTOR y se dice sin suavizarla:** al aplicar el guion documental, el ancla de la condición 4 se construyó con indentación 3/6 cuando el fichero la tiene 2/5, y el `assert` de unicidad devolvió cero. El guion NO traía ese ancla transcrito —decía sólo «§3, ficha de O-ajuste-cierre, condición 4, añade al final»—, así que la indentación la puso el ejecutor al leerla de una salida de `awk` cuyo prefijo `%4d| ` ya incluía un espacio: contó tres donde había dos. Es el mismo defecto de la familia —el guion busca lo que espera encontrar y no lo que hay—, y lo que evitó el daño fue no reconstruir de memoria ni abortar a ciegas: se miraron los bytes con `cat -A`, se localizó el ancla real y se dijo. Y **nueva instancia de `D-declarado-sin-artefacto`**: la entrada de S144 afirma «queda escrito el censo completo» de los cinco pares multiplaza y enumera UNO; el arnés que los produjo se borró antes del cierre y los otros cuatro no están en ningún fichero, comprobado por grep sobre plan, bitácora, `scripts/` y `tools/`. Corregida esa frase en la entrada de S144. Se registra **porque un método que sólo anota los aciertos no mide nada (S138)**, y en la misma línea se anota que **el ejecutor NO guardó copia del estado previo a la pasada complementaria** y lo compensó comparando contra el origen `s144-m2` y zanjando los tres bytes de la página 33 con el `.dump`; el método correcto es fotografiar antes, el resultado quedó igualmente probado, y por eso se dice en vez de ocultarse. **AVISO R4:** este cierre ACORTA el censo de tokens, y `D-censo-r4-ciego-a-la-extincion` advierte que una lista de sospechosos más corta puede significar que algo ha empeorado; aquí no, la baja es deliberada y argumentada. **Deuda bloqueante del proyecto: 0.**
  R4 / COSTURA. **El paso M1.4 lo cazó el INSTRUMENTO, no la persona, y el apunte es POSITIVO.** `verificar-cierre.py` falló en duro al cerrar: S143 estaba archivada y los dos censos de la bitácora actualizados a S143, pero la crónica de archivado seguía en S142 —y es EXACTAMENTE el paso que S130 dejó sin hacer, así que la omisión es reincidente y la lectura humana no la vio ninguna de las dos veces—. Se corrigió en el acto, y de paso la frase de ventana, que seguía nombrando S143 y S144. Tras la corrección: 0 comprobaciones duras con fallo, y los índices de `gestion_proyecto.md` (27) y `plan_trabajo_horarios.md` (66) con 0 entradas descuadradas. **Censo de tokens sospechosos: 33.** **NO consta el número de la sesión anterior**, comprobado y no supuesto: la entrada de S144 no lo registra, `verificar-cierre.py` no escribe historial de ninguna clase, y las únicas cifras del repositorio son de S130 (27), S131 (26 contra 30) y S141 (29). Por tanto **la comparación no es posible esta vez**, y se dice en vez de inventar un número previo: `D-censo-r4-ciego-a-la-extincion` advierte que una lista más corta puede significar que algo ha EMPEORADO, y sin el dato anterior esa vigilancia no se puede ejercer. Queda el 33 escrito para que S146 sí pueda.
  SUITES. vitest **441 -> 477** (+36, de 50 a 52 ficheros), app **334 -> 334**, solver **91 -> 91**. `solver/` no tocado, así que `referencia-codigo-solver.md` NO se regenera. Ficheros nuevos: `models/ajuste.model.ts`, `services/ajuste.service.ts` con su spec, `horario/ajuste.ts` con su spec; y `horario/pines.ts` gana `filaDeClave` con tres casos propios.
  BANCOS. Referencia `educhronos-s137-centro-completo.db` en `64d671fe…`, verificada al abrir y al cerrar, **INTACTA**. Los tres bancos anteriores (`s142-m2`, `s143-m4`, `s144-m2` en `5f02ef43…` y `s144-m4` en `73dcabd2…`) intactos. Nueva copia `educhronos-s145-m4.db`, que cierra en **`95050aaa8f6b7e97ca8e3273f011221a`** con el par multiplaza permutado y SIN hacer la vuelta. **Ninguna generación lanzada y ningún presupuesto de 600 s gastado**, pese a que «Generar horario» estuvo vivo en pantalla durante todo el M4.
