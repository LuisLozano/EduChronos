...
- L865 — #### O-disponibilidad — "El centro introduce la disponibilidad de su profesorado." (REGISTRADO S159, NO ABIERTO)
- L895 — ### H1 — Ajustar (cierre)
- L897 — #### O-ajuste-cierre — "El ajuste manual está completo y verificado." ✔ TERMINADO (S146)
- L1074 — #### O-diseño — "La aplicación tiene un aspecto cuidado y coherente." ✔ TERMINADO (S133)
- L1365 — #### O-navegación — "La aplicación se maneja como una aplicación de escritorio." ✔ TERMINADO (S127)
- L1555 — ### H3 — Exportar
- L1557 — #### O-exportación — "El horario sale de la aplicación, en papel y en datos." ✔ TERMINADO (S150)
- L1618 — ### H4 — Instalar y pasar de curso
- L1620 — #### O-instalación — "La aplicación se instala, arranca y se cierra en un Windows limpio sin ayuda técnica." ✔ TERMINADO (S156)
- L1794 — #### O-curso — "El centro empieza el curso siguiente sin perder el anterior."
- L1890 — ## 4. Clasificación del trabajo pendiente
- L1908 — ### Clasificación de las deudas vivas actuales
- L1914 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1921 — #### Deuda técnica real, colgada de su objetivo
- L2024 — #### Mejora futura, cuelga y espera
- L2062 — #### Decisión arquitectónica consciente → sale de la cola
- L2075 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L2086 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L2095 — #### Deuda ya CERRADA (histórico, no pendiente)
- L2162 — ## 5. Revisión del roadmap: por qué H2 va primero
- L2227 — ## 6. Reglas estratégicas
- L2287 — ## 7. Métricas del sistema
- L2308 — ## 8. El sistema respondiendo a las preguntas clave
...
#### O-disponibilidad — "El centro introduce la disponibilidad de su profesorado." (REGISTRADO S159, NO ABIERTO)
...
- **Terminado cuando:** SE ESCRIBE AL ABRIRLO, sobre medición. Pautas fijadas en S159: tres estados por tramo,
  `{sin fila, BLANDA, DURA}` (modelo §4.3), sin el recreo; `peso` NO se expone (`D-F8.5-E-a`); el criterio se
  verifica con un caso sintético y no depende de los datos del centro.
- **Dependencia externa (REESCRITA en S162, decisión del usuario tras el cierre de S161):** los datos de
  disponibilidad del profesorado están en papel y no se digitalizarán fuera de la herramienta: el centro los
  introducirá con la propia interfaz de este objetivo, y hasta entonces se genera con los datos que hay. No
  bloquea la apertura ni cambia el orden (`O-curso` → `O-disponibilidad`). **Pendiente externo:** pedir al
  centro el formulario en papel, en blanco o uno relleno de ejemplo, para escribir el criterio de terminado
  sobre lo que realmente recoge: horas semanales por profesor, imposibilidades y preferencias. Si recoge algo
  que las pautas de S159 no contemplan (tres estados por tramo, sin `peso`), se decide al abrir.
- **Deudas:** `D-F8.5-E-b` cuelga de aquí y NO bloquea (§4). `D-F8.5-E-a` sigue como limitación conocida.
- **Fuera:** las preferencias positivas (modelo §7) y la calibración de `peso` (D21).
...
#### O-curso — "El centro empieza el curso siguiente sin perder el anterior."
...
- **Fuera de este objetivo:** la cadena entera de §1 (su orden respecto a la Fase 12 se decide al cerrar
  ...
  (Fase 12); `D-curso-sin-borrado` (mejora futura; se revisa al cerrar éste).
- **Métrica de §7:** si pasa de ocho sesiones, esconde dos objetivos.
