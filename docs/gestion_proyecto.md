...
- L1745 — ## 4. Clasificación del trabajo pendiente
- L1763 — ### Clasificación de las deudas vivas actuales
- L1769 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1776 — #### Deuda técnica real, colgada de su objetivo
- L1873 — #### Mejora futura, cuelga y espera
- L1907 — #### Decisión arquitectónica consciente → sale de la cola
- L1919 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L1929 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L1937 — #### Deuda ya CERRADA (histórico, no pendiente)
- L2004 — ## 5. Revisión del roadmap: por qué H2 va primero
- L2069 — ## 6. Reglas estratégicas
- L2128 — ## 7. Métricas del sistema
- L2149 — ## 8. El sistema respondiendo a las preguntas clave
...
- **Cambios que agrupa:** `C-alcance-instalacion` ✔ HECHO (S151), el M0 y las mediciones
  ...
  ✔ HECHO (S153), el post-procesador de la ruta de la base y la condición 7; `C-arranque-cierre` ✔ HECHO (S154), el modo escritorio (instancia única, navegador, bandeja, diálogos y log) y la escucha local, condiciones 4, 5, 6 y 8. Los demás se deciden en sus M0. Agrupación orientativa,
...
| D-entrega-caducada-indetectable (el `.ps1` verifica la entrega contra el `SHA256SUMS` de su misma carpeta, así que una entrega vieja pasa sus propias huellas) | O-instalación, el Cambio de la condición 3 (medio de entrega) | No | Nace en S154 y costó una ronda entera de M4 en Windows: se construyó la entrega del principio de la sesión (jar `71d1299f…`, código de S153) en lugar de la v3 (`68af23ae…`); el guion dio las dos huellas por buenas y el bundle salió sin modo escritorio. Lo delataron tres pruebas ajenas al guion: el tamaño idéntico al byte al de S153, la línea `java-options` ausente y el `.cfg` sin la propiedad. El arreglo va con el medio de entrega: que la huella esperada viaje por un canal distinto de la propia entrega, o que se compare con la que imprime el guion de Linux. No se paga ahora |
