...
- L1672 — ## 4. Clasificación del trabajo pendiente
- L1690 — ### Clasificación de las deudas vivas actuales
- L1696 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1703 — #### Deuda técnica real, colgada de su objetivo
- L1793 — #### Mejora futura, cuelga y espera
- L1827 — #### Decisión arquitectónica consciente → sale de la cola
- L1839 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L1848 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L1856 — #### Deuda ya CERRADA (histórico, no pendiente)
- L1923 — ## 5. Revisión del roadmap: por qué H2 va primero
- L1988 — ## 6. Reglas estratégicas
- L2035 — ## 7. Métricas del sistema
- L2056 — ## 8. El sistema respondiendo a las preguntas clave
...
- **Terminado cuando** — ESCRITO en S151 sobre medición: M2 en Linux (clon limpio y
  ...
  1. **Construcción reproducible.** Un guion versionado en el repo construye el app-image
     de Windows desde `mvn clean`, con un JDK 17 portable fijado por versión y sha256, sin
     ningún paso fuera del guion. El jar lleva un único `main-*.js`, el que cita
     `index.html`. Hoy no existe: las órdenes de S151 vivieron en un `LEEME` de `/tmp`, y
     sin `clean` el jar acumula un `main-*.js` huérfano por build.
...
- **Cambios que agrupa:** `C-alcance-instalacion` ✔ HECHO (S151), el M0 y las mediciones
  que escribieron este criterio. Los demás se deciden en sus M0. Agrupación orientativa,
...
- **Decisiones que quedan para sus Cambios:** el medio de entrega y la firma del
  ...
  bandeja del sistema es candidato: el runtime ya lleva `java.desktop`, que entonces no
  podrá recortarse); la carpeta de datos.
