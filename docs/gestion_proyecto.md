...
- L926 — #### O-diseño — "La aplicación tiene un aspecto cuidado y coherente." ✔ TERMINADO (S133)
- L1217 — #### O-navegación — "La aplicación se maneja como una aplicación de escritorio." ✔ TERMINADO (S127)
- L1409 — ## 4. Clasificación del trabajo pendiente
...
- **Terminado cuando** — REESCRITO en S142 sobre medición (precedente de forma y
  ...
  cuando:
  1. Una sesión del horario generado **se recoloca a otro tramo y persiste**, sin
     regenerar el horario entero.
     **NOTA DE S143, medida y no supuesta:** la mitad SERVIDORA está construida y
     medida (`PUT /api/horarios/{id}/instancias`, veredicto por diferencia, 14 tests,
     commits `8729e62` y `c5df06a`). Pero esta condición **no es producible sobre el
     centro real**: los 28 grupos están a 30/30 tramos y no hay plazas sin subgrupo,
     así que un barrido de los 29 destinos rechazó 29 de 29, en una instancia de 6
     filas y en otra de 1. Lo que este centro necesita es INTERCAMBIAR, no mover, y un
     intercambio no se compone de dos movimientos porque el estado intermedio es
     inválido. **El M0 de S144 decide si esta condición se reescribe sobre el
     intercambio**, y si el objetivo son dos.
