...
- L62 — ## 1. Estado final del proyecto
- L89 — ## 2. Hitos
- L103 — ### Hitos: valor, dependencias, orden
- L126 — ## 3. Objetivos técnicos
- L137 — ### H2 — Configurar un centro desde cero
- L139 — #### O-shell — "La aplicación es navegable." ✔ TERMINADO (S100)
- L155 — #### O-catálogo — "Creo los elementos simples del centro." ✔ TERMINADO (S106)
- L227 — #### O-estructura — "Expreso la complejidad real del centro." ✔ TERMINADO (S114)
...
- L1962 — ## 4. Clasificación del trabajo pendiente
- L1980 — ### Clasificación de las deudas vivas actuales
- L1986 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1993 — #### Deuda técnica real, colgada de su objetivo
- L2101 — #### Mejora futura, cuelga y espera
- L2139 — #### Decisión arquitectónica consciente → sale de la cola
- L2152 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L2164 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L2173 — #### Deuda ya CERRADA (histórico, no pendiente)
- L2240 — ## 5. Revisión del roadmap: por qué H2 va primero
- L2305 — ## 6. Reglas estratégicas
- L2365 — ## 7. Métricas del sistema
- L2386 — ## 8. El sistema respondiendo a las preguntas clave
...
#### O-disponibilidad — "El centro introduce la disponibilidad de su profesorado." (ABIERTO S164)
...
- **Terminado cuando** — ESCRITO en S164 sobre medición (M2 de solo lectura, HEAD `fc9076d`, bancos s137
  ...
  5. **La DURA se cumple entera.** Ninguna sesión ocupa un tramo DURA de sus
     profesores. Con sesiones de un tramo lo garantiza el solver; si un profesor
     con restricciones horarias (DURA o BLANDA) imparte una actividad de más de
     un tramo, la generación se rechaza antes del solver con un 422 que nombra
     al profesor y la actividad. El verificador tiene una regla dura de
     indisponibilidad del profesor por tramo ocupado, con cualquier duración, y
     la BLANDA penaliza en él cada tramo ocupado, con lo que un movimiento manual
     a un tramo vetado se rechaza. Se demuestra con tests que fallan antes del
     arreglo. **REESCRITA en S165** (decisión del usuario, con el motivo del
     arquitecto): la versión de S164 exigía que el solver respetara también los
     bloques. El propósito de la decisión (D) de S164 —que la rejilla no prometa
     algo que el horario no cumple— queda satisfecho con el rechazo; ningún banco
     del centro tiene actividades de más de un tramo (0 de 219, medido en S165).
     El arreglo del modelo queda en `D-indisp-solo-tramo-de-inicio`.
...
- **Decisiones (S165):** (F, del usuario, con el motivo del arquitecto) La
  condición 5 se reescribe: el caso de más de un tramo se rechaza en vez de
  resolverse, y `D-indisp-solo-tramo-de-inicio` pasa a limitación conocida. (G,
...
- **Cambios:** `C-alcance-disponibilidad` ✔ HECHO (S164, este criterio). `C-dura-completa` EN CURSO (S165): regla `INDISPONIBILIDAD_PROFESOR` y BLANDA
  por tramo ocupado en el verificador, cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE`
  y `PIN_SOBRE_TRAMO_DURA` en la prevalidación; cumple la 5 reescrita; le falta
  medir el cuerpo HTTP del 422 de la 6.
  `C-rejilla-disponibilidad` (condiciones 1 a 4 y 7; maqueta M-mockup y frontend).
- **ESTADO en S164: 0 de 7.**
- **ESTADO en S165: 1 de 7** (la 5, reescrita). La 6 está construida
  (`PIN_SOBRE_TRAMO_DURA`, mira todos los tramos que ocuparía la sesión) y
  probada en MockMvc, pero no se cuenta: falta comprobar que el motivo del 422
  llega en el cuerpo HTTP real. Riesgo: `D-F8.6-ii-a` (en Boot 4 el mensaje no
  viaja en el cuerpo); si se confirma, su arreglo pasa a bloquear la 6.
...
- **Terminado cuando** — ESCRITO en S147 sobre medición (M2 de solo lectura sobre el
  ...
  4. CSV: descargable DESDE LA VISTA DEL HORARIO, un proceso que lee SÓLO el CSV
     ...
     NAVEGADOR, cuyo md5 (`eff9b823c6baca32b1b6b36e1569933d`) coincide con el de `curl`.
...
| D-e2e-centro-minimo-rojo (`centro-minimo.spec.ts:265` falla: 0 instancias tras generar) | Fase 12 (CI) | No | Nace en S154, TÉCNICA REAL. Medido sobre `1e5baaf` limpio, en un worktree con su propio `npm ci`: falla idéntico, así que es PREEXISTENTE y ajeno a `C-arranque-cierre`; no se sabe desde qué sesión. IPv6 descartado con tres medidas (escucha `[::ffff:127.0.0.1]:8080`, `curl localhost` 200, `fetch` de Node 200). NO bloquea `O-instalación`: la condición 3 verifica descargas sobre copia del banco y su ficha dice «no depende de generar». Pero es el eslabón «crear centro → generar» de R-e2e: un e2e rojo que nadie mira es una señal perdida sobre hitos cerrados, y bloquea la Fase 12, que no puede montar CI con él en rojo. La causa no se investigó (fuera de alcance) |
...
| D-itinerario-como-grupo (un itinerario entero concentrado en un grupo administrativo, tercera forma de partición no modelada) | Sin sede (S141 cerró O-particiones) | No | Nace en S134 al medir 4º ESO: Humanidades/CCSS vive en 4ºC y por eso Latín y Economía son clase de grupo completo. No impide cargar ni generar; importa al CONSTRUIR un 4º por la UI. **Remodelar 4º ESO no es trabajo de O-demo** |
...
| D-aula-tipo-sin-uso-real (la tipificación de `aula.tipo` no sigue el uso real del centro) | O-demo | No | Nace en S135. Taller 2 y Taller 4 tipados `TALLER_FPB` con la rejilla VACÍA; Taller 3, el único que imparte FPB de verdad, tipado `ORDINARIA`. Medido que hoy no decide nada: el solver no ve el campo (`TipoAula` vive sólo en persistencia) y `AsignaturaAulaCompatible` tiene CERO filas. Arreglarlo obliga a revisar los 43 tipos y no bloquea; el riesgo es el día que esa tabla se llene |
...
| D-proyeccion-sin-duracion (la proyección no transporta la duración, así que sólo se conoce el tramo de inicio) | O-exportación (cerrado S150) | No sobre el centro de referencia | Nace en el M3 de S148, medida y no supuesta. `SesionVistaDTO` no lleva la duración y la tabla `sesion` sólo guarda `tramo_inicio_id`, de modo que la proyección —y con ella el CSV y los PDF que la consuman— sitúa cada sesión en UNA celda y no en el rango que ocupa. La vista tampoco la obtiene por otra vía: `grep -rni duracion` sobre `app/frontend/src` da 20 coincidencias, TODAS del catálogo de actividades, y 0 en `horario-view`, `horario-grid` y `horario.service.ts`. NO bloquea el criterio porque las 219 actividades del centro de referencia duran 1 tramo, pero el producto admite cualquier N —Hallazgo G del plan: el centro tiene un bloque de FPB de 3 tramos en el origen—, y en cuanto un centro use N>1 un PDF pintará una clase de dos horas como si fuera de una. |
...
| D-aviso-fuera-del-dialogo (un AVISO de pre-validación no llega al diálogo de confirmar la generación) | Sin sede | Nace en S146. `generar()` (`horario-view.ts:571`) filtra por `severidad === 'ERROR'` antes de abrir `ConfirmarGeneracion`, así que el AVISO de S8 —el primero con productor— sólo se ve en el panel, que arranca colapsado cuando no hay ERROR. Medido en el M4 de S146 sobre el centro real: panel colapsado, contador «otros hallazgos» a 2 y diálogo idéntico al de un catálogo sin hallazgos. No es deuda técnica real: el diseño de AVISO de S92 es exactamente este y nada está mal hecho, y perderse el aviso cuesta poco, porque S8 no depende de la colocación y se corrige cambiando el tutor sin regenerar. Sin sede porque O-ajuste-cierre cierra en la misma sesión, y colgarla de un objetivo cerrado equivale a decidir que no se paga nunca (precedente de S141). Arreglo si algún día hace falta: pasar también los AVISO al diálogo, con su propio rótulo |
| D-curso-sin-borrado (un curso no se puede eliminar desde la aplicación) | Objetivo de aceptación (sin ficha) | No | Nace en el M1 de S160, tras el selector: la aplicación lista, abre y duplica cursos, pero no borra, así que un duplicado con el año equivocado sólo se quita borrando el fichero en la carpeta de datos. No está en el criterio de `O-curso` ni en el paso 6 de §1, y es destructivo sobre el histórico del centro, con preguntas sin decidir: qué se puede borrar (¿el activo?, ¿el último?) y con qué confirmación. La unicidad del nombre por contenido (S160, `bc0355a`) quita el caso más probable. Disposición: se revisa al cerrar `O-curso`, y o pasa a condición del criterio o queda como mejora. **Revisada en S163 al cerrar `O-curso`:** queda como MEJORA FUTURA, por decisión del usuario. Duplicar por error no pierde datos, y un curso mal nombrado sólo estorba en la lista y se quita borrando su fichero con la aplicación cerrada. Pasa a colgar del objetivo de aceptación y se reabre si ésta muestra que hace falta. |
...
| D-indisp-solo-tramo-de-inicio (el solver sólo aplica DURA y BLANDA al tramo de inicio de cada sesión) | Sin objetivo (limitación conocida). No: LIMITACIÓN CONOCIDA desde S165. La condición 5 se reescribió (decisión del usuario) y el caso lo rechaza el cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE`. Se reabre si un centro necesita actividades de más de un tramo con disponibilidad. Nace en S165 (M2, T1/T4, medido). S165 no toca el solver y lo sustituye por el cortafuegos `RESTRICCION_HORARIA_CON_BLOQUE` de la prevalidación, que SÓLO se retira junto con el arreglo del solver para duración > 1, nunca por separado. Puerta del arreglo: huella canónica en los 44 fixtures e invariante de indexación de tramos (S165, fase 0, 0.3 y 0.2). El arreglo NO reutiliza `VerificadorSolucion.tramosOcupados`: el verificador sigue siendo oráculo independiente. Texto íntegro en «Deuda consciente VIVA». |
