...
- L739 — #### O-particiones — "Un grupo nuevo entra en el curso sin reconfigurar a mano." (ESBOZADO S115)
- L772 — ### H1 — Ajustar (cierre)
- L774 — #### O-ajuste-cierre — "El ajuste manual está completo y verificado."
- L788 — #### O-diseño — "La aplicación tiene un aspecto cuidado y coherente." ✔ TERMINADO (S133)
- L1079 — #### O-navegación — "La aplicación se maneja como una aplicación de escritorio." ✔ TERMINADO (S127)
- L1271 — ## 4. Clasificación del trabajo pendiente
- L1289 — ### Clasificación de las deudas vivas actuales
- L1295 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1302 — #### Deuda técnica real, colgada de su objetivo
- L1365 — #### Mejora futura, cuelga y espera
- L1397 — #### Decisión arquitectónica consciente → sale de la cola
- L1409 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L1418 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L1425 — #### Deuda ya CERRADA (histórico, no pendiente)
- L1484 — ## 5. Revisión del roadmap: por qué H2 va primero
- L1538 — ## 6. Reglas estratégicas
- L1585 — ## 7. Métricas del sistema
- L1606 — ## 8. El sistema respondiendo a las preguntas clave
...
- **Terminado cuando:** existe el gesto de despinar (hoy hueco funcional real,
  ...
  **[CRITERIO INVÁLIDO — CORREGIDO en S135 (R5), pendiente de REESCRIBIR.]** Las dos mitades de esta frase están cumplidas: `D-F8.6-ii-b` está CERRADA desde S83, medido en el M0 de S134, así que el gesto de despinar EXISTE. Leído al pie de la letra, este objetivo estaría terminado. Lo que falta no es deuda sino CRITERIO: hay que reescribirlo sobre los huecos funcionales que el guion de aceptación de H1 destape, y eso es trabajo de su propio M0. **Este objetivo NO está bloqueado por deuda alguna.**
...
| D-hallazgo-E-refutado (el Hallazgo E de `modelo_datos_fase1.md` afirma dos cosas que el centro y los datos desmienten) | O-demo | No | Nace en S134. GH6 no es tutor de 1º Bach A (dicho por el centro) y PTVE no lo imparte el tutor (Mejías Márquez lo da a 1B-B y 1B-D). Documento de dominio con estado falso: familia de `D-tokens-inexistentes`. No se corrige sin saber quién es el tutor real; espera a `D-tutores-bachillerato` |
| D-tutores-bachillerato (faltan los tutores de los 7 grupos de Bachillerato) | O-demo | No | Nace en S134. Deuda de REQUISITOS: el dato vive en el sistema del centro y en ningún PDF entregado. La heurística PTVE está refutada. Se cierra preguntando |
| D-nombres-sin-codigo (el PDF de profesores usa nombres completos; el catálogo, códigos) | O-demo | No | Nace en S134. Bloquea cargar los 21 tutores encontrados, no O-demo. Derivable por cruce de celdas, pero es trabajo |
| D-censo-profesores-80-59 (80 páginas de profesor frente a 59 profesores cargados) | O-demo | No | Nace en S134. Hipótesis SIN MEDIR: las 21 de más serían personal sin horario lectivo. Se anota como hipótesis, no como explicación |
| D-itinerario-como-grupo (un itinerario entero concentrado en un grupo administrativo, tercera forma de partición no modelada) | Candidata a O-particiones | No | Nace en S134 al medir 4º ESO: Humanidades/CCSS vive en 4ºC y por eso Latín y Economía son clase de grupo completo. No impide cargar ni generar; importa al CONSTRUIR un 4º por la UI. **Remodelar 4º ESO no es trabajo de O-demo** |
