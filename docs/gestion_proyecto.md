...
- L60 — ## 1. Estado final del proyecto
- L86 — ## 2. Hitos
- L100 — ### Hitos: valor, dependencias, orden
- L123 — ## 3. Objetivos técnicos
- L134 — ### H2 — Configurar un centro desde cero
- L136 — #### O-shell — "La aplicación es navegable." ✔ TERMINADO (S100)
- L152 — #### O-catálogo — "Creo los elementos simples del centro." ✔ TERMINADO (S106)
- L224 — #### O-estructura — "Expreso la complejidad real del centro." ✔ TERMINADO (S114)
- L474 — #### O-demo — "El centro real funciona de punta a punta." ✔ TERMINADO (S137)
- L761 — #### O-particiones — "Un grupo nuevo entra en el curso sin reconfigurar a mano." ✔ TERMINADO (S141)
- L862 — ### H1 — Ajustar (cierre)
- L864 — #### O-ajuste-cierre — "El ajuste manual está completo y verificado." ✔ TERMINADO (S146)
- L1041 — #### O-diseño — "La aplicación tiene un aspecto cuidado y coherente." ✔ TERMINADO (S133)
- L1332 — #### O-navegación — "La aplicación se maneja como una aplicación de escritorio." ✔ TERMINADO (S127)
- L1522 — ### H3 — Exportar
- L1524 — #### O-exportación — "El horario sale de la aplicación, en papel y en datos." ✔ TERMINADO (S150)
- L1585 — ### H4 — Instalar y pasar de curso
- L1587 — #### O-instalación — "La aplicación se instala, arranca y se cierra en un Windows limpio sin ayuda técnica." ABIERTO (S151)
- L1719 — ## 4. Clasificación del trabajo pendiente
- L1737 — ### Clasificación de las deudas vivas actuales
- L1743 — #### Objetivos disfrazados de deuda → se PROMUEVEN a objetivo (§3)
- L1750 — #### Deuda técnica real, colgada de su objetivo
- L1845 — #### Mejora futura, cuelga y espera
- L1879 — #### Decisión arquitectónica consciente → sale de la cola
- L1891 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L1900 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L1908 — #### Deuda ya CERRADA (histórico, no pendiente)
- L1975 — ## 5. Revisión del roadmap: por qué H2 va primero
- L2040 — ## 6. Reglas estratégicas
- L2099 — ## 7. Métricas del sistema
- L2120 — ## 8. El sistema respondiendo a las preguntas clave
...
#### O-instalación — "La aplicación se instala, arranca y se cierra en un Windows limpio sin ayuda técnica." ABIERTO (S151)
...
- **Terminado cuando** — ESCRITO en S151 sobre medición: M2 en Linux (clon limpio y
  ...
  3. **Windows limpio.** En Windows 10 y en Windows 11, sin Java ni Node y con una cuenta
     ESTÁNDAR (no miembro de Administradores), el bundle llegado por el medio de entrega
     que fije su Cambio se extrae y arranca con doble clic sin ningún diálogo que pida una
     decisión de seguridad o credenciales. Si ese medio produce el aviso de SmartScreen
     porque el ejecutable no va firmado, el aviso se documenta y se supera sin
     credenciales. Sobre una copia del banco, las cuatro descargas hechas desde la
     interfaz pasan el oráculo (1285/0/0, 835/0/0, 819/0/0 y CSV OK), y el CSV se abre en
     Excel con columnas separadas y tildes correctas. Es la verificación final de las
     demás condiciones.
  4. **Arranque visible e instancia única.** Al lanzarla, se abre el navegador en la
     aplicación. Un segundo lanzamiento no deja otra instancia viva: lleva a la que ya
     corre. Nace de la prueba del usuario en S151, que relanzó la aplicación porque no veía
     nada.
  5. **Cierre sin Administrador de tareas.** El usuario cierra la aplicación desde ella
     misma o desde un elemento visible de Windows. El cierre es ordenado (el log lo
     registra) y después no queda ningún proceso `Educhronos` ni el puerto ocupado. Hoy
     sólo se para matando el lanzador y su JVM.
  6. **Resultado visible.** Si el arranque falla (puerto ocupado, base inaccesible), el
     usuario ve un mensaje comprensible, y existe un log en una ubicación fija y
     documentada. Hoy el lanzador se desengancha de la consola y ningún error se ve.
  7. **Datos del usuario.** ✔ CUMPLIDA (S153). La base vive en una carpeta del usuario que
     ...
     Fase 10.
  8. **Sólo local.** La aplicación escucha sólo en la interfaz de bucle local. Hoy escucha
     en todas (`*:8080` en Linux, `::` en Windows), y en Windows eso provoca el aviso del
     cortafuegos al primer arranque. Hipótesis NO verificada: escuchando sólo en local, el
     aviso no aparece; lo comprueba la condición 3.
  9. **Rutas de la SPA.** F5 y una URL directa a una vista la sirven. Salda
     `D-spa-sin-fallback-de-rutas`.
...
- **Cambios que agrupa:** `C-alcance-instalacion` ✔ HECHO (S151), el M0 y las mediciones
  ...
  ✔ HECHO (S153), el post-procesador de la ruta de la base y la condición 7. Los demás se deciden en sus M0. Agrupación orientativa,
...
- **Decisiones que quedan para sus Cambios:** el medio de entrega y la firma del
  ejecutable (condicionan SmartScreen); el mecanismo de arranque y cierre (un icono en la
  bandeja del sistema es candidato). **La carpeta de datos ya no está pendiente: DECIDIDA en
...
| D-censo-r4-cuenta-menciones (el censo de R4 cuenta menciones igual que citas, así que una nota sobre un token huérfano lo rescata) | Transversal, con el script de R4 pendiente desde S101 | No | Nace en el cierre de S141 y se mide sola: el bullet de R4 registró que tres tokens bajaban a una cita tras el archivado, y al NOMBRARLOS les devolvió la segunda, con lo que el censo volvió de 29 a 26 en el mismo acto de documentarlo. El censo cuenta apariciones en el corpus vivo sin distinguir definición, cita de trabajo y mención incidental. Dos daños, y el segundo es el grave: una cifra registrada deja de ser reproducible con el instrumento que la produjo (familia de `D-declarado-sin-artefacto`), y **una mención en una entrada de sesión enmascara para siempre la orfandad futura de ese token**, que es justo lo que R4 existe para detectar. Es la familia de `D-guion-exit-enmascarado`: un instrumento que mide un conjunto que SE PARECE al que se cree medir. Arreglo candidato, para el script de R4: clasificar la aparición por su sede —definición en §4, cita de trabajo en código o en una ficha viva, mención en entrada de sesión— y no contar la tercera. Le da a ese script su caso más concreto desde S101 |
