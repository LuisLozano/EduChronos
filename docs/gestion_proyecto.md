...
- L1705 — #### Mejora futura, cuelga y espera
- L1739 — #### Decisión arquitectónica consciente → sale de la cola
- L1751 — #### Limitación conocida → sale de la cola, se documenta el "no se hará"
- L1760 — #### Deuda de MÉTODO → se integra en `metodo.md`, no en el producto
- L1767 — #### Deuda ya CERRADA (histórico, no pendiente)
- L1834 — ## 5. Revisión del roadmap: por qué H2 va primero
- L1899 — ## 6. Reglas estratégicas
- L1946 — ## 7. Métricas del sistema
- L1967 — ## 8. El sistema respondiendo a las preguntas clave
...
- **Terminado cuando** — ESCRITO en S147 sobre medición (M2 de solo lectura sobre el
  ...
  5. Sin dependencia de plataforma comprobable desde Linux: el PDF lleva incrustadas todas
     ...
     propósito, porque es nuestro bundle el que ha de funcionar en un Windows limpio. **MEDIA CUMPLIDA en S148:** el CSV declara su codificación por partida doble —BOM UTF-8 y `charset=UTF-8` en la respuesta—, verificado por bytes; falta el PDF y su `pdffonts`.
...
- **Cambios que agrupa:** `C-alcance-exportacion` ✔ HECHO (S147), el M0 y la medición que
  ...
  y media 5 y deja construido el ORÁCULO que la condición 2 de los tres PDF reutiliza. Los
