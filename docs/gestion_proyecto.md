...
- **Cambios que agrupa (creados en el M0 de S121; la ficha no los tenía, y crearlos
  ...
  - **C-revisión** — pasada por las tres vistas y los tres diálogos, M4 en navegador,
    ...
    - *Tramo 2 — el criterio 3.* PENDIENTE: los 195 valores de espaciado (171 caen exactos en un escalón,
      el 88 %), el radio de los once controles y `D-select-nativo-desparejo`, ya corregida y ampliada.
      Gobernado por decisiones escritas, así que es aplicación y no deliberación.
    - *Tramo 3 — el juicio.* PENDIENTE: la pasada del arquitecto por las once pantallas y los tres
      diálogos, más `D-tokens-sin-uso`. Cierra el objetivo.
    Queda escrito que si el tramo 2 resulta tan mecánico como promete el 88 %, los tramos 2 y 3 pueden
    fundirse —el M4 de la aplicación ES el recorrido por todas las pantallas—; lo decide su propio M0 con
    la UI aplicada delante.
...
| D-select-nativo-desparejo (los `<select>` salen con el estilo nativo del navegador y no casan con los `<input>`) | O-diseño, C-revisión | No | Nace en el M4 de S121, visto en el formulario de actividad: en la misma columna los `<input>` quedan blancos con borde de token y los `<select>` grises azulados por estilo de agente de usuario, porque hoy nadie los estila. NO es regresión de C-sustitución —era así antes, solo que con bordes `#666` se notaba menos—. Cae dentro del criterio de O-diseño (punto 3, coherencia entre vistas) y se paga en C-revisión. **CORREGIDA Y AMPLIADA en S129: su texto era FALSO.** Censado, **12 de los 14 `<select>` llevan la misma clase que los `<input>` de su formulario**, así que «nadie los estila» no describe el código. Lo cierto y no visto: los dos `<select>` de `horario-view.html:13` y `:22` no llevan clase ninguna; el `<button>` de `panel-prevalidacion` tampoco; y el desparejo GRANDE es otro —las diez reglas `__input` son nueve veces byte a byte idénticas y las diez llevan `border-radius: 4px` literal, que no es `--radio-s` (3px) ni `--radio-m` (6px)—. Queda NO SABIDO si un `<select>` con clase de `<input>` sigue pintando su flecha nativa, cosa que decide si alcanza a los 14 o sólo a los 2: nadie estila el elemento `select` en ninguna hoja, pero eso no se ha medido en navegador. Radio elegido por el arquitecto: `--radio-m`. Sede: **C-revisión, TRAMO 2** |
