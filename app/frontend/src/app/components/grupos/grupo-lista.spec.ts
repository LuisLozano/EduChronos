...
describe('GrupoLista', () => {
   ...
   * del botón caza el género —«Nuevo grupo» no se deriva de «Grupos»— que es lo
   ...
   */
  it('(16) monta la cabecera compartida con su rótulo y su texto de alta', async () => {
    flushLista([]);
    await fixture.whenStable();
    const cabecera = fixture.nativeElement.querySelector('app-cabecera-lista');
    expect(cabecera).toBeTruthy();
    expect(cabecera.querySelector('.cabecera-lista__titulo').textContent).toContain('Grupos');
    expect(cabecera.querySelector('.cabecera-lista__nuevo').textContent.trim()).toBe('Nuevo grupo');
  });
  ...
  /**
   * La búsqueda recorta la tabla y el contador lo dice (el código de nivel casa aunque se teclee en minúsculas).
   ...
   */
  it('(17) al buscar se ve solo la fila que casa, y el contador dice «1 de 2»', async () => {
    flushLista(FILAS);
    await fixture.whenStable();
    ...
    expect(fixture.nativeElement.textContent).toContain('2ESOB');
    expect(fixture.nativeElement.textContent).not.toContain('1ESOA');
    expect(
   ...
   */
  it('(18) una búsqueda sin resultados da su propio mensaje, no el de catálogo vacío', async () => {
    flushLista(FILAS);
    await fixture.whenStable();
